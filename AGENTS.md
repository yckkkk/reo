...
- UI 与动画改动必须先建立严谨但最小的数学模型：明确实体、坐标、不变量、源变量、派生位移/缩放/时间曲线，再实现；不得靠零散数值试调、互相覆盖的 transform 或隐含副作用堆出效果。
...
- 在需要对齐意图、写 spec 或写 plan 之前，必须先调用 `request_user_input` 或 `askUserQuestion` / `askuserquestion` 工具完成意图对齐；工具不可用时，先说明阻塞并等待用户确认，不得直接代替用户定稿。
...
- `rg` / search 只负责定位候选文件、符号或文本，不作为判断依据；命中的文件必须再用 `sed` / read 打开核对具体上下文、相邻代码和真实语义后再下结论。禁止只凭搜索摘要、匹配行或文件列表判断代码/文档是否存在、已清理、已覆盖或有问题。
...
TDD 是风险控制，不是仪式，也不是默认流程。多数简单或低风险改动不需要 TDD，强行套用是大题小做、浪费时间。开始改动前先判断风险面：只有命中下列高风险面时才执行真实 TDD，否则用范围内的轻量验证即可。
...
- 纯文档、机械配置、格式化、重命名、删除死代码、无行为变化的类型收窄、简单文案、简单样式或布局调整、已被现有测试覆盖的小改动默认豁免 TDD；用 typecheck、相关单测或运行时观察验证即可，只需在最终说明里一句话交代验证方式，不必为此新建 spec 或补写形式化测试。
...
- E2E 验证不得一次性生成覆盖整个流程的大测试。先列当前目标流程的状态机和不可变约束，再拆成多个小场景或多个可复查步骤，每步只验证一个关键转移、边界或副作用。
- 不得假覆盖：只跑完整点击路径、只断言页面出现文本或命令退出成功，不算有效覆盖。E2E 必须断言关键分支和副作用，例如状态转移、文件或 DB 变化、manifest/hash/cache 收敛、UI 投影、错误分类、回滚、恢复或外部可见输出。
- 当 E2E 暴露需要修复的高风险分支时，先把该分支抽成会先失败的 focused test；GREEN 后再用小步 E2E 验证真实 runtime 串联。

## 文档纪律

- 真源文档优先保持简洁：只记录长期有价值、稳定、跨任务复用的事实；不是每个决策或每次实现取舍都写入，能从源码、git、测试或 `package.json` 直接读出的内容不重复抄写。先判断这条信息半年后是否仍然有人需要从文档读到，否则不写。
...
`npm run verify:quick` 不在任务过程中频繁运行：它只在提交前或声明项目干净前运行一次。任务进行中用范围内的 targeted 检查（相关单元测试、typecheck）确认行为，避免反复跑全量验证浪费时间。
