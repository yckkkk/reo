...
test('workspace contract exposes only the explicit chooseDirectory channel', () => {
  assert.deepEqual(WORKSPACE_IPC_CHANNELS, [
    ...
    'workspace:listArtifactRuntimeSecretSlots',
    'workspace:getArtifactRuntimeSecret',
    'workspace:setArtifactRuntimeSecret',
    'workspace:clearArtifactRuntimeSecret',
  ...
  const namedChannelContracts: readonly (readonly [string, string])[] = [
    ...
    [WORKSPACE_GET_ARTIFACT_RUNTIME_SECRET_CHANNEL, 'workspace:getArtifactRuntimeSecret'],
    [WORKSPACE_SET_ARTIFACT_RUNTIME_SECRET_CHANNEL, 'workspace:setArtifactRuntimeSecret'],
    [WORKSPACE_CLEAR_ARTIFACT_RUNTIME_SECRET_CHANNEL, 'workspace:clearArtifactRuntimeSecret'],
...
test('workspace projection contract accepts artifact html segment and supplement', () => {
  const parsed = workspaceSegmentProjectionSchema.parse({
    ...
    cover: { source: 'default' },
    supplementCount: 1,
    contentTabOrder: ['segment', 'supplement:sup_artifact_1'],
    supplements: [
      {
        workspaceId: 'ws_1',
        memoryId: 'mem_1',
        segmentId: 'seg_artifact_1',
        supplementId: 'sup_artifact_1',
        type: 'artifact',
        format: 'html',
        title: '补充作品',
        createdAt: '2026-06-03T14:44:00.000Z',
    ...
    readonly supplements?: ReadonlyArray<Record<string, unknown>>;
  };

  assert.equal(parsed['type'], 'artifact');
  assert.equal(parsed['format'], 'html');
  assert.equal(parsed['entryHash'], 'a'.repeat(64));
  assert.equal(parsed.supplements?.[0]?.['type'], 'artifact');
  assert.deepEqual(
    (
      workspaceMemorySummarySchema.parse({
        memoryId: 'mem_1',
