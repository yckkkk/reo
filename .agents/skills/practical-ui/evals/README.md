...
This is **triggering accuracy**, not content quality. The book is a strong external design reference, but in real use its guidance still has to be reconciled with product context and local design-system truth through explicit design review when tensions appear.
...
python -m scripts.run_loop \
  --eval-set skills/practical-ui/evals/trigger-evals.json \
  --skill-path skills/practical-ui \
  --model claude-opus-4-7 \
  --max-iterations 5 \
  --verbose
```

(Replace `skills/practical-ui` with the concrete host path in your environment. Script path assumes you have the `skill-creator` installed — see `~/.claude/skills/skill-creator/scripts/run_loop.py`.)
