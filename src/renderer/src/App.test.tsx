...
describe('App', () => {
  ...
  const reoWorkspace = {
    ...
    readSegmentSpeechAudio: vi.fn(),
    readSegmentSupplementSpeechAudio: vi.fn(),
  ...
  function missingNoteSpeechSynthesis() {
    return {
  ...
  function readyNoteSpeechSynthesis() {
    return {
      status: 'ready' as const,
      audioByteLength: 3,
      contentHash: BASELINE_HASH_C,
      format: 'mp3' as const,
  ...
  function textClipboardData(text: string) {
    return {
      files: [],
  ...
  async function replaceRichEditorMarkdown(editor: HTMLElement, markdown: string) {
    ...
    fireEvent.paste(editor, { clipboardData: textClipboardData(markdown) });
  }

  function placeRichEditorCaretAfterText(editor: HTMLElement, text: string) {
    const walker = document.createTreeWalker(editor, NodeFilter.SHOW_TEXT);
    let current = walker.nextNode();
    while (current) {
      const index = current.textContent?.indexOf(text) ?? -1;
      if (index >= 0) {
        const offset = index + text.length;
        editor.focus();
    ...
    throw new Error(`Could not find editor text: ${text}`);
  }
  ...
  beforeEach(() => {
    ...
    reoWorkspace.readFinalizedAudioSegmentAudio.mockImplementation(async (payload) => ({
      ok: true,
      value: {
        ...
        audioHash: payload.audioHash ?? 'a'.repeat(64),
      },
    }));
    reoWorkspace.readFinalizedAudioSegmentSupplementAudio.mockImplementation(async (payload) => ({
      ok: true,
      value: {
        requestId: payload.requestId,
        workspaceId: payload.workspaceId,
    ...
    reoWorkspace.readVoiceTranscriptionSettings.mockResolvedValue({
      ...
      value: {
        settings: {
          ...
          speechSynthesisSpeaker: 'zh_female_vv_uranus_bigtts',
          lastTranscriptionValidatedAt: enabled ? '2026-05-09T09:00:00.000Z' : null,
          lastTranscriptionValidationOk: enabled ? true : null,
          lastTranscriptionValidationCode: enabled ? 'ok' : null,
          lastSpeechSynthesisValidatedAt: enabled ? '2026-05-09T09:00:00.000Z' : null,
  ...
  type AudioSegmentSupplementFixture = Extract<
    ...
    { type: 'audio' }
  >;
  type NoteSegmentFixture = Extract<WorkspaceMemoryDetail['segments'][number], { type: 'note' }>;
  type NoteSegmentSupplementFixture = Extract<
    WorkspaceMemoryDetail['segments'][number]['supplements'][number],
    { type: 'note' }
  >;
  ...
  function createNoteSupplementFixture(): NoteSegmentSupplementFixture {
    return {
      workspaceId: 'ws_1',
      memoryId: 'mem_birthday',
  ...
  function createNoteSegmentFixture({
    supplement,
  }: {
    readonly supplement?: NoteSegmentSupplementFixture;
  } = {}) {
    const memory = {
      memoryId: 'mem_birthday',
      title: 'My seventh birthday',
    ...
    return { memory, segment, snapshot };
  }

  function mockLoadedNoteWorkspace({
    memory,
    segment,
    snapshot,
  }: ReturnType<typeof createNoteSegmentFixture>) {
    reoWorkspace.chooseDirectory.mockResolvedValue({
      ok: true,
      value: {
        status: 'selected',
  ...
  function mockNoteSegmentContent(
    getContent: () => Omit<
      WorkspaceNoteSegmentContent,
      'memoryId' | 'requestId' | 'segmentId' | 'speechSynthesis' | 'title' | 'type' | 'workspaceId'
    >
  ) {
  ...
  function mockNoteSegmentSupplementContent(
    getContent: () => Omit<
      WorkspaceNoteSegmentSupplementContent,
      | 'memoryId'
      | 'requestId'
  ...
  it('opens the rich dev workspace scenario from the URL parameter', async () => {
    ...
    expectRichEditorContent(await screen.findByLabelText('转录正文'), '真实 UI 调试转录正文');
  ...
  it('requests segment transcription backfill from the Segment menu while the inline editor stays visible', async () => {
    ...
    expectRichEditorContent(await within(content).findByLabelText('转录正文'), '');
  ...
  it('requests note segment speech synthesis and refreshes the note speech player', async () => {
    const user = userEvent.setup();
    const createObjectURL = vi.spyOn(URL, 'createObjectURL').mockReturnValue('blob:note-speech');
    mockVoiceTranscriptionSettings(true);
    const fixture = createNoteSegmentFixture();
    mockLoadedNoteWorkspace(fixture);
    ...
    reoWorkspace.readSegmentContent.mockImplementation(async (payload) => ({
      ...
      value: {
        ...
        segmentId: payload.segmentId,
        type: 'note',
        title: fixture.segment.title,
        bodyMarkdown: '笔记正文',
        bodyByteLength: 12,
        baselineContentHash: BASELINE_HASH_A,
        baselineTiptapContentHash: BASELINE_TIPTAP_HASH_A,
        bodyTiptapJson: noteTiptapDoc('笔记正文'),
        speechSynthesis: speechReady ? readyNoteSpeechSynthesis() : missingNoteSpeechSynthesis(),
      },
    }));
    reoWorkspace.readSegmentSpeechAudio.mockImplementation(async (payload) =>
      speechReady
        ? {
            ok: true,
            value: {
    ...
    reoWorkspace.requestSegmentSpeechSynthesis.mockImplementation(async () => {
      speechReady = true;
      return {
        ok: true,
        value: { speechSynthesis: readyNoteSpeechSynthesis() },
      };
    });
    ...
    await screen.findByText('Memory');
    await user.click(screen.getByRole('button', { name: '创建' }));
    expectRichEditorContent(await screen.findByLabelText('笔记正文'), '笔记正文');

    await user.click(
      screen.getByRole('button', { name: `片段 ${fixture.segment.title} 更多操作` })
    );
    await user.click(screen.getByRole('menuitem', { name: '生成/重新生成语音' }));
    ...
    expect(await screen.findByText('已生成语音')).toBeInTheDocument();
    const content = screen.getByRole('region', { name: '片段内容' });
    expect(
      await within(content).findByRole('button', { name: `播放笔记语音 ${fixture.segment.title}` })
    ).toBeInTheDocument();
    expect(reoWorkspace.readSegmentContent.mock.calls.length).toBeGreaterThanOrEqual(2);
    expect(
  ...
  it('ignores an in-flight segment transcription backfill response after reopening the same workspace with a new handle', async () => {
    ...
    expectRichEditorContent(await within(reopenedContent).findByLabelText('转录正文'), '');
    await user.click(screen.getByRole('button', { name: `片段 ${segment.title} 更多操作` }));
    await user.click(screen.getByRole('menuitem', { name: '生成转录' }));
  ...
  it('requests supplement transcription backfill from the Supplement menu while the inline editor stays visible', async () => {
    ...
    expectRichEditorContent(await screen.findByLabelText('补充录音转录正文'), '');
  ...
  it('confirms before regenerating an existing supplement transcript', async () => {
    ...
    expectRichEditorContent(await screen.findByLabelText('补充录音转录正文'), '补充录音转写正文');
    ...
    expectRichEditorContent(screen.getByLabelText('补充录音转录正文'), '补充录音转写正文');
    backfill.resolve({
      ok: true,
      value: {
        memory: fixture.memory,
  ...
  it('confirms before regenerating an existing segment transcript', async () => {
    ...
    expectRichEditorContent(await screen.findByLabelText('转录正文'), '已有转录正文');
    ...
    expectRichEditorContent(screen.getByLabelText('转录正文'), '已有转录正文');
  ...
  it.each([
    [
      'voice recognition is disabled',
      {
        ...
        apiKeyLastFour: '1234',
        speechSynthesisSpeaker: 'zh_female_vv_uranus_bigtts',
        lastTranscriptionValidatedAt: '2026-05-16T18:00:00.000Z',
        lastTranscriptionValidationOk: true,
        lastTranscriptionValidationCode: 'ok',
        lastSpeechSynthesisValidatedAt: '2026-05-16T18:00:00.000Z',
  ...
  it('uses the titlebar Memory control to collapse and expand the workspace Memory list', async () => {
    stubWorkspaceRailInlineMedia(true);
    const user = userEvent.setup();
    reoWorkspace.chooseDirectory.mockResolvedValue({
      ok: true,
      value: {
  ...
  it('blocks settings navigation and window unload while inline transcript edits are dirty', async () => {
    ...
    await replaceRichEditorMarkdown(inlineTranscriptEditor, '修正后的转录');
    ...
    expect(screen.queryByRole('region', { name: '语音设置' })).not.toBeInTheDocument();
    expectRichEditorContent(screen.getByLabelText('转录正文'), '修正后的转录');
  }, 10_000);
  ...
  it('refreshes non-body workspace projections while a dirty Note editor stays open', async () => {
    ...
    await replaceRichEditorMarkdown(body, 'My dirty editor body');
    currentNoteContent = {
      bodyByteLength: 21,
    ...
    expectRichEditorContent(body, 'My dirty editor body');
    expect(
  ...
  it('reloads a clean Note editor body from visibility refresh content', async () => {
    ...
    const editor = await openInlineNoteEditor(user);
    const body = within(editor).getByLabelText('笔记正文');
    expectRichEditorContent(body, 'Disk before edit');
    currentNoteContent = {
      bodyByteLength: 21,
      bodyMarkdown: 'Disk changed by agent',
      baselineContentHash: BASELINE_HASH_B,
    ...
    await waitFor(() => expectRichEditorContent(body, 'Disk changed by agent'));
    expect(
  ...
  it('reloads clean Note editor JSON-only rich marks from file truth events without reselect', async () => {
    ...
    mockLoadedNoteWorkspace(noteWorkspace);
    mockNoteSegmentContent(() => currentNoteContent);
  ...
  it('reloads a clean Note SegmentSupplement body from file truth events without reselect', async () => {
    const user = userEvent.setup();
    let fileTruthChanged: Parameters<Window['reoWorkspace']['onFileTruthChanged']>[0] | null = null;
    const noteWorkspace = createNoteSegmentFixture({
      supplement: createNoteSupplementFixture(),
    });
    let currentSupplementContent = {
      bodyByteLength: 25,
      bodyMarkdown: 'Supplement before file event',
      bodyTiptapJson: noteTiptapDoc('Supplement before file event'),
      baselineContentHash: BASELINE_HASH_A,
      baselineTiptapContentHash: BASELINE_TIPTAP_HASH_A,
    };
    reoWorkspace.onFileTruthChanged.mockImplementation((listener) => {
    ...
    mockLoadedNoteWorkspace(noteWorkspace);
    mockNoteSegmentContent(() => ({
      bodyMarkdown: 'Parent note',
      bodyTiptapJson: noteTiptapDoc('Parent note'),
      bodyByteLength: 11,
      baselineContentHash: BASELINE_HASH_C,
      baselineTiptapContentHash: BASELINE_TIPTAP_HASH_C,
    }));
    mockNoteSegmentSupplementContent(() => currentSupplementContent);
    ...
    await screen.findByText('Memory');
    await user.click(screen.getByRole('button', { name: '创建' }));
    const supplementTab = await screen.findByRole('tab', { name: '补充笔记1' });
    await user.click(supplementTab);
    const editor = await openInlineSupplementNoteEditor(user);
    const body = within(editor).getByLabelText('补充笔记正文');
    expectRichEditorContent(body, 'Supplement before file event');
    await waitFor(() =>
      expect(reoWorkspace.readWorkspaceSnapshot.mock.calls.length).toBeGreaterThan(0)
    );
    const snapshotReadCountBeforeFileEvent = reoWorkspace.readWorkspaceSnapshot.mock.calls.length;
    const supplementReadCountBeforeFileEvent =
      reoWorkspace.readSegmentSupplementContent.mock.calls.filter(
        ([payload]) => payload.supplementId === 'sup_note_1'
      ).length;
    currentSupplementContent = {
      bodyByteLength: 31,
      bodyMarkdown: 'Supplement changed by agent',
      bodyTiptapJson: noteTiptapDoc('Supplement changed by agent'),
      baselineContentHash: BASELINE_HASH_B,
      baselineTiptapContentHash: BASELINE_TIPTAP_HASH_B,
    };
    ...
    await waitFor(() => expectRichEditorContent(body, 'Supplement changed by agent'));
    expect(reoWorkspace.readWorkspaceSnapshot.mock.calls.length).toBeGreaterThan(
      snapshotReadCountBeforeFileEvent
    );
    expect(
    ...
    ).toBeGreaterThan(supplementReadCountBeforeFileEvent);
  });
  ...
  it('detects body-only Note editor refresh when the Workspace snapshot is unchanged', async () => {
    ...
    const editor = await openInlineNoteEditor(user);
    const body = within(editor).getByLabelText('笔记正文');
    await replaceRichEditorMarkdown(body, 'Local body');
    currentNoteContent = {
      bodyByteLength: 16,
    ...
    expectRichEditorContent(body, 'Local body');
    const contentInvalidation = invalidateSpy.mock.calls
      ...
      .find(
        (options) =>
          ...
          options.predicate({ queryKey: memoryDetailKey } as never)
      );
  ...
  it('keeps cached Note Segment body on selection until file truth invalidates it', async () => {
    const user = userEvent.setup();
    const memory = {
      memoryId: 'mem_birthday',
      title: 'My seventh birthday',
    ...
    expect(await screen.findByText('Segment one before')).toBeInTheDocument();
    expect(
      reoWorkspace.readSegmentContent.mock.calls.filter(
        ([payload]) => payload.segmentId === 'seg_note_1'
      )
  ...
  it('reuses cached Memory detail while reopening the same workspace refetches with the new handle', async () => {
    ...
    const reopenedSegmentContent =
      createDeferred<Awaited<ReturnType<Window['reoWorkspace']['readFinalizedAudioSegment']>>>();
    ...
    reoWorkspace.readFinalizedAudioSegment.mockImplementation((payload) => {
      if (payload.workspaceHandle === 'workspace-handle-2') {
        return reopenedSegmentContent.promise;
      }
      ...
      return Promise.resolve({
        ...
        value: {
          ...
          transcript: {
            exists: true,
            text:
              payload.segmentId === 'seg_fresh'
                ? 'Fresh reopened transcript.'
                : 'Cached reopened transcript.',
            baselineHash: BASELINE_HASH_A,
          },
        },
      });
    ...
    expect(await screen.findByText('Cached reopened transcript.')).toBeInTheDocument();
    ...
    expect(screen.getByText('Cached reopened transcript.')).toBeInTheDocument();
    expect(reoWorkspace.readMemoryDetail).toHaveBeenCalledTimes(2);
    ...
    await waitFor(() =>
      expect(
        reoWorkspace.readFinalizedAudioSegment.mock.calls.some(
          ([payload]) => payload.workspaceHandle === 'workspace-handle-2'
        )
      ).toBe(true)
    );
    const reopenedContentCall = reoWorkspace.readFinalizedAudioSegment.mock.calls.find(
      ([payload]) => payload.workspaceHandle === 'workspace-handle-2'
    )?.[0];
    expect(reopenedContentCall).toMatchObject({
      workspaceHandle: 'workspace-handle-2',
      workspaceId: 'ws_1',
      memoryId: memory.memoryId,
      segmentId: cachedSegment.segmentId,
    });

    await act(async () => {
      if (!reopenedContentCall) {
        throw new Error('Expected reopened Segment content request');
      }
      reopenedSegmentContent.resolve({
        ok: true,
        value: {
          requestId: reopenedContentCall.requestId,
          workspaceId: reopenedContentCall.workspaceId,
          memoryId: reopenedContentCall.memoryId,
          segmentId: reopenedContentCall.segmentId,
          audioByteLength: 3,
          audioHash: BASELINE_HASH_B,
          transcript: {
            exists: true,
            text: 'Refetched reopened transcript.',
            baselineHash: BASELINE_HASH_B,
            tiptapJson: {
              type: 'doc',
              content: [
                {
                  type: 'paragraph',
                  content: [{ type: 'text', text: 'Refetched reopened transcript.' }],
                },
              ],
            },
            baselineTiptapContentHash: BASELINE_TIPTAP_HASH_B,
      ...
      await reopenedSegmentContent.promise;
    });
    expect(await screen.findByText('Refetched reopened transcript.')).toBeInTheDocument();
  ...
  it('finalizes a FAB Note against the current selected memory without creating a new memory', async () => {
    ...
    expect(within(dialog).queryByRole('button', { name: '添加图片' })).toBeNull();
    expect(within(dialog).queryByLabelText('图片附件')).toBeNull();
    expect(reoWorkspace.createNoteSegmentDraft).not.toHaveBeenCalled();
    await replaceRichEditorMarkdown(noteBody, '## Cake plan\n\n- Buy candles');
    ...
    expectRichEditorContent(await screen.findByLabelText('笔记正文'), ['Cake plan', 'Buy candles']);
  ...
  it('confirms before closing a dirty FAB Note without creating a draft', async () => {
    ...
    await user.click(screen.getByRole('menuitem', { name: '笔记' }));
    const dialog = await screen.findByRole('dialog', { name: '笔记' });
    await replaceRichEditorMarkdown(within(dialog).getByLabelText('笔记正文'), 'Unsaved note body');
    ...
    expectRichEditorContent(within(dialog).getByLabelText('笔记正文'), 'Unsaved note body');
  ...
  it('retries a FAB Note save with the latest draft revision after finalize fails', async () => {
    ...
    await user.click(screen.getByRole('menuitem', { name: '笔记' }));
    const dialog = await screen.findByRole('dialog', { name: '笔记' });
    await replaceRichEditorMarkdown(
      within(dialog).getByLabelText('笔记正文'),
      '## Cake plan\n\n- Buy candles'
    );
    await user.click(within(dialog).getByRole('button', { name: '保存笔记' }));
    ...
    expectRichEditorDisabled(within(dialog).getByLabelText('笔记正文'));
  ...
  it('does not create a Note draft when the create overlay is closed before saving', async () => {
    ...
    await user.click(screen.getByRole('menuitem', { name: '笔记' }));
    const dialog = await screen.findByRole('dialog', { name: '笔记' });
    await replaceRichEditorMarkdown(within(dialog).getByLabelText('笔记正文'), 'Uncommitted note');
    await user.click(within(dialog).getByRole('button', { name: '返回' }));
  ...
  it('keeps dirty Note segment edits intact when save detects an external conflict', async () => {
    ...
    expectRichEditorContent(body, 'Disk changed by agent');
  ...
  it('reloads a clean Note segment editor after accepted disk content later returns to the opening hash', async () => {
    ...
    await waitFor(() => expectRichEditorContent(body, 'Disk reverted after conflict'));
    expect(
      within(editor).queryByText('磁盘内容已变化。保存时将进行冲突检查。')
    ).not.toBeInTheDocument();
  });
  ...
  it('inserts a dropped image attachment reference at the finalized Note cursor', async () => {
    ...
    reoWorkspace.writeSegmentContent.mockResolvedValue({
      ok: true,
      value: {
        bodyByteLength: initialBodyMarkdown.length + insertedAttachmentMarkdown.length,
        saved: true,
      },
    });
    ...
    const editor = await openInlineNoteEditor(user);
    const body = within(editor).getByLabelText('笔记正文');
    expect(within(body).getAllByRole('img', { name: 'Cake Plan' })).toHaveLength(1);
    placeRichEditorCaretAfterText(body, 'Alpha ');

    expect(within(editor).queryByRole('button', { name: '插入图片' })).toBeNull();
    expect(within(editor).getByRole('button', { name: '添加图片' })).toBeEnabled();
    ...
    await waitFor(() => expectRichEditorDisabled(body));
    expectRichEditorContent(body, ['Intro', 'Alpha bravo', 'Charlie']);
    ...
    const expectedBodyMarkdown = [
      `Intro ${existingAttachmentMarkdown}`,
      'Alpha ',
      insertedAttachmentMarkdown,
      'bravo',
      'Charlie',
    ].join('\n\n');
    await waitFor(() =>
    ...
    await waitFor(() =>
      expect(reoWorkspace.writeSegmentContent).toHaveBeenCalledWith(
        expect.objectContaining({
          bodyMarkdown: expectedBodyMarkdown,
        })
      )
    );
  });
  ...
  it('inserts a pasted image attachment reference at the finalized Note cursor', async () => {
    ...
    fireEvent.paste(body, {
      clipboardData: filesClipboardData([
        new File([new Uint8Array([4, 5, 6])], '', { type: 'image/png' }),
      ]),
    });
    ...
    await waitFor(() =>
      expect(within(body).getByRole('img', { name: 'pasted-image' })).toHaveAttribute(
        'src',
        'reo-attachment://ws_1/segments/seg_note_1/cafefeed1234--paste-image.png'
      )
    );
  });
  ...
  it('reenables the finalized Note body when pasted image attachment save throws', async () => {
    ...
    fireEvent.paste(body, {
      clipboardData: filesClipboardData([
        new File([new Uint8Array([4, 5, 6])], 'Paste Image.png', { type: 'image/png' }),
      ]),
    });

    await waitFor(() => expect(reoWorkspace.saveSegmentAttachment).toHaveBeenCalledTimes(1));
    await waitFor(() => expectRichEditorEnabled(body));
    expectRichEditorContent(body, 'Alpha bravo');
    expect(screen.getByRole('status')).toHaveTextContent('无法插入图片附件。');
  });

  it('shows Note Segment attachment image references as editable image nodes', async () => {
    const user = userEvent.setup();
    const birthdayMemory = {
      memoryId: 'mem_birthday',
      title: 'My seventh birthday',
    ...
    expect(within(body).queryByText('![Local cake](attachments/cake.png)')).not.toBeInTheDocument();
    expect(within(body).getByRole('img', { name: 'Local cake' })).toHaveAttribute(
      'src',
      'reo-attachment://ws_1/segments/seg_note_1/cake.png'
    );
    for (const name of ['Remote cake', 'File cake', 'Absolute cake']) {
      const unsupportedImage = within(body).getByRole('img', { name });
  ...
  it('shows Note SegmentSupplement attachment image references as editable image nodes', async () => {
    const user = userEvent.setup();
    const birthdayMemory = {
      memoryId: 'mem_birthday',
      title: 'My seventh birthday',
    ...
    const body = await screen.findByLabelText('补充笔记正文');
    expect(
      within(body).queryByText('![Supplement local](attachments/supplement-photo.png)')
    ).not.toBeInTheDocument();
    expect(within(body).getByRole('img', { name: 'Supplement local' })).toHaveAttribute(
      'src',
      'reo-attachment://ws_1/segments/seg_note_1/supplements/sup_note_1/supplement-photo.png'
    );
    const unsupportedImage = within(body).getByRole('img', { name: 'Supplement remote' });
    expect(unsupportedImage).not.toHaveAttribute('src');
    expect(unsupportedImage).toHaveAttribute('data-reo-image-source', 'unsupported');
  });
  ...
  it('refreshes an active SegmentSupplement panel after its transcript is saved', async () => {
    ...
    expect((await screen.findByLabelText('补充录音转录正文')).textContent).toBe('');
    await waitFor(() =>
      expect(reoWorkspace.saveSegmentSupplementTranscript).toHaveBeenCalledWith({
        workspaceHandle: 'workspace-handle-1',
        workspaceId: 'ws_1',
    ...
    await waitFor(() =>
      expectRichEditorContent(screen.getByLabelText('补充录音转录正文'), '现场补充转写')
    );
  }, 10_000);
