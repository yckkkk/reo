...
function useFileUpload(options: UploadOptions) {
  ...
  const setSafeFileItems = (updater: (items: FileItem[]) => FileItem[]) => {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      fileItemsRef.current.forEach((item) => {
        ...
        if (item.url && isObjectUrl(item.url)) {
  ...
  const uploadFile = async (file: File): Promise<UploadedFile | null> => {
    if (file.size > options.maxSize) {
      const error = new Error(`文件大小超过上限（${options.maxSize / 1024 / 1024}MB）`);
    ...
    setSafeFileItems((prev) => [...prev, newFileItem]);
  ...
  const uploadFiles = async (files: File[]): Promise<UploadedFile[]> => {
    ...
    if (options.limit && files.length > options.limit) {
      options.onError?.(new Error(`最多只能上传 ${options.limit} 个文件`));
      return [];
    }
  ...
  const removeFileItem = (fileId: string) => {
    setSafeFileItems((prev) => {
      ...
      if (fileToRemove?.url && isObjectUrl(fileToRemove.url)) {
