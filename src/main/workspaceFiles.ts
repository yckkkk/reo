...
export const DEFAULT_REO_GENERATIVE_RUNTIME_SKILL_MD =
  [
    ...
    'description: Shared Reo generative runtime skill for building small local Web app bundles used by works and future components. Use for entry.html/runtime.json/state.json/assets, window.reo bridge, state, secrets, templates, network, scaffold and validation.',
    '---',
    '',
    ...
    '- `runtime.json`: description, entry, template family, state stores, bridge needs and secret slots.',
    ...
    '## State',
    '',
    '`state.json` is the durable agent-readable state file. Runtime code may read and write it through `window.reo.state` with a version/baseline contract. Browser storage such as localStorage and IndexedDB is allowed for interactive state and compatibility cache.',
    '',
    'Read `references/state-and-storage.md` for store naming, versioning and merge rules.',
    '',
    '## Bridge',
    '',
    'To use Reo data, state, secret, UI, mutation or agent prompt actions, explicitly load `reo-artifact://vendor/reo-runtime/bridge.js` from `entry.html`. This provides `window.reo` inside the iframe. Do not invent any other host bridge.',
    '',
...
const DEFAULT_REO_GENERATIVE_RUNTIME_STATE_REFERENCE_MD =
  [
    ...
    '## Browser persistence',
    '',
    'Each runtime object has its own origin, so localStorage and IndexedDB are isolated per object. Use browser storage for fast UI cache when helpful. Keep `state.json` as the visible durable state that users and agents can inspect and modify.',
    '',
    '## Agent updates',
    '',
...
const DEFAULT_REO_GENERATIVE_RUNTIME_BRIDGE_REFERENCE_MD =
  [
    ...
    '',
    'Use this reference when a work needs live Reo context, durable state, secret values, host UI coordination, typed product writes or agent prompt actions.',
    '',
    '## Setup',
    '',
    ...
    '- `window.reo.secrets.list/get/set/clear` for current object secret slots declared in `runtime.json`.',
...
export const DEFAULT_REO_WORKS_SKILL_MD =
  [
    ...
    '- 作品需要 Reo state、content、secret、mutation、fullscreen 或 agent prompt action 时，使用 `window.reo` documented bridge；不要发明其他宿主 API。',
...
export const DEFAULT_REO_WORKS_DESIGN_CORE_REFERENCE_MD =
  [
    ...
    '- Use documented `window.reo` bridge calls when the work needs Reo state, content, secrets, UI, mutations or agent prompt actions.',
