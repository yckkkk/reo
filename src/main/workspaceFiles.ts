...
export const DEFAULT_REO_GENERATIVE_RUNTIME_SKILL_MD =
  [
    ...
    'description: Shared Reo generative runtime skill for building small local Web app bundles used by works and future components. Use for entry.html/runtime.json/state.json/assets, state, templates, network, scaffold and validation.',
    '---',
    '',
    ...
    '- `runtime.json`: description, entry, template family, state stores and future bridge needs.',
    ...
    '## State',
    '',
    '`state.json` is the durable agent-readable state file. Browser storage such as localStorage and IndexedDB is allowed for interactive state and compatibility cache. In the current M2.1 slice, runtime code should not assume a Reo write bridge exists; if you need live in-app persistence, use localStorage and keep `state.json` as the editable source/seed for agents.',
    '',
...
const DEFAULT_REO_GENERATIVE_RUNTIME_BUNDLE_CONTRACT_REFERENCE_MD =
  [
    ...
    '```',
    '',
    'For supplements, use the same fields in `supplement.md` with a `sup_...` id.',
    '',
    '## runtime.json',
    '',
    'Minimum shape:',
...
const DEFAULT_REO_GENERATIVE_RUNTIME_STATE_REFERENCE_MD =
  [
    ...
    '## Browser persistence',
    '',
    'Each runtime object has its own origin, so localStorage and IndexedDB are isolated per object. Use browser storage for live in-app persistence when the user interacts with the work. Keep `state.json` as the visible seed/export file that users and agents can inspect and modify.',
    '',
    '## Agent updates',
    '',
...
const DEFAULT_REO_GENERATIVE_RUNTIME_TEMPLATES_REFERENCE_MD =
  [
    ...
    '- todo: task list with localStorage persistence and visible progress.',
...
export const DEFAULT_REO_GENERATIVE_RUNTIME_SCAFFOLD_SCRIPT_MJS =
  [
    ...
    '}',
    '',
    'await writeNoReplace(path.join(target, "runtime.json"), `${JSON.stringify({ schemaVersion: 1, title, entry: "entry.html", template, state: { schemaVersion: 1, stores: ["ui", "data", "progress", "draft"] }, bridge: { needs: [] }, secrets: [] }, null, 2)}\\n`);',
...
export const DEFAULT_REO_GENERATIVE_RUNTIME_VALIDATE_SCRIPT_MJS =
  [
    ...
    '        if (!entry.isFile()) add("asset-not-file", `assets/${entry.name}`, "Assets must be ordinary direct files in M2.1.");',
    '      }',
    '    }',
    '  } catch {',
...
export const DEFAULT_REO_WORKS_SKILL_MD =
  [
    ...
    '4. 按 `skills/reo-generative-runtime/SKILL.md` 写入同目录 runtime bundle：`entry.html`、`runtime.json`、`state.json`、`assets/`。',
    '5. 可先运行 `node skills/reo-generative-runtime/scripts/scaffold-runtime.mjs <segment-directory> --title "标题" --template <family>`，再把 scaffold 改成用户需要的作品。',
    '',
    '最小形态：',
    '',
    '```markdown',
    ...
    '- `state.json` 是用户和 agent 可查看、可修改的状态文件；实时交互也可以使用 localStorage/IndexedDB。',
...
export const DEFAULT_REO_WORKS_DESIGN_SKILL_MD =
  [
    ...
    '',
    '- `references/core-design-system.md`：Reo 作品 tokens、排版、颜色、深色模式和 runtime 边界。',
    ...
    '- 数据更新优先通过 agent 后续重写 `state.json` / `entry.html`，或由作品使用 browser storage 保存交互状态；Reo live bridge 进入后续 runtime 切片。',
  ].join('\n') + '\n';
...
export const DEFAULT_REO_WORKS_WORKFLOWS_REFERENCE_MD =
  [
    ...
    '5. If the form has visual or interaction complexity, read `skills/reo-works-design/SKILL.md` and the specific design reference.',
...
export const DEFAULT_REO_WORKS_DESIGN_CORE_REFERENCE_MD =
  [
    ...
    '- No Reo host bridge dependency in M2.1. Do not invent `window.reo` calls until the bridge exists.',
...
export const DEFAULT_REO_WORKS_DESIGN_INTERACTIONS_REFERENCE_MD =
  [
    ...
    '',
    '- No invented chat bridge, prompt sending API or Reo host mutation API.',
