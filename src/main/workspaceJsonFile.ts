...
export function readBoundedJsonNoFollowSync<T>({
  beforeFinalAssert,
  filePath,
  maxBytes,
  schema,
}: {
  readonly beforeFinalAssert?: (() => void) | undefined;
  readonly filePath: string;
  readonly maxBytes: number;
  readonly schema: z.ZodType<T>;
}): WorkspaceJsonReadResult<T> {
  let fileDescriptor: number | null = null;
  const directory = path.dirname(filePath);
  let directoryIdentity: ReturnType<typeof readSafeDirectoryIdentitySync>;

  try {
    directoryIdentity = readSafeDirectoryIdentitySync(directory);
  } catch (error) {
    return errorCode(error) === 'ENOENT' ? { status: 'missing' } : { status: 'invalid' };
  }
  ...
  try {
    ...
    const value = schema.parse(JSON.parse(readFileSync(fileDescriptor, 'utf8')));
    beforeFinalAssert?.();
    assertSameDirectoryIdentitySync(directory, directoryIdentity);
    return { status: 'ok', value };
  } catch (error) {
    const code = errorCode(error);
    if (code === 'ENOENT') {
    ...
    if (fileDescriptor !== null) {
      try {
        ...
        // Read callers fall back to default state after a close failure.
      }
    }
  }
}
