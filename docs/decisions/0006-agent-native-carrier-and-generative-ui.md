...
- **引导 skills**：基于当前 Memory 的累积材料，引导用户用最合适的模态做下一步补充——可能是补录音、拍照、写笔记、上传文件
- **回顾 skills**：结合记忆曲线 + 随机推送式回顾节奏，从所有模态材料中挑出值得回顾、值得复述、值得深化的内容
- **整理总结 skills**：对 Memory / 多 Memory 范围的跨模态材料做摘要、提取、归纳、关联
- **Widget 生成 skills**：让 agent 基于 Memory 的多模态材料 + example 库生成新 widget（例如成长册、概念时间线、关系曲线、引言库）
