...
Renderer 调用特权能力时，只能通过窄 `contextBridge` API 和显式 IPC channel 暴露。当前 preload API 是 `window.reoWorkspace`，IPC surface 是 记忆空间文件、记忆空间列表、录音、audio playback 和 microphone intent 所需的显式 channels。

## 原因

- 当前工程只需要稳定的 Electron dev/build 基线。
- Packaging 生命周期会改变 entry、输出、ASAR、fuses、签名和发布边界。
- 空 preload、空 IPC、通用 IPC 和 Forge config 会扩大当前 surface。
...
- 新增或扩展 IPC 必须定义 owner、contract、runtime validation、sender validation 和错误语义。
