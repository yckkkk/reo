...
- Reo 打开、文件真源刷新和 index rebuild 时从当前文件真源重建 Memory read model 并持久化 `.reo/index.json`；显式 `readWorkspaceSnapshotFromIndex` 只用于读取缓存快照，不代表打开空间可以信任旧 index。重建时只对 `memories/*/segments/*/segment.md`、`memories/*/segments/*/supplements/*/supplement.md` 和 `widgets/*/widget.md` 做浅层有界候选识别。缺少 frontmatter 的 note Segment 或 note SegmentSupplement 可以在身份唯一且路径安全时被补全 `id`、`title`、`kind: note` 和 manifest mirror；新的 `kind: artifact` 候选必须声明 `format: html` 并存在可读、有界的同目录 `entry.html` 才能建立 manifest 并进入 ready 投影，缺入口、入口过大或 unsupported format 进入 needs-review。新的 `kind: widget` 候选必须声明 `format: html` 和 `mount: workspace-rail`，并存在可读、有界的同目录 `entry.html` 才能进入 Workspace snapshot；缺入口、入口过大、unsupported format 或 unsupported mount 进入 needs-review。已存在 manifest 的 artifact 入口后来缺失或过大时，在 Memory detail 中保留 fault 投影而不让用户失去修复入口；已识别 Widget 的入口后来缺失或过大时，在 Workspace snapshot 中保留 fault 投影。`runtime.json`、`state.json` 和 `assets/` 是 agent 与 runtime 使用的同目录 bundle 文件；缺失这些文件不阻止已有 `entry.html` 作品或 Widget 被投影，但托管 skill 的验证脚本会提示补齐。`runtime.json` 可声明 bridge needs；Reo 不为 artifact/Widget runtime 提供隐藏 key、token 或 value 存储。
