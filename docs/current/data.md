...
- 当前 durable data contract 是记忆空间文件夹文件、main-owned memory space registry、main-owned system Draft store 和 main-owned Home Component app state。
- 当前 TanStack Query key 覆盖 memory space list、system Draft projection、recent expression feed、Home Component list、Workspace snapshot、Memory detail、selected Segment content、selected SegmentSupplement content、playback audio bytes、note speech audio bytes 和 voice transcription settings。
- 当前没有 Zustand stores。
...
- `.reo/workspace.json` 保存 `schemaVersion`、`workspaceId`、title、description、createdAt 和可选 `widgetTabOrder`；它定义记忆空间身份与 Widget tab durable presentation order，不保存当前 active rail tab、root path、handle 或查询缓存。
- Workspace snapshot 是 IPC response 和 TanStack Query cache 投影，只包含 `workspaceId`、title、description、`memories[]`、可选 `widgets[]` 和 review 汇总；不包含顶层 `segments[]`、root path、selection token 或 `workspaceHandle`。
- Memory、Segment 和 SegmentSupplement 是用户内容文件空间节点。用户可编辑语义写在 Markdown/frontmatter；Memory 的稳定 id 来自目录名前缀，`memory.md` 中可出现冗余 `id` 但不作为关系真源；Segment 和 SegmentSupplement 的稳定 id 写在对应 Markdown frontmatter `id`，`.reo/objects/<kind>/<id>.json` manifest 镜像 id、归属、kind、时长、音频字节数、正文 byte 长度、presentation state 和事务字段。
...
- Memory summary 由 `.reo/index.json` 的 `memories[]` 提供，包含 identity、title、timestamps、Segment 计数、audio 聚合、note presence、artifact Segment count、supplement count 和 cover projection。`hasAnyNote` 表示存在 note Segment 或 note SegmentSupplement。
...
- Segment projection 当前支持 `audio`、`note` 和 `artifact`，并包含 Segment cover projection。Audio Segment 持有 duration、audio byte length、transcript presence、lastTranscriptionAttempt、supplements、contentTabOrder 和可选 default cover template mirror；note Segment 持有 bodyByteLength、supplements、contentTabOrder 和可选 default cover template mirror；ready artifact Segment 持有 `format: html`、entry byte length、entry hash、runtime bundle-derived `previewVersion`、supplements、contentTabOrder 和可选 default cover template mirror；已由 manifest 识别过的 artifact 在 `entry.html` 缺失或过大时保持可见并投影 runtime fault，fault projection 不带 ready preview/hash 字段。
- Segment primary content tab 的用户可见名称来自 `segment.md` frontmatter `content_title`，缺失时由 renderer 按 Segment type 显示 `转录`、`正文` 或 `作品`。
- SegmentSupplement projection 当前支持 `audio`、`note` 和 `artifact`。Audio supplement 持有 duration、audio byte length、transcript presence 和 lastTranscriptionAttempt；note supplement 持有 bodyByteLength；ready artifact supplement 持有 `format: html`、entry byte length、entry hash 和 runtime bundle-derived `previewVersion`；已由 manifest 识别过的 artifact supplement 在 `entry.html` 缺失或过大时同样以 runtime fault 投影，且不带 ready preview/hash 字段。
- Workspace Widget 是记忆空间根目录 `widgets/<widgetDirectory>/` 下的 workspace-level side-rail 对象。`widget.md` frontmatter 必须包含 `id`、`title`、`kind: widget`、`format: html` 和 `mount: workspace-rail`；ready Widget projection 持有 entry byte length、entry hash、runtime bundle-derived `previewVersion`、可选 `assets/icon.svg` 图标和可选 runtime fault。Widget 只进入 Workspace snapshot，不进入 Memory detail、`.reo/objects/*` manifest、Memory activity 排序或 Segment content tab order。
- Home Component 是 app-level 首页组件区对象，位于 app `userData/home-components/<componentDirectory>/` 下，不属于任何记忆空间。`component.md` frontmatter 必须包含 `id`、`title`、`kind: home-component`、`format: html` 和 `mount: home`；ready Home Component projection 持有 entry byte length、entry hash、runtime bundle-derived `previewVersion`、可选 `assets/icon.svg` 图标和可选 runtime fault。Home Component 只进入 Home Component list，不进入 Workspace snapshot、Memory detail、`.reo/objects/*` manifest、Memory activity 排序、Segment content tab order 或 `.reo/workspace.json`。
...
- Memory cover 和 Segment cover 分别是 Memory 或 Segment 文件空间节点下的可选 `cover/` 子目录。`cover/` 中按文件名排序的首个合法普通图片文件作为自定义封面，当前只接受 PNG、JPEG 和 WebP；缺失、空目录、symlink、unsafe filename 或不支持格式都投影为默认封面。默认封面来自 renderer 内置 shared cover template pool；没有显式选择时按 MemoryId 或 SegmentId 稳定映射，不写入图片文件。用户切换随机默认封面时，Reo 只把内置模板 id 写入对应 Memory 或 Segment manifest 的 `defaultCoverTemplateId`。自定义 cover projection 只包含 `source`、`filename` 和基于文件 `mtimeNs+size` 的 cache version，不包含 raw path。
- 当前 finalized read model 只接受 `audio`、`note` 和 `artifact`；photo、video、imported_file 和其它 kind 不进入投影。
- Draft 属于 `.reo/drafts/segments/<segmentId>/` 或 `.reo/drafts/supplements/<supplementId>/`。Note draft 节点与 finalized note 节点一样同时保存 Markdown body 和同节点 `content.tiptap.json`；draft body write 失败必须回滚 metadata、Markdown 和 sidecar 到本次写入前状态，finalize 时迁移到目标 file-space node。Finalized Segment 属于 matching Memory 的 `segments/<segmentDirectory>/`；Finalized SegmentSupplement 属于 matching Segment 的 `supplements/<supplementDirectory>/`。
- Audio finalized object 由 Markdown、manifest、`audio.webm` 和 transcript Tiptap sidecar 表达；note finalized object 由 Markdown、Tiptap JSON sidecar 和 manifest 表达；artifact finalized object 由 Markdown、manifest 和同目录 runtime bundle 表达，bundle 当前包含 `entry.html`、`runtime.json`、`state.json` 和 `assets/`。Artifact manifest 只镜像 entry byte length/hash；Memory detail projection 的 `previewVersion` 由 entry、runtime JSON 和 `assets/` 直系文件指纹生成，用于外部修改启动文件或资源后刷新 iframe。Workspace Widget 使用同一 runtime bundle 文件名，但身份和顺序来自 `widget.md` 与 `.reo/workspace.json.widgetTabOrder`，不创建 `.reo/objects` manifest。Home Component 也使用同一 runtime bundle 文件名，身份来自 `component.md`，tab order 和 last active tab 来自 app `userData/home-components.json`，不创建 `.reo/objects` manifest。`state.json` 是 artifact/Widget/Home Component 长期可见状态真源，不参与 host preview URL/version，必须是 JSON object；`{ schemaVersion, stores }` 是 scaffold 推荐组织方式，不是投影或 runtime API 的硬性形状。Runtime 通过 `window.reo.state` 带 version/baseline 读写，外部 agent 也可直接查看、修改或替换该文件。Note Segment 与 note SegmentSupplement 可以包含同目录 `attachments/<filename>` 图片 payload，也可以包含 Reo 生成的同目录 `speech.mp3`。Audio 的 `content.tiptap.json` 只映射 `segment.md` / `supplement.md` 中 `## Transcript` 后的正文。
- Note Segment / note SegmentSupplement body 和 audio Segment / audio SegmentSupplement transcript 的 `content.tiptap.json` 是同节点可编辑正文的富结构载体，保存 `schemaVersion`、`objectType: 'tiptap-content'`、对应 Markdown body hash、profile、canonical Tiptap JSON hash 和有界 Tiptap JSON `content`。Markdown 缺 sidecar 时由 `@tiptap/markdown` 生成；Markdown 未变且 sidecar JSON 改变时由 sidecar serialize 回 Markdown；Markdown 改变且 sidecar 未改变时重新生成 sidecar；双方同时发生不可自动合并的变化或 sidecar 含无法无损序列化的 Tiptap 内容时进入 review/错误，不覆盖任一方。Draft note body 和 finalized note body 使用同一 sidecar profile 与 reconcile 规则。当前 durable Tiptap profile 覆盖 heading、paragraph text-align、blockquote、bullet/ordered/task list、code block、horizontal rule、image、bold、italic、strike、inline code、underline、link、highlight、colored highlight、superscript 和 subscript。Tiptap JSON attrs 只接受 JSON 可表达值，不接受 `NaN` / `Infinity`；colored highlight 只接受 Reo toolbar 暴露的 `var(--tt-color-highlight-gray|brown|orange|yellow|green|blue|purple|pink|red)` 变量，任意外部 CSS color 不进入 durable JSON/Markdown 合同；heading level、text-align、ordered-list start、task checked、code language、image attrs 和 link href/title 只接受当前 durable profile 可无损 roundtrip 的官方形状，unsupported attr 值进入 review/错误而不是 silent coercion。
- Reo 打开、文件真源刷新和 index rebuild 时从当前文件真源重建 Memory read model 并持久化 `.reo/index.json`；显式 `readWorkspaceSnapshotFromIndex` 只用于读取缓存快照，不代表打开空间可以信任旧 index。重建时只对 `memories/*/segments/*/segment.md`、`memories/*/segments/*/supplements/*/supplement.md` 和 `widgets/*/widget.md` 做浅层有界候选识别。Home Component list 由 app-level `userData/home-components/*/component.md` 浅层有界扫描独立生成。缺少 frontmatter 的 note Segment 或 note SegmentSupplement 可以在身份唯一且路径安全时被补全 `id`、`title`、`kind: note` 和 manifest mirror；新的 `kind: artifact` 候选必须声明 `format: html` 并存在可读、有界的同目录 `entry.html` 才能建立 manifest 并进入 ready 投影，缺入口、入口过大或 unsupported format 进入 needs-review。新的 `kind: widget` 候选必须声明 `format: html` 和 `mount: workspace-rail`，并存在可读、有界的同目录 `entry.html` 才能进入 Workspace snapshot；新的 `kind: home-component` 候选必须声明 `format: html` 和 `mount: home`，并存在可读、有界的同目录 `entry.html` 才能进入 Home Component list；缺入口、入口过大、unsupported format 或 unsupported mount 进入对应 fault 投影。已存在 manifest 的 artifact 入口后来缺失或过大时，在 Memory detail 中保留 fault 投影而不让用户失去修复入口；已识别 Widget 的入口后来缺失或过大时，在 Workspace snapshot 中保留 fault 投影；已识别 Home Component 的入口后来缺失或过大时，在 Home Component list 中保留 fault 投影。`runtime.json`、`state.json` 和 `assets/` 是 agent 与 runtime 使用的同目录 bundle 文件；缺失这些文件不阻止已有 `entry.html` 作品、Widget 或 Home Component 被投影，但托管 skill 的验证脚本会提示补齐。`runtime.json` 可声明 bridge needs；Reo 不为 artifact/Widget/Home Component runtime 提供隐藏 key、token 或 value 存储。
- 合法外部移动是同层级整目录移动：Memory 移动到任一 Reo memory space 的 `memories/` 下，Segment 移动到任一 Memory 的 `segments/` 下，SegmentSupplement 移动到任一 Segment 的 `supplements/` 下。目标 memory space 打开、刷新、read detail 或 index rebuild 时以当前文件树位置为归属真源，修复本 root 的 `.reo/objects/memories/*`、`.reo/objects/segments/*`、`.reo/objects/supplements/*` 和 `.reo/index.json`；源 root `.reo` 不做跨空间清理。note/audio/artifact Segment 目录移动到另一个 Memory 下时修复 Segment manifest 的 `workspaceId` 和 `memoryId`；note/audio/artifact SegmentSupplement 目录移动到另一个 Segment 下时修复 Supplement manifest 的 `workspaceId`、`memoryId` 和 `segmentId`。Audio 移动修复要求同目录存在 `audio.webm`；目标 root 已有 audio manifest 时 byte length 必须匹配，目标 root 缺 audio manifest 时用当前 `audio.webm` byte length 建立目标 manifest，`durationMs` 初值为 0、`nextSequence` 初值为 1。artifact 移动修复要求同目录 `entry.html` 与 manifest entry byte length/hash 匹配，缺目标 manifest 时由当前 `entry.html` 建立。保存事务后的 index refresh 不执行跨父级移动修复，ownership mismatch 仍返回 typed error。duplicate id、混合对象形态、unsafe path 或无法唯一归属的候选不进入 Memory detail、Workspace snapshot 或 `.reo/index.json`。
- Finalized Segment manifest 的 `contentTabOrder` 是 Reo 管理的可选 durable presentation 字段，元素为 `segment` 或 `supplement:<supplementId>`；`defaultCoverTemplateId` 是 Reo 管理的可选默认封面模板字段。读取投影时按当前合法 tab 归一化，并只在没有自定义 `cover/` 时展示默认封面模板。
...
- Home Component shell state 属于 main-owned app state，文件位于 `userData/home-components.json`，只保存 custom component tab order 和 last active tab。Custom component directory 删除时移入 app-level `userData/home-components-trash/` 并通过 restore token 恢复；rename 只更新 `component.md` frontmatter title，不改目录名。
...
- Memory space stable files 包括 Reo 托管入口 `.reo/REO.md`、Reo 新建空记忆空间时可创建的一次性 root `AGENTS.md` 入口、Reo 托管的官方同名 `skills/reo-edit/`、`skills/reo-cover-image/`、`skills/reo-cover-aesthetic/`、`skills/reo-works/`、`skills/reo-generative-runtime/`、`skills/reo-works-design/` 和 `skills/reo-doctor/`、`.reo` metadata/index/object manifests、Memory/Segment/Supplement/Widget Markdown、Memory/Segment cover payload、audio payload、note attachment payload、artifact/Widget runtime bundle、draft、trash、lock 和 recovery marker。打开和修复时 Reo 更新 `.reo/REO.md` 与官方同名 Reo skills，不更新已有用户 root `AGENTS.md`，也不修改用户自带的其它 skill。具体路径合同以 workspace contract 与文件读写代码为准；本文档只记录所有权与真源边界。
- `memory.md`、`segment.md` 和 `supplement.md` 是用户和 agent 可编辑的语义真源；`.reo/workspace.json`、`.reo/objects/*/*.json`、draft、trash、lock 和 recovery marker 是 Reo 管理的技术完整性层。`.reo/objects/*/*.json` mirror 如果没有对应 active 语义文件且也不在 trash 中，属于 doctor 可报告的 orphan mirror 诊断，不提升为用户语义真源。
- 用户内容目录中的普通 `.json`、`.md`、`.html` 或其它文件可以存在，但不自动成为 Reo 对象，也不作为 Reo schema 输入。只有被 `kind: artifact` + `format: html` Markdown 合同识别的同目录 `entry.html` 才作为作品 ready 入口，只有被 `kind: widget` + `format: html` + `mount: workspace-rail` Markdown 合同识别的 `widgets/*/entry.html` 才作为 Workspace Widget ready 入口；二者都只通过隔离 `reo-render://` iframe 预览。其它 `.html` 仍是不可信普通用户文件。候选对象进入 needs-review 时，main-owned diagnostics 只记录类别和计数，不记录 root path、file path、title、正文、frontmatter 原文或 id 列表。
- `.reo/review/needs-review.json` 是 main-owned 本地恢复报告，不是用户语义内容真源。报告 entry 只包含 review category、reason、object type、kind 和 workspace-relative POSIX paths；不得包含 root path、absolute path、title、正文、transcript、frontmatter 原文、source hash 或 content hash。`.reo/review/needs-review.md` 和 `reo-doctor` 输出可以按 reason 展示保守恢复提示，但不改变 JSON entry 形状。Workspace snapshot 只暴露 `review` aggregate counts，不暴露 report entries 或 paths。
...
- TanStack Query 拥有 main-backed memory space list、system Draft projection、recent expression feed、Home Component list、Workspace snapshot、Memory detail、selected Segment content、selected SegmentSupplement content、playback audio bytes、note speech audio bytes 和 voice settings snapshot。Memory detail、note Segment content、note SegmentSupplement content 和 Home Component list 是文件真源投影，fresh cache 在重新打开或重新选择同一未失效目标时可直接复用；file truth event、Home Component event、保存 mutation 或 exact invalidation 会让对应 cache 失效并在重新挂载或 active refetch 时读取 main process 文件真源。Audio Segment / audio SegmentSupplement content cache 只持有 transcript projection、audioByteLength、可选 audioHash 和 baseline hash，不持有本地录音 audio bytes；note content cache 只持有 speech projection、Markdown/Tiptap 正文和 baseline hash，不持有 generated speech MP3 bytes；artifact preview URL 由 Memory detail projection 派生，不新增 selected content Query。Home Component preview URL 由 Home Component list projection 派生，不新增 selected content Query。Recent expression feed 是应用级只读投影，由 main 枚举 system Draft 和普通 registry memory spaces 后按 request content kind 轻量扫描 finalized Segment 与 SegmentSupplement 的 identity、标题、kind、source workspace/Memory、timestamp、所属 Segment cover 和可选 playback descriptor，排序和 limit 后只为返回项读取可选正文摘要；playback descriptor 只为 finalized audio 或 ready note speech 暴露可播放能力，不携带音频 bytes；不可读空间只返回 redacted skipped summary，不返回 root path 或 file path。本地录音 playback audio bytes 和 note speech audio bytes 都由独立短生命周期 query 与 Loaded Workspace Frame 有界 Blob URL LRU 持有。baseline 只用于下一次保存的外部修改检测。
