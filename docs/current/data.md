...
- Memory summary 由 `.reo/index.json` 的 `memories[]` 提供，字段包含 `memoryId`、title、createdAt、updatedAt、assetCount、durationMs、audioByteLength、hasTranscript 和 hasReflections。
- Memory detail 由 `memory.json` 和当前实现的 Asset 文件投影组成。`memory.json.assetIds` 是 Memory 的核心关系，当前只接受已实现的 recording asset id；note、photo、video 和 upload 进入 runtime 前必须扩展 Asset 文件合同、schema、IPC 和恢复路径。
...
- `.reo/index.json` 当前保存 `memories[]` 投影；每个 summary 包含 `memoryId`、title、created/updated time、assetCount、duration、audio bytes、非空 transcript/reflections presence。
...
- Memory create 使用 `workspace:createMemory`，request 只包含 `workspaceHandle` 和 trimmed title；response 返回新建的 Memory summary，不返回 root path、recording list 或 Asset 字段。Main process 生成稳定 `memoryId`，在 `memories/<memoryId>/` 下创建空 `memory.json`，`assetIds` 初始为空，并刷新该 Memory 的 `.reo/index.json` summary entry。Title 只写入 `memory.json`，不参与 directory name；rename 不移动 Memory folder。Renderer 只在成功 response 后合并当前 session snapshot、seed snapshot Query cache，并为该 Memory seed 空 detail cache；该 flow 不做 optimistic update。
- Memory rename 使用 `workspace:updateMemoryTitle`，request 只包含 `workspaceHandle`、`memoryId` 和 trimmed title；response 返回更新后的 Memory summary，不返回 recording 详情、root path 或 Asset 字段。Main process 以 `memories/<memoryId>/memory.json` 为真源更新 title 和 updatedAt，并刷新该 Memory 的 `.reo/index.json` summary entry；index refresh 失败时仍以 file truth 返回成功 summary，后续 open/rebuild 会协调 index。Renderer 只在成功 response 后合并当前 session snapshot 和 snapshot Query cache，更新已有 memory detail cache 的 title/updatedAt/summary flags，并 invalidate 同一个 memory detail query；该 flow 不做 optimistic update。
- Memory detail response 读取 `memory.json` 作为 detail identity 真源，使用 `.reo/index.json` 中同一 memory summary 的 `assetCount`、`hasTranscript` 和 `hasReflections` 投影，并只返回前 24 条 recording summary 作为当前首屏 preview；`recordingsTruncated` 标明还有未展示的 recordings，避免 detail navigation 对长 memory 做无界 recording 文件读取或 DOM 渲染。
...
- 新建 Memory 容器不得复用已存在的 `memories/<memoryId>/`；重复 `memoryId` 或重复 durable `recordingId` 必须失败并保留既有文件，不得覆盖或删除用户内容。
...
- Workspace file、`.reo` metadata、rebuildable index、TanStack Query cache、component state 和 form state 不能存储同一事实的不同版本；Asset 关系只在 memory detail 和 `memory.json.assetIds` 中出现，Workspace snapshot 只保存 Memory summary。
