...
- `.reo/review/needs-review.json` 是 main-owned 本地恢复报告，不是用户语义内容真源。报告 entry 只包含 review category、reason、object type、kind 和 workspace-relative POSIX paths；不得包含 root path、absolute path、title、正文、transcript、frontmatter 原文、source hash 或 content hash。Workspace snapshot 只暴露 `review` aggregate counts，不暴露 report entries 或 paths。
