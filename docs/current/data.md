...
- Memory cover 是 Memory 文件空间节点下的可选 `cover/` 子目录。`cover/` 中按文件名排序的首个合法普通图片文件作为自定义封面，当前只接受 PNG、JPEG 和 WebP；缺失、空目录、symlink、unsafe filename 或不支持格式都投影为默认封面。默认封面来自 renderer 内置 cover template，不写入记忆空间文件。自定义 cover projection 只包含 `source`、`filename` 和基于文件 `mtimeNs+size` 的 cache version，不包含 raw path。
...
- Memory space stable files 包括入口 `AGENTS.md`、Reo 托管的 `skills/reo-edit/`、`skills/reo-memory-cover/`、`skills/reo-cover-aesthetic/` 和 `skills/reo-doctor/`、`.reo` metadata/index/object manifests、Memory/Segment/Supplement Markdown、Memory cover payload、audio payload、note attachment payload、draft、trash、lock 和 recovery marker。具体路径合同以 workspace contract 与文件读写代码为准；本文档只记录所有权与真源边界。
