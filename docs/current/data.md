...
- Reo 打开、刷新和 index rebuild 时只对 `memories/*/segments/*/segment.md` 与 `memories/*/segments/*/supplements/*/supplement.md` 做浅层有界候选识别。缺少 frontmatter 的 note Segment 或 note SegmentSupplement 可以在身份唯一且路径安全时被补全 `id`、`title`、`kind: note` 和 manifest mirror；新的 `kind: artifact` 候选必须声明 `format: html` 并存在可读、有界的同目录 `entry.html` 才能建立 manifest 并进入 ready 投影，缺入口、入口过大或 unsupported format 进入 needs-review。已存在 manifest 的 artifact 入口后来缺失或过大时，在 Memory detail 中保留 fault 投影而不让用户失去修复入口。`runtime.json`、`state.json` 和 `assets/` 是 agent 与 runtime 使用的同目录 bundle 文件；缺失这些文件不阻止已有 `entry.html` 作品被投影，但托管 skill 的验证脚本会提示补齐。`runtime.json` 可声明 secret slots 和 bridge needs；secret 明文值不属于记忆空间 bundle，而是 main-owned app state 中的解密运行时值，静态落盘只保存 safeStorage 密文。已存在的 note/audio/artifact Segment 目录移动到另一个 Memory 下时修复 Segment manifest 的 `memoryId`，已存在的 note/audio/artifact SegmentSupplement 目录移动到另一个 Segment 下时修复 Supplement manifest 的 `memoryId` 和 `segmentId`。Audio 移动修复要求同目录 `audio.webm` 与 manifest byte length 匹配；artifact 移动修复要求同目录 `entry.html` 与 manifest entry byte length 匹配。保存事务后的 index refresh 不执行跨父级移动修复，ownership mismatch 仍返回 typed error。duplicate id、混合对象形态、unsafe path 或无法唯一归属的候选不进入 Memory detail、Workspace snapshot 或 `.reo/index.json`。
...
- Artifact runtime secret values 属于 main-owned app state，文件位于 `userData/artifact-runtime-secrets.json`，按 runtime object identity 和 slot id 绑定，并使用 Electron `safeStorage` 保存密文。Renderer/iframe 只能通过当前 artifact preview 的 `window.reo.secrets` 读取或更新当前 `runtime.json` 已声明的 slot 值；bundle、Memory detail projection、workspace snapshot、日志和错误信封都不包含 secret 明文。
...
- Memory space stable files 包括入口 `AGENTS.md`、Reo 托管的 `skills/reo-edit/`、`skills/reo-cover-image/`、`skills/reo-cover-aesthetic/`、`skills/reo-works/`、`skills/reo-generative-runtime/`、`skills/reo-works-design/` 和 `skills/reo-doctor/`、`.reo` metadata/index/object manifests、Memory/Segment/Supplement Markdown、Memory/Segment cover payload、audio payload、note attachment payload、artifact runtime bundle、draft、trash、lock 和 recovery marker。具体路径合同以 workspace contract 与文件读写代码为准；本文档只记录所有权与真源边界。
...
- 用户内容目录中的普通 `.json`、`.md`、`.html` 或其它文件可以存在，但不自动成为 Reo 对象，也不作为 Reo schema 输入。只有被 `kind: artifact` + `format: html` Markdown 合同识别的同目录 `entry.html` 才作为作品 ready 入口，并且只通过隔离 `reo-artifact://` iframe 预览；其它 `.html` 仍是不可信普通用户文件。候选对象进入 needs-review 时，main-owned diagnostics 只记录类别和计数，不记录 root path、file path、title、正文、frontmatter 原文或 id 列表。
