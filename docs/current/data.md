...
- Audio finalized object 由 Markdown、manifest、`audio.webm` 和 transcript Tiptap sidecar 表达；note finalized object 由 Markdown、Tiptap JSON sidecar 和 manifest 表达；artifact finalized object 由 Markdown、manifest 和同目录 `segment.html` 或 `supplement.html` 入口表达。Note Segment 与 note SegmentSupplement 可以包含同目录 `attachments/<filename>` 图片 payload，也可以包含 Reo 生成的同目录 `speech.mp3`。Audio 的 `content.tiptap.json` 只映射 `segment.md` / `supplement.md` 中 `## Transcript` 后的正文。
...
- Memory space stable files 包括入口 `AGENTS.md`、Reo 托管的 `skills/reo-edit/`、`skills/reo-cover-image/`、`skills/reo-cover-aesthetic/`、`skills/reo-works/`、`skills/reo-works-design/` 和 `skills/reo-doctor/`、`.reo` metadata/index/object manifests、Memory/Segment/Supplement Markdown、Memory/Segment cover payload、audio payload、note attachment payload、artifact HTML payload、draft、trash、lock 和 recovery marker。具体路径合同以 workspace contract 与文件读写代码为准；本文档只记录所有权与真源边界。
...
- 用户内容目录中的普通 `.json`、`.md`、`.html` 或其它文件可以存在，但不自动成为 Reo 对象，也不作为 Reo schema 输入。只有被 `kind: artifact` + `format: html` Markdown 合同识别的同目录 `segment.html` 或 `supplement.html` 才作为作品入口，并且只通过隔离 `reo-artifact://` iframe 预览；其它 `.html` 仍是不可信普通用户文件。候选对象进入 needs-review 时，main-owned diagnostics 只记录类别和计数，不记录 root path、file path、title、正文、frontmatter 原文或 id 列表。
