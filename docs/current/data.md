...
- Segment projection 当前支持 `audio`、`note` 和 `artifact`，并包含 Segment cover projection。Audio Segment 持有 duration、audio byte length、transcript presence、lastTranscriptionAttempt、supplements、contentTabOrder 和可选 default cover template mirror；note Segment 持有 bodyByteLength、supplements、contentTabOrder 和可选 default cover template mirror；artifact Segment 持有 `format: html`、entry byte length、entry hash、runtime bundle-derived `previewVersion`、supplements、contentTabOrder 和可选 default cover template mirror。
- Segment primary content tab 的用户可见名称来自 `segment.md` frontmatter `content_title`，缺失时由 renderer 按 Segment type 显示 `转录`、`正文` 或 `作品`。
- SegmentSupplement projection 当前支持 `audio`、`note` 和 `artifact`。Audio supplement 持有 duration、audio byte length、transcript presence 和 lastTranscriptionAttempt；note supplement 持有 bodyByteLength；artifact supplement 持有 `format: html`、entry byte length、entry hash 和 runtime bundle-derived `previewVersion`。
...
- 用户内容目录中的普通 `.json`、`.md`、`.html` 或其它文件可以存在，但不自动成为 Reo 对象，也不作为 Reo schema 输入。只有被 `kind: artifact` + `format: html` Markdown 合同识别的同目录 `entry.html` 才作为作品入口，并且只通过隔离 `reo-artifact://` iframe 预览；其它 `.html` 仍是不可信普通用户文件。候选对象进入 needs-review 时，main-owned diagnostics 只记录类别和计数，不记录 root path、file path、title、正文、frontmatter 原文或 id 列表。
