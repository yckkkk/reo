...
- 当前没有 Zustand stores。
- 当前 React Hook Form form owner 覆盖 create memory space submit 前的 title、description、selection token、displayPath draft、Memory create title draft、Memory rename title draft 和 memory space rename title draft。
...
- React Hook Form 当前只服务真实 create memory space form submit、folder selection draft、Memory create title draft、Memory rename title draft、memory space rename title draft 和 validation owner。
...
- Memory space registry 不是记忆空间内容真源；记忆空间 title、description、memory summary 和 segment truth 仍来自记忆空间文件。Registry 文件缺失、损坏、schema 不匹配或 symlink leaf 按空记忆空间列表处理；不可读 IO 错误返回 typed error envelope。Registry upsert 在单 registry 实例内串行化，避免并发导入覆盖既有记忆空间。读取或解析 registry 时，main 会先用当前 root 的 `.reo/workspace.json` 返回 title/description 投影；若 registry root 已不存在，则只在原父目录下最多检查 200 个直接子目录，查找同一 `workspaceId` 的 Reo 记忆空间 folder，找到后返回 canonical root 投影，并把列表 title 投影为新 folder basename。Registry 读取和解析不写入记忆空间文件；Finder 改名后的 folder basename 只有在 open 获取 single-writer lock、且在任何 recovery/index 写入前只读确认打开的 `workspaceId` 匹配 registry entry 后，才写回 `.reo/workspace.json` 并 upsert registry projection。
- Memory space rename 使用 `workspace:updateMemorySpaceTitle`。Active 记忆空间 request 只包含 `workspaceHandle` 和 trimmed title；inactive registry entry request 只包含 `workspaceId` 和 trimmed title。Main process 以 `.reo/workspace.json` 为真源更新 title；active path 以文件真源写入成功作为保存成功，registry projection 写入失败不阻断当前会话 response；inactive path 需要先通过 registry 解析 root，并在文件真源更新后更新 registry title/description/root projection，但保留该 entry 的 addedAt、lastOpenedAt 和列表顺序。Response 返回更新后的 Workspace snapshot，不返回 root path 或 handle。Renderer 只在成功 response 后更新当前 session snapshot、seed snapshot Query cache，并同步或 invalidate `['workspace', 'memory-spaces']`；该 flow 不做 optimistic update。
...
- React Hook Form 只拥有 create memory space submit 前的 form draft，包括 title、description、selection token 和 displayPath；也拥有 Memory create Dialog、Memory rename Dialog 和 memory space rename Dialog 提交前的 title draft。
...
- Recording finalize 后 renderer seed 当前记忆空间 snapshot cache 的 `memories[]` 投影，把受影响 Memory 设为当前 Memory context，并用 finalize response 中的完整 finalized audio Segment projection 精准 upsert 对应 `['workspace', 'memory-detail', workspaceId, memoryId]` cache。该更新不依赖 workspaceHandle 或 raw path，也不把 Segment 写入其他 Memory detail cache。
