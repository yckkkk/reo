...
- Memory summary 由 `.reo/index.json` 的 `memories[]` 提供，包含 identity、title、timestamps、Segment 计数、audio 聚合、note presence、supplement count 和 cover projection。`hasAnyNote` 表示存在 note Segment 或 note SegmentSupplement。
...
- Segment projection 当前支持 `audio` 和 `note`，并包含 Segment cover projection。Audio Segment 持有 duration、audio byte length、transcript presence、lastTranscriptionAttempt、supplements、contentTabOrder 和可选 default cover template mirror；note Segment 持有 bodyByteLength、supplements、contentTabOrder 和可选 default cover template mirror。
...
- Audio finalized object 由 Markdown、manifest、`audio.webm` 和 transcript Tiptap sidecar 表达；note finalized object 由 Markdown、Tiptap JSON sidecar 和 manifest 表达。Note Segment 与 note SegmentSupplement 可以包含同目录 `attachments/<filename>` 图片 payload，也可以包含 Reo 生成的同目录 `speech.mp3`。Audio 的 `content.tiptap.json` 只映射 `segment.md` / `supplement.md` 中 `## Transcript` 后的正文。
...
- Memory space stable files 包括入口 `AGENTS.md`、Reo 托管的 `skills/reo-edit/`、`skills/reo-cover-image/`、`skills/reo-cover-aesthetic/` 和 `skills/reo-doctor/`、`.reo` metadata/index/object manifests、Memory/Segment/Supplement Markdown、Memory/Segment cover payload、audio payload、note attachment payload、draft、trash、lock 和 recovery marker。具体路径合同以 workspace contract 与文件读写代码为准；本文档只记录所有权与真源边界。
