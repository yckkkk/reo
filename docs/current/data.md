...
- TanStack Query 拥有 main-backed memory space list、Workspace snapshot、Memory detail、selected Segment content、selected SegmentSupplement content 和 voice settings snapshot。Segment/audio content cache 可持有本地音频 bytes 和 baseline hash；baseline 只用于下一次保存的外部修改检测。
...
- Note body 和 transcript save 使用 baseline hash 防止外部修改覆盖。Baseline 不匹配不覆盖 Markdown、不刷新 index、不推进 manifest。
