...
Audio Segment card 使用现代扁平矢量 + 毛玻璃录音卡：紧凑正方形比例、Card Glass surface、2px token 边框、`--radius-panels`、标题直入、状态文本、动态 waveform bars 和 Geist Mono duration。卡片宽度由 Memory Studio feature-local geometry 限定在 `clamp(124px, 20%, 144px)`，内部字号、padding、waveform 和时间尺寸随紧凑卡片重排，避免通过单纯缩放造成文字或时间溢出。active waveform 使用 15 根纯色 bars、中心伸缩、错相节奏和分散 duration，避免整排 bars 同步成同一种上下律动。
