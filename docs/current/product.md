...
selected Segment 存在 finalized SegmentAttachment 时，内容区显示 `补充` tab；同一 selected Segment 从无补充变为有补充时，Memory Studio 自动切到 `补充` tab 让新内容可见。补充录音在该 tab 内使用与主播放条一致的播放按钮、真实 waveform slider 和等宽时间，但不展示 transcript。
