...
记忆空间 root 使用 `AGENTS.md` 作为 agent 定义与导航入口；普通编辑、创建、重命名、移动和整理范式位于 `skills/reo-edit/`，Reo 文件空间异常诊断位于 `skills/reo-doctor/`。Reo metadata 位于 `.reo/workspace.json`，可重建 index 位于 `.reo/index.json`，对象技术完整性 manifest 位于 `.reo/objects/<kind>/<id>.json`，needs-review report 位于 `.reo/review/needs-review.json` 和 `.reo/review/needs-review.md`，single-writer lock 位于 `.reo/workspace.lock*`。用户语义内容使用普通 Markdown 文件保存：Memory 使用 `memory.md`，Segment 使用 `segment.md`，SegmentSupplement 使用 `supplement.md`；audio 对象额外保存 `audio.webm`，note 正文和 audio transcript 都可使用同节点 `content.tiptap.json` 富结构载体，note 对象还可使用同节点 `attachments/`。Audio sidecar 只映射 `## Transcript` 正文，不映射音频元数据或整个 Markdown 文件。Draft 位于 `.reo/drafts/*`，trash 位于 `.reo/trash/*`。目录 basename 是用户可见名称真源；Memory id 来自 Memory 目录前缀，`memory.md` 内多余的 `id` 只作为可容忍冗余字段；Segment 和 SegmentSupplement 的 Markdown frontmatter `id` 是身份载体，`.reo/objects/*/*.json` manifest 镜像身份、归属和技术完整性字段。合法外部编辑通过 index cache、active snapshot refresh 和 focused detail read 重新投影 UI；合法外部跨父级移动必须移动整个 Segment 或 SegmentSupplement 目录，由读模型修复 manifest parent mirror。Agent 可以编辑任意文件；Reo 的默认协作合同是让一般任务直接修改语义 Markdown 和目录，sidecar、hash、manifest mirror 与缺失配置由 Reo 做确定性收敛，无法确定的冲突写入 needs-review report。普通 `.json`、`.html` 或未被对象合同识别的文件不自动成为 Reo 对象。字段级文件合同见 `data.md` 和 `flow.md`。
