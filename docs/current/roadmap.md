...
- audio 主链是多模态实现路径里复杂度最高的；note 主链证明同一 Memory / Segment / SegmentSupplement 文件空间可以承载非音频表达；artifact first slice 证明外部 agent 可以通过 prompt-bridge 写入作品并由 Reo 安全预览。基础主链稳固后继续推进产品本质长期轨道，把剩余多模态 Segment 类型、widget runtime、Gallery 走马灯、prompt-bridge UI、AGENTS.md / skills 全部到位。
...
- **作品 Segment / SegmentSupplement 深化**：agent 生成的 HTML 报告 / 作品页 / 复盘文档以 `artifact` 产品类型和 `format: html` 文件格式落到现有文件合同；首个静态隔离预览能力之后，继续补齐全屏、更多 agent 操作入口和未来 Widget 数据桥边界。
...
下列方向在产品本质长期轨道之外，进入主线前需要独立判断和 spec。注意：剩余多模态 Segment 类型（video / photo / imported_file）与 artifact 作品深化已提升到产品本质长期轨道，**不在本节**。
