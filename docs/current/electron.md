...
- `workspace:requestSegmentTranscriptionBackfill` 只接受 `workspaceHandle`、`workspaceId`、`memoryId` 和 `segmentId`；`workspace:requestSegmentSupplementTranscriptionBackfill` 只接受 `workspaceHandle`、`workspaceId`、parent `memoryId`、parent `segmentId` 和 `supplementId`。两个 channel 都校验 trusted sender、handle ownership、workspaceId 与 handle 匹配和 lock usability。Main process 从 finalized `audio.webm` 读取有界音频 bytes，在 main 内部用随包 ffmpeg remux 为 OGG/Opus，再用豆包极速版 `audio.data` 调用补转录，成功后复用现有 transcript save 路径写入 Markdown 和 manifest `lastTranscriptionAttempt: 'success'`。Response 返回现有 save response shape，不返回 raw path、audio bytes、base64、API key、火山请求体或 transcript 外的诊断字段；同 target 已在队列或运行中时返回 `ERR_BACKFILL_ALREADY_RUNNING`。
