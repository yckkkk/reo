...
- `workspace:saveTranscript` 只接受 finalized audio segment identity：`workspaceHandle`、`memoryId` 和 `segmentId`。成功 response 返回当前 Memory summary，把 transcript 写入 `segment.md` 正文，并在复核 workspace、Memory 和 Segment ownership 后把 Segment manifest 的 `lastTranscriptionAttempt` 写为 `'success'`。`workspace:saveSegmentSupplementTranscript` 只接受 `workspaceHandle`、`workspaceId`、parent `memoryId`、parent `segmentId`、`supplementId` 和 markdown；main 通过 parent Segment 下 matching SegmentSupplement 文件空间节点写入 `supplement.md` 正文，刷新父 Memory summary，在复核 workspace、Memory、parent Segment 和 SegmentSupplement ownership 后把 SegmentSupplement manifest 的 `lastTranscriptionAttempt` 写为 `'success'`，并返回父 Memory summary、parent Segment projection 和 supplement projection。两个保存 channel 都不返回 raw path、file path、selection token 或 handle；保存失败不改变既有 manifest attempt。Segment draft create/append/finalize/discard 使用 draft `segmentId` transaction identity。
- `workspace:requestSegmentTranscriptionBackfill` 只接受 `workspaceHandle`、`workspaceId`、`memoryId`、`segmentId` 和 `mode: 'fill-missing' | 'regenerate'`；`workspace:requestSegmentSupplementTranscriptionBackfill` 只接受 `workspaceHandle`、`workspaceId`、parent `memoryId`、parent `segmentId`、`supplementId` 和同一 `mode`。两个 channel 都校验 trusted sender、handle ownership、workspaceId 与 handle 匹配和 lock usability。Main process 从 finalized `audio.webm` 读取有界音频 bytes，在 main 内部用随包 ffmpeg remux 为 OGG/Opus，再用豆包极速版 `audio.data` 调用补转录；`fill-missing` 只允许 transcript 为空的目标，`regenerate` 只允许 manual request 覆盖既有 transcript。成功后复用现有 transcript save 路径写入 Markdown 和 manifest `lastTranscriptionAttempt: 'success'`。Regenerate 保存前发现 transcript snapshot 已变化时返回 `ERR_BACKFILL_TRANSCRIPT_CHANGED`，不覆盖 Markdown，也不推进 manifest。Response 返回现有 save response shape，不返回 raw path、audio bytes、base64、API key、火山请求体、transcript digest 或 transcript 外的诊断字段；同 target 已在队列或运行中时返回 `ERR_BACKFILL_ALREADY_RUNNING`。
