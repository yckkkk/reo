...
- `workspace:readFinalizedAudioSegmentSupplement` request 接受 `workspaceHandle`、`workspaceId`、`memoryId`、`segmentId`、`supplementId` 和 requestId；main 校验 sender、handle ownership、workspaceId 与当前 handle workspace 匹配和 lock usability 后，按 supplement metadata id 定位 parent Segment 下 matching finalized audio SegmentSupplement 文件空间节点，并只读取其中的 `audio.webm`。Response 返回同 requestId、identity、bounded audio bytes 和 audioByteLength，不返回 transcript text、`workspaceHandle`、root path、file path、selection token 或 raw path。
