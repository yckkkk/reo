...
- `workspace:saveVoiceTranscriptionApiKey` request 接受 `{ apiKey }`；main 先用 `safeStorage` 加密写入 `voiceSettingsStore`，再立即执行 voice transcription probe，response 携带最新 snapshot 与可选 `validationError`。
