...
- `workspace:readFinalizedAudioSegmentAttachment` request 接受 `workspaceHandle`、`workspaceId`、`memoryId`、`segmentId`、`attachmentId` 和 requestId；main 校验 sender、handle ownership、workspaceId 与当前 handle workspace 匹配和 lock usability 后，只读取该 Memory 的 parent Segment 下 matching finalized audio SegmentAttachment 的 `audio.webm`。Response 返回同 requestId、identity、bounded audio bytes 和 audioByteLength，不返回 transcript text、`workspaceHandle`、root path、file path、selection token 或 raw path。
...
- `workspace:saveTranscript` 只接受 finalized audio segment identity：`workspaceHandle`、`memoryId` 和 `segmentId`。成功 response 返回当前 Memory summary。Segment draft create/append/finalize/discard 使用 draft `segmentId` transaction identity。
