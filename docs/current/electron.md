...
- 当前 `npm run dev` 通过 `scripts/run-dev.mjs` 先加载 git-ignored `.env.local`，再启动 `electron-vite dev`。`.env.local` 只作为本机 shell 环境注入给 Electron dev process；loader 不注入 `VITE_*` key。豆包 App ID / Access Token 使用未带 `VITE` scope prefix 的 `REO_DOUBAO_ASR_APP_ID` 和 `REO_DOUBAO_ASR_ACCESS_TOKEN`，只由 main process 读取，不进入 renderer、preload、IPC DTO、DOM、Query key、记忆空间文件或日志。
...
- `workspace:readFinalizedAudioSegmentSupplement` request 接受 `workspaceHandle`、`workspaceId`、`memoryId`、`segmentId`、`supplementId` 和 requestId；main 校验 sender、handle ownership、workspaceId 与当前 handle workspace 匹配和 lock usability 后，按 supplement manifest id 定位 parent Segment 下 matching finalized audio SegmentSupplement 文件空间节点，并只读取其中的 `audio.webm`。Response 返回同 requestId、identity、bounded audio bytes 和 audioByteLength，不返回 transcript text、`workspaceHandle`、root path、file path、selection token 或 raw path。
