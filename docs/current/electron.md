...
- 当前 `npm run dev` 通过 `scripts/run-dev.mjs` 先按 Node DotEnv 语法加载 git-ignored `.env.local`，再启动 `electron-vite dev`。Renderer development server 固定使用 `http://localhost:5183` 且启用 `strictPort`；端口被占用时启动失败，不自动漂移到未知端口。`.env.local` 只作为本机 shell 环境注入给 Electron dev process；loader 不注入 `VITE_*` key。
- 当前豆包语音能力凭证使用 Electron `safeStorage` 加密存放在 `userData/voice-transcription-settings.json`，由 main process `voiceSettingsStore` 持有。Renderer/preload 通过 application-scoped voice settings IPC 读取不含密文的 snapshot：`enabled`、`apiKeyConfigured`、`apiKeyLastFour`、`speechSynthesisSpeaker`、`lastTranscriptionValidatedAt`、`lastTranscriptionValidationOk`、`lastTranscriptionValidationCode`、`lastSpeechSynthesisValidatedAt`、`lastSpeechSynthesisValidationOk` 和 `lastSpeechSynthesisValidationCode`。同一 X-Api-Key 服务录音中流式识别、finalized audio 自动补转录、手动重新生成转录和 note speech synthesis；X-Api-Key 只在用户保存设置的 request 和 main process 解密后的运行时输入中出现；settings response、录音 IPC response、补转录 IPC response、speech synthesis IPC response、日志、错误信封和记忆空间内容文件不返回明文或密文。
...
- 当前生产 CSP 包含 `media-src 'self' blob:`，只用于本地 audio playback Blob URL；`img-src` 允许 `'self'`、`data:`、`blob:` 和 `reo-attachment:`，用于 Markdown reading surface、Tiptap editing surface 中 note 图片附件预览，以及 Memory/Segment cover 图片预览。生产 CSP 不把 `reo-attachment:` 加入 `connect-src`。
...
- Workspace snapshot 的 needs-review visibility 只能通过 aggregate counts 暴露给 renderer。`.reo/review/needs-review.json` 的具体 workspace-relative paths 只存在于本地文件系统和 main-owned `reo-doctor` 脚本输出，不通过 preload/IPC 作为 entries 或 raw paths 传给 renderer。Needs-review toast 的复制提示词使用显式 `workspace:copyNeedsReviewAgentPrompt` IPC；renderer 只传当前 `workspaceHandle`、`workspaceId` 和 count，main process 生成安全提示词并调用 `clipboard.writeText`，该 channel 不接受 prompt、report path、report entries 或 raw path。
...
- Memory space IPC channels 覆盖记忆空间选择/列表/初始化/打开/关闭、Workspace snapshot、Memory/Segment/SegmentSupplement 读写、Memory cover reset/restore/default switch、Segment cover reset/restore/default switch、note draft/finalize、audio recording draft/finalize、transcript save、manual transcription backfill、note speech synthesis、imported note speech batch regeneration、microphone intent、recording transcription、voice settings、受限外部链接打开和实体 More 菜单 shell 动作。具体 channel name、DTO 和错误信封以 `src/workspace-contract/workspace-channels.ts`、`workspace-contract.ts` 和 bridge contract 为准。
- `workspace:readVoiceTranscriptionSettings` request 不接受 payload；response 返回 `settings: { enabled, apiKeyConfigured, apiKeyLastFour, speechSynthesisSpeaker, lastTranscriptionValidatedAt, lastTranscriptionValidationOk, lastTranscriptionValidationCode, lastSpeechSynthesisValidatedAt, lastSpeechSynthesisValidationOk, lastSpeechSynthesisValidationCode }`，不返回 X-Api-Key 明文或密文。
- `workspace:setVoiceTranscriptionEnabled` request 接受 `{ enabled }`；response 与 read 同。
- `workspace:saveVoiceTranscriptionApiKey` request 接受 `{ apiKey }`；main 先用 `safeStorage` 加密写入 `voiceSettingsStore`，再立即执行 ASR probe 和当前 speech synthesis speaker 的短 TTS probe，response 携带最新 snapshot，不返回密钥、probe 原始错误、note 正文或额外 validation 字段。若 key 文件已写入但 validation snapshot 写入失败，错误信封使用 `dataRetention: "file-written-index-stale"`；renderer 重新读取 settings snapshot，不重发或显示已保存密钥明文。
...
- `workspace:validateVoiceTranscriptionCredentials` request 不接受 payload；main 解密当前 X-Api-Key 后执行 ASR probe 和短 TTS probe，response 返回 `{ code: 'ok' | 'auth' | 'network', message? }`，并同步更新 store 的 split validation 字段。
- `workspace:setVoiceSpeechSynthesisSpeaker` request 接受 `{ speaker }`；speaker 必须是当前 contract 中列出的固定豆包 TTS speaker id。Main 写入 speaker 后，如果语音开关已启用且 X-Api-Key 已配置，则用新 speaker 执行短 TTS probe 并更新 speech synthesis validation snapshot。该 channel 不会自动替换任何已有 note speech 文件。
...
- `workspace:readWorkspaceSnapshot` request 只接受 `workspaceHandle`；main process 校验 sender、handle ownership 和 lock usability 后，从当前 `.reo/workspace.json` 和记忆空间文件 read model 重新返回 Workspace snapshot。该 read model 会在合法 finalized note/audio Segment 与 SegmentSupplement 浅层扫描内，对已存在的 `content.tiptap.json` 执行 passive reconcile；安全 sidecar-authored 变化会先写回 Markdown mirror，再返回 snapshot。该 channel 用于 active session 同步外部合法文件修改，不返回 root path、selection token、handle internals、file path 或顶层 recording 投影。Active workspace handle 同时拥有 main-owned file truth watcher；watcher 不跟随 symlink，只发送 coalesced path-redacted 事件，错误诊断也不得包含 raw path。Preload 只暴露 `workspace:fileTruthChanged` 的 safe subscription，event payload 只包含 kind、reason、sequence、workspaceId 和 workspaceHandle，不暴露 raw path 或 Electron event object。
...
- `workspace:readFinalizedAudioSegment` request 接受 `workspaceHandle`、`workspaceId`、`memoryId`、`segmentId` 和 requestId；main 校验 sender、handle ownership、workspaceId 与当前 handle workspace 匹配和 lock usability 后，只读取该 Memory 下 matching finalized audio Segment 的 `segment.md` transcript section、同级 `content.tiptap.json` 和 audio metadata projection。Response 返回同 requestId、identity、audioByteLength、可选 audioHash、transcript text、transcript Tiptap JSON、transcript `baselineHash` 和 `baselineTiptapContentHash`，不返回 audio bytes、`workspaceHandle`、root path、file path、selection token 或 raw path。`workspace:readFinalizedAudioSegmentAudio` 单独接受同一 audio identity、expected audioByteLength、可选 expected audioHash 和 requestId；main 校验当前 `audio.webm` 后只返回 bounded audio bytes、MIME、audioByteLength 和 audioHash。
- `workspace:readFinalizedAudioSegmentSupplement` request 接受 `workspaceHandle`、`workspaceId`、`memoryId`、`segmentId`、`supplementId` 和 requestId；main 校验 sender、handle ownership、workspaceId 与当前 handle workspace 匹配和 lock usability 后，按 supplement manifest id 定位 parent Segment 下 matching finalized audio SegmentSupplement 文件空间节点，并只读取 `supplement.md` transcript section、同级 `content.tiptap.json` 和 audio metadata projection。Response 返回同 requestId、identity、audioByteLength、可选 audioHash、transcript text、transcript Tiptap JSON、transcript `baselineHash` 和 `baselineTiptapContentHash`，不返回 audio bytes、`workspaceHandle`、root path、file path、selection token 或 raw path。`workspace:readFinalizedAudioSegmentSupplementAudio` 单独接受同一 audio supplement identity、expected audioByteLength、可选 expected audioHash 和 requestId；main 校验当前 `audio.webm` 后只返回 bounded audio bytes、MIME、audioByteLength 和 audioHash。
- `workspace:createNoteSegmentDraft` request 接受 `workspaceHandle`、`workspaceId`、`memoryId` 和 title；main 校验 sender、handle ownership、workspaceId 与当前 handle workspace 匹配和 lock usability 后，在 `.reo/drafts/segments/<segmentId>/` 创建 note draft metadata、markdown body 和 `content.tiptap.json`，返回 `segmentId` 和 revision，不返回 raw path。`workspace:writeNoteSegmentDraftBody` 接受 `workspaceHandle`、`segmentId`、bodyMarkdown、可选 bodyTiptapJson 和 revision；main 按 revision 写入 draft metadata、markdown body 与 sidecar，sidecar 校验或写入失败时回滚本次 draft 写入并返回错误，成功后返回下一 revision。`workspace:finalizeNoteSegmentDraft` 接受 `workspaceHandle`、`workspaceId`、`memoryId`、`segmentId` 和 title，先 reconcile draft Markdown/sidecar，再把 draft finalize 到 matching Memory 的 note Segment 文件空间节点，返回当前 Memory summary 和 note Segment projection。
- `workspace:createSegmentSupplementNoteDraft` request 接受 `workspaceHandle`、`workspaceId`、parent `memoryId`、parent `segmentId` 和 title；main 校验 sender、handle ownership、workspaceId 与当前 handle workspace 匹配和 lock usability 后，在 `.reo/drafts/supplements/<supplementId>/` 创建 note supplement draft metadata、markdown body 和 `content.tiptap.json`，返回 `supplementId` 和 revision，不返回 raw path。`workspace:writeSegmentSupplementNoteDraftBody` 接受 `workspaceHandle`、`supplementId`、bodyMarkdown、可选 bodyTiptapJson 和 revision；main 按 revision 写入 draft metadata、markdown body 与 sidecar，sidecar 校验或写入失败时回滚本次 draft 写入并返回错误，成功后返回下一 revision。`workspace:finalizeSegmentSupplementNoteDraft` 接受 `workspaceHandle`、`workspaceId`、parent `memoryId`、parent `segmentId`、`supplementId` 和 title，先 reconcile draft Markdown/sidecar，再把 draft finalize 到 parent Segment 的 note SegmentSupplement 文件空间节点，返回当前 Memory summary、parent Segment projection 和 note supplement projection。
- `workspace:readSegmentContent` request 接受 `workspaceHandle`、`workspaceId`、`memoryId`、`segmentId` 和 requestId；main 读取 finalized note Segment 的 `segment.md` 正文、`content.tiptap.json`、bodyByteLength 和 speech synthesis projection，并在 response 前 reconcile Markdown 与 sidecar：sidecar 缺失时补齐，sidecar JSON 改变且 Markdown 未变时先 atomic 写回 `segment.md`、manifest byte length 和父 Memory index。Response 返回同 requestId、identity、`type: 'note'`、title、bodyMarkdown、bodyTiptapJson、bodyByteLength、`speechSynthesis`、`baselineContentHash` 和 `baselineTiptapContentHash`，不返回 speech MP3 bytes、root path、file path、selection token 或 handle。`workspace:writeSegmentContent` 接受同一 finalized note Segment identity、bodyMarkdown、可选 bodyTiptapJson、`baselineContentHash` 和 `baselineTiptapContentHash`；request 携带 bodyTiptapJson 时 baselineTiptapContentHash 必填；main 在当前 Segment directory identity 内先比较当前磁盘正文和 sidecar baseline，匹配后 atomic 写入 `segment.md` 正文与 sidecar、刷新父 Memory summary，并返回 bodyByteLength 与新的两组 baseline hash。Audio Segment transcript save 仍使用 `workspace:saveTranscript`，不走该 channel。
- `workspace:readSegmentSupplementContent` request 接受 `workspaceHandle`、`workspaceId`、parent `memoryId`、parent `segmentId`、`supplementId` 和 requestId；main 读取 finalized note SegmentSupplement 的 `supplement.md` 正文、`content.tiptap.json`、bodyByteLength 和 speech synthesis projection，并在 response 前 reconcile Markdown 与 sidecar：sidecar 缺失时补齐，sidecar JSON 改变且 Markdown 未变时先 atomic 写回 `supplement.md`、manifest byte length 和父 Memory index。Response 返回同 requestId、identity、`type: 'note'`、title、bodyMarkdown、bodyTiptapJson、bodyByteLength、`speechSynthesis`、`baselineContentHash` 和 `baselineTiptapContentHash`，不返回 speech MP3 bytes、root path、file path、selection token 或 handle。`workspace:writeSegmentSupplementContent` 接受同一 finalized note SegmentSupplement identity、bodyMarkdown、可选 bodyTiptapJson、`baselineContentHash` 和 `baselineTiptapContentHash`；request 携带 bodyTiptapJson 时 baselineTiptapContentHash 必填；main 在当前 SegmentSupplement directory identity 内先比较当前磁盘正文和 sidecar baseline，匹配后 atomic 写入 `supplement.md` 正文与 sidecar、刷新父 Memory summary，并返回 bodyByteLength 与新的两组 baseline hash。Audio SegmentSupplement transcript save 仍使用 `workspace:saveSegmentSupplementTranscript`，不走该 channel。
- `workspace:readSegmentSpeechAudio` request 接受当前 finalized note Segment identity、requestId 和 `speechSynthesis` manifest identity；`workspace:readSegmentSupplementSpeechAudio` request 接受当前 finalized note SegmentSupplement identity、requestId 和 `speechSynthesis` manifest identity。Main 只在 target 当前 projection 为 ready 或 stale、request manifest identity 与当前 manifest 匹配、`speech.mp3` byte length 和 audio hash 校验通过时返回 bounded MP3 bytes 与 MIME；response 不返回 note 正文、provider request text、root path、file path、selection token 或 handle。
...
- `workspace:resetMemoryCover` request 只接受 `workspaceHandle` 和 `memoryId`；main 通过当前 handle 授权把该 Memory 的自定义 `cover/` 目录移动到 `.reo/trash/memory-covers/<restoreToken>`，刷新 Memory summary，并只返回 Memory summary、Workspace memories projection 和 restore token。`workspace:restoreMemoryCover` request 只接受 `workspaceHandle`、`memoryId` 和 restore token；main 只把该 token 对应的 cover trash 目录恢复为 active `cover/` 并刷新投影。`workspace:switchMemoryDefaultCover` 只接受 `workspaceHandle`、`memoryId` 和内置 default cover template id；main 只写 Memory manifest 的 `defaultCoverTemplateId`，刷新 Memory summary 和 Workspace memories projection。`workspace:resetSegmentCover` / `workspace:restoreSegmentCover` 使用当前 workspace、parent Memory 和 Segment identity，把 Segment 文件空间节点的 `cover/` 移入或恢复自 `.reo/trash/segment-covers/<restoreToken>`，response 只返回父 Memory summary、Segment projection 和 restore token。`workspace:switchSegmentDefaultCover` 只接受当前 workspace、parent Memory、Segment identity 和内置 default cover template id；main 只写 Segment manifest 的 `defaultCoverTemplateId`，并返回父 Memory summary 与 Segment projection。Cover mutation channels 都不接受 filename 或 raw path，也不返回 root path、cover path、trash path、selection token 或 handle。
...
- `workspace:saveTranscript` 和 `workspace:saveSegmentSupplementTranscript` 只接受 finalized audio identity、markdown、可选 Tiptap JSON 和 baseline；request 携带 Tiptap JSON 时 baselineTiptapContentHash 必填。Baseline 不匹配时返回 stale typed error，不覆盖 Markdown、不刷新 index、不推进 manifest。成功后写入对应 Markdown transcript 与同级 `content.tiptap.json`，刷新父 Memory projection，并在 ownership 复核后把对应 manifest `lastTranscriptionAttempt` 写为 `'success'`。两个保存 channel 都不返回 raw path、file path、selection token 或 handle。
...
- `workspace:requestSegmentSpeechSynthesis` 和 `workspace:requestSegmentSupplementSpeechSynthesis` 只接受 finalized note identity、`fill-missing` 或 `regenerate` mode，以及可选 one-shot `speaker`。Main 读取 saved note body 文件真源，抽取可读文本，使用当前 main-owned X-Api-Key、`seed-tts-2.0` resource、`seed-tts-2.0-expressive` model、request speaker 或 settings default speaker 和 MP3 24000 Hz 调用豆包 TTS，并把生成的 `speech.mp3` 写回对应 note 文件空间节点。Response 只返回 speech synthesis projection，不返回 raw path、API key、火山 header、provider request body、note body text、speech text、audio bytes 或 base64。
- `workspace:regenerateImportedSpeechSynthesis` 只接受 settings UI 发起的 batch request：`mode: 'all'` 携带 selected speaker，可选 active workspace identity；`mode: 'retry'` 携带 selected speaker、上一批返回的 failed targets 和可选 active workspace identity。Main 只从 main-owned memory space registry 枚举已导入 Memory Space；active workspace 复用当前 handle，其它 workspace 通过 temporary lock 逐个处理。该 channel 对 eligible note Segment 与 note SegmentSupplement 执行 `regenerate`，返回汇总 counts、speaker 和 failedTargets，不返回 raw path、正文、provider request body、audio bytes、base64、X-Api-Key、handle internals 或 registry root。
...
- Memory space initialize 把 selection token 指向的目录视为父目录，记忆空间 title 必须是安全文件夹名；main process 在父目录下用 no-replace `mkdir` 创建同名 child memory space root，child 已存在返回 `ERR_WORKSPACE_ALREADY_EXISTS`，然后只在新 child root 内获取 lock 和写入 Reo 文件。新建或打开时，`AGENTS.md` 内的 Reo managed block、`skills/reo-edit/SKILL.md`、`skills/reo-cover-image/SKILL.md`、`skills/reo-cover-aesthetic/SKILL.md`、`skills/reo-doctor/SKILL.md` 和 skill 内 `scripts/reo-doctor.mjs` 由 main process 写入或更新；初始化不得覆盖已有用户 `AGENTS.md`，打开已有记忆空间时只更新托管块并保留用户内容。`.reo`、`.reo/drafts`、`.reo/drafts/segments`、`memories/` 或 Reo 托管 skill 路径为 symlink 或非目录时不得跟随到记忆空间外；lock 必须绑定当前 `.reo` directory identity，`.reo/workspace.lock` leaf 用 no-follow 打开，`.reo/workspace.lock.lock` 只在同一目录内创建并绑定 identity，owner file 在该目录内 no-follow 创建，写入 main process pid 与进程启动指纹并 fsync；owner 进程已退出、启动指纹不匹配、无效 owner file 或可判定 PID 复用的 owner file 可在下一次 acquire 时替换。
...
- `reo-attachment://<workspaceId>/segments/<segmentId>/<filename>` 和 `reo-attachment://<workspaceId>/segments/<segmentId>/supplements/<supplementId>/<filename>` 只服务当前 active workspace 下 finalized note Segment 或 note SegmentSupplement 的图片附件预览；`reo-attachment://<workspaceId>/memories/<memoryId>/cover/<filename>?v=<version>` 和 `reo-attachment://<workspaceId>/segments/<segmentId>/cover/<filename>?v=<version>` 只服务当前 active workspace 下 Memory 或 Segment `cover/` 子目录中的自定义封面图片。Handler 只接受 GET，按 active workspace handle 解析 root，在 main process 内复核 workspace、entity manifest 或 Memory directory、child directory identity、filename containment、ordinary file 和 25 MiB 上限；response 使用 `Content-Type` 返回 bytes，不返回 raw path。Note attachment response 使用 `Cache-Control: no-store`；cover URL 带 version，成功 response 使用 immutable versioned cache header。`reo-attachment` scheme 启用 CORS 只服务封面 `<img crossorigin="anonymous">` 与 canvas 区域取色；Cover response 仅对可信 `reo-app://renderer` 或当前 loopback dev renderer origin 返回 `Access-Control-Allow-Origin`，普通 note attachment response 不返回该 header。`reo-attachment:` 只进入 renderer `img-src`，不进入 `connect-src`，不作为导航、window open、fetch 主通道或通用文件读取能力。
- Audio append 每个 chunk 最多 1 MiB，每个 recording 只允许 1 个 append 在途。
- Finalized audio playback 使用 finalized audio content projection channel 和独立 audio bytes channel，服务 selected Segment 和 selected SegmentSupplement 的本地播放与 transcript 查看。Renderer 只把 audio bytes response 转为生命周期受控 Blob URL；draft playback 只读取当前 recording draft 或 renderer chunk prefix，不读取 finalized audio，也不返回 raw path。
...
- 豆包 note speech synthesis 在 main 侧运行 TTS V3 HTTP Chunked/SSE：由当前 voice settings 解密得到同一 X-Api-Key，HTTP endpoint 固定为 `https://openspeech.bytedance.com/api/v3/tts/unidirectional`，资源 ID 固定为 `seed-tts-2.0`。请求只使用从 saved note Markdown 转换出的短文本、request speaker 或 settings default speaker、`seed-tts-2.0-expressive` model 和 MP3 24000 Hz audio params。Automatic scanner 只入队 eligible missing/stale note speech target；`regenerate` 来自手动菜单 request 或 settings batch regeneration request。Renderer 不得接触 note speech provider header、request body、正文文本、speech 文本、audio bytes、base64 或 X-Api-Key。
...
- 当前开发启动前按 Node DotEnv 语法加载 `.env.local`；已有 shell 环境变量优先于 `.env.local`，便于临时覆盖本机开发变量。
