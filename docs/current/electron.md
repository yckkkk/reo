...
- `workspace:requestSegmentTranscriptionBackfill` 只接受 `workspaceHandle`、`workspaceId`、`memoryId` 和 `segmentId`；`workspace:requestSegmentSupplementTranscriptionBackfill` 只接受 `workspaceHandle`、`workspaceId`、parent `memoryId`、parent `segmentId` 和 `supplementId`。两个 channel 都校验 trusted sender、handle owner、workspaceId、lock usability、voice settings enabled/key/validation 和目标 eligibility（finalized audio、`lastTranscriptionAttempt='failed'`、transcript 空）。Main 把 manual task 插入 BackfillQueue 队首但不抢占 in-flight，IPC handler await task 完成；成功后复用现有 transcript save 函数写入 Markdown 并返回 save shape，失败返回 `ERR_BACKFILL_*` 或既有 workspace typed error。Response 不返回 transcript provider URL、raw path、audio bytes、X-Api-Key、TOS credential、queue state 或 handle internals。
