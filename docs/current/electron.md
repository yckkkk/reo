...
- `workspace:readFinalizedAudioSegment` request 接受 `workspaceHandle`、`workspaceId`、`memoryId`、`segmentId` 和 requestId；main 校验 sender、handle ownership、workspaceId 与当前 handle workspace 匹配和 lock usability 后，只读取该 Memory 下 matching finalized audio Segment 的 `audio.webm`、`segment.md` transcript section 和同级 `content.tiptap.json`。Response 返回同 requestId、identity、bounded audio bytes、audioByteLength、transcript text、transcript Tiptap JSON、transcript `baselineHash` 和 `baselineTiptapContentHash`，不返回 `workspaceHandle`、root path、file path、selection token 或 raw path。
- `workspace:readFinalizedAudioSegmentSupplement` request 接受 `workspaceHandle`、`workspaceId`、`memoryId`、`segmentId`、`supplementId` 和 requestId；main 校验 sender、handle ownership、workspaceId 与当前 handle workspace 匹配和 lock usability 后，按 supplement manifest id 定位 parent Segment 下 matching finalized audio SegmentSupplement 文件空间节点，并读取其中的 `audio.webm`、`supplement.md` transcript section 和同级 `content.tiptap.json`。Response 返回同 requestId、identity、bounded audio bytes、audioByteLength、transcript text、transcript Tiptap JSON、transcript `baselineHash` 和 `baselineTiptapContentHash`，不返回 `workspaceHandle`、root path、file path、selection token 或 raw path。
...
- `workspace:readSegmentContent` request 接受 `workspaceHandle`、`workspaceId`、`memoryId`、`segmentId` 和 requestId；main 读取 finalized note Segment 的 `segment.md` 正文、`content.tiptap.json` 和 bodyByteLength，并在 response 前 reconcile Markdown 与 sidecar：sidecar 缺失时补齐，sidecar JSON 改变且 Markdown 未变时先 atomic 写回 `segment.md`、manifest byte length 和父 Memory index。Response 返回同 requestId、identity、`type: 'note'`、title、bodyMarkdown、bodyTiptapJson、bodyByteLength、`baselineContentHash` 和 `baselineTiptapContentHash`，不返回 root path、file path、selection token 或 handle。`workspace:writeSegmentContent` 接受同一 finalized note Segment identity、bodyMarkdown、可选 bodyTiptapJson、`baselineContentHash` 和 `baselineTiptapContentHash`；request 携带 bodyTiptapJson 时 baselineTiptapContentHash 必填；main 在当前 Segment directory identity 内先比较当前磁盘正文和 sidecar baseline，匹配后 atomic 写入 `segment.md` 正文与 sidecar、刷新父 Memory summary，并返回 bodyByteLength 与新的两组 baseline hash。Audio Segment transcript save 仍使用 `workspace:saveTranscript`，不走该 channel。
- `workspace:readSegmentSupplementContent` request 接受 `workspaceHandle`、`workspaceId`、parent `memoryId`、parent `segmentId`、`supplementId` 和 requestId；main 读取 finalized note SegmentSupplement 的 `supplement.md` 正文、`content.tiptap.json` 和 bodyByteLength，并在 response 前 reconcile Markdown 与 sidecar：sidecar 缺失时补齐，sidecar JSON 改变且 Markdown 未变时先 atomic 写回 `supplement.md`、manifest byte length 和父 Memory index。Response 返回同 requestId、identity、`type: 'note'`、title、bodyMarkdown、bodyTiptapJson、bodyByteLength、`baselineContentHash` 和 `baselineTiptapContentHash`，不返回 root path、file path、selection token 或 handle。`workspace:writeSegmentSupplementContent` 接受同一 finalized note SegmentSupplement identity、bodyMarkdown、可选 bodyTiptapJson、`baselineContentHash` 和 `baselineTiptapContentHash`；request 携带 bodyTiptapJson 时 baselineTiptapContentHash 必填；main 在当前 SegmentSupplement directory identity 内先比较当前磁盘正文和 sidecar baseline，匹配后 atomic 写入 `supplement.md` 正文与 sidecar、刷新父 Memory summary，并返回 bodyByteLength 与新的两组 baseline hash。Audio SegmentSupplement transcript save 仍使用 `workspace:saveSegmentSupplementTranscript`，不走该 channel。
