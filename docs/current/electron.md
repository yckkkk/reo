...
- `workspace:saveTranscript` 只接受 finalized audio segment identity：`workspaceHandle`、`memoryId` 和 `segmentId`。成功 response 返回当前 Memory summary。Draft create/append/finalize/discard 仍使用 draft `segmentId` transaction identity。
