...
- 当前生产 CSP 包含 `media-src 'self' blob:`，只用于本地 audio playback Blob URL；`img-src` 允许 `'self'`、`data:`、`blob:` 和 `reo-attachment:`，用于 Markdown reading surface、Tiptap editing surface 中 note 图片附件预览，以及 Memory cover 图片预览。生产 CSP 不把 `reo-attachment:` 加入 `connect-src`。
