...
- `destructive`：删除、清空、放弃等不可逆或高风险动作。
...
3. Grayscale State Interaction：hover、active、selected 通过 `card -> secondary/accent` 的灰度阶梯表达；黑色用于核心动作、focus ring 和状态点，ember 只用于 FAB 和录音主按钮，destructive red 只用于危险动作。
...
- Memory card 使用 `ReoCardSurface` 默认形态，Segment preview card 使用 `ReoCardSurface` 的 `segmentPreview` 形态；Segment preview card 按 Figma 参考卡片 88 / 365 的比例还原，136px 卡片约 33px radius，148px 卡片约 36px radius。二者通过灰度填充表达状态，不使用常态 border、blur 或 shadow；普通列表项使用灰度填充状态，不继承 card smoothing。
