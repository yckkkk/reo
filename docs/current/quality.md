...
- Artifact main tests 覆盖 `artifact` Segment / SegmentSupplement recognition、`entry.html` manifest hash/byte 收敛、`state.json` / `runtime.json` / `assets/` 外部修改刷新 preview version、新 artifact 候选 missing/oversized entry needs-review、已识别 artifact missing/oversized entry runtime fault projection、unsupported format review、runtime bundle protocol serving、managed vendor bridge serving、vendor bridge parent-only response、pending cap/timeout、`reo-artifact://` per-object URL containment、GET-only handler、MIME/size gate、response CSP、app shell `frame-src reo-artifact:`、artifact prompt-copy IPC/preload bridge、runtime prompt untrusted quoting/size gate、runtime state baseline conflict/size gate、runtime secret object+slot safeStorage persistence、undeclared slot rejection、secure storage unavailable failure 和 managed scaffold no-user-text `innerHTML` rendering。
...
- Content tab action 和 inline text edit tests 覆盖当前用户可见合同：primary content More 只承载内容槽路径、重命名和清空；Workspace titlebar 右侧 `新记忆` 入口不承载 Segment 创建；finalized 转录、Note 正文和补充内容共用 Tiptap-backed 轻量 Markdown editor surface；finalized inline text autosave、stale save、baseline 更新、toolbar 能力和 Tiptap JSON 携带按当前编辑合同验证。
