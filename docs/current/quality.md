...
- Workspace Widget tests 覆盖 `widgets/*/widget.md` 识别、`entry.html` hash/byte 收敛、runtime fault 投影、invalid Widget needs-review、`widgetTabOrder` 归一化、IPC/preload bridge surface、`reo-render://` Widget URL serving、titlebar rail tab 展开行为和 runtime bridge 的 `ui.selectMemory`。
...
- `test/main/workspaceFiles.test.ts` 覆盖记忆空间初始化、打开、managed `AGENTS.md` block、`skills/reo-edit` / `skills/reo-cover-image` / `skills/reo-cover-aesthetic` / `skills/reo-works` / `skills/reo-generative-runtime` / `skills/reo-works-design` / `skills/reo-doctor` 生成，作品与 runtime skill reference/script 文件生成，缺失配置静默补全和 doctor script 不覆盖用户 `AGENTS.md` 内容。
