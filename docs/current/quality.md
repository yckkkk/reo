...
- Workspace entry UI 由 renderer tests 覆盖：Home `+` 打开 create/open workspace Dialog、Dialog accessible name/description、Dialog close control、create/open pending 互斥、初始 focus、submit-time validation、folder picker cancel 保留输入、`AGENTS.md` conflict alert、open existing workspace branch、raw path/displayPath 不进入 IPC payload、workspace snapshot query key 不包含 handle、initialize/open 成功后 route state 切换。
- App shell UI 由 renderer tests 覆盖：starter shell 只显示 Home、loaded shell 显示 Home/New memory、sidebar 240-520px resize clamp、8px resize hit area、separator keyboard resize、covered panel transform、drag resize 关闭 motion、pointer cancel cleanup、light/dark theme toggle、创建 workspace 后 theme 保持、未实现 media/file capability 负向断言。
...
- Workspace home 和最小 UI primitives 由 renderer tests 覆盖：Button role/name/focus-visible/disabled、Label accessible name、`All memories` header、本地 `Search memories` 过滤、月份分组、memory card metadata、memory card 打开当前 detail、空 workspace 状态、单一 record action、no future capabilities。
- Memory detail 由 renderer tests 覆盖：通过 `getMemoryDetail` 渲染 title/date/recordings/Transcript/Reflections/Memory content、按 `hasTranscript/hasReflections` 显示 saved/empty 状态、detail query key 不包含 `workspaceHandle`、More/Rename/Delete/Show in folder/Export/Films/photo/video/file/AI 不进入 runtime command surface、App Home ↔ detail ↔ Home in-memory route。
