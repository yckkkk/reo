...
- TanStack Query、React Hook Form 和 `@hookform/resolvers` 已安装，当前服务 workspace creation form、workspace project list、workspace snapshot cache 和 memory detail cache。
...
- Workspace project registry 由 main tests 覆盖：registry project metadata 不暴露 root path、list response 遇到 rootPath unknown field 必须 reject、registry entry 跨新 registry 实例持久读取、并发 upsert 不丢项目、损坏 registry JSON 按空列表处理、不可读 registry 返回 `ERR_WORKSPACE_PROJECT_REGISTRY_READ_FAILED`、registry project count 和 display text 有界、`openProject` 使用跨实例 registry 文件打开项目且不需要 selection token、persisted root folder missing 返回 `ERR_WORKSPACE_ROOT_MISSING`、`removeProject` 只移除 registry entry 且不触碰 workspace folder、remove missing project 不写 registry。
- Workspace create/open 行为由 main tests 覆盖：create selection folder 是 parent location，initialize 在 parent 下 no-replace 创建 title 同名 child workspace root；同名 child 已存在返回 `ERR_WORKSPACE_ALREADY_EXISTS`；open 空文件夹原地初始化为 Reo workspace；open 非空非 Reo 文件夹返回 metadata invalid 且不写 `.reo/workspace.lock*`；open 空文件夹在 lock 后变成非空时返回 metadata invalid、清理 lock-only `.reo` 并允许重新获取 lock。
- Workspace entry UI 由 renderer tests 覆盖：Home 主内容区不显示独立 `+` 创建按钮、sidebar 添加菜单打开专用“创建本地工作区” Dialog、Dialog accessible name、Dialog close control、workspace action pending 保护、初始 focus、名称/描述/位置字段、workspace 名称 folder-safe validation、description payload、submit-time validation、folder picker cancel 保留输入、同名 workspace folder error、`打开本地工作区` open branch、导入项目重启后显示在 sidebar、点击导入项目只发送 `workspaceId`、已删除 workspace folder 的导入项目显示 missing-folder alert、sidebar 更多菜单确认移除项目且不删除本地文件夹、移除成功显示 shared toast、移除失败只显示 toast 且不在弹层内重复显示旧错误、移除 active 项目时 registry entry removal 不被 close failure 阻断、切换 persisted/local workspace 会释放旧 handle、open promise reject 显示可见 alert、sidebar `首页` 从 loaded workspace 释放当前 handle 并回到 starter shell、loaded workspace 中 `首页` 不保持 current page 高亮、raw path/displayPath 不进入 IPC payload、workspace projects/snapshot query key 不包含 handle、initialize/open 成功后 route state 切换。
- App shell 与 App flow UI 由 renderer tests 覆盖：sidebar 显示 `首页`/`资料库` 且不显示 `新记忆`、`资料库` 打开空白 Library page、`首页`/`资料库` 和 workspace 项目项触发真实导航并关闭已打开的 sidebar 菜单、workspace 项目更多操作菜单和 `移除工作区` menuitem accessible name、workspace 项目更多按钮位于同一项目行容器内且仅在对应项目行 hover/focus/open 时显示、48px 无边框 titlebar shell slot、main panel titlebar slot、titlebar drag/no-drag region、titlebar hide/show control token 定位和垂直居中、sidebar 240-520px resize clamp、8px resize hit area、separator keyboard resize、covered panel right-edge anchoring、sidebar add menu stacking above main panel、add menu trigger-left anchoring、project header hover/focus/open reveal、interactive component accessible names、drag resize 关闭 motion、pointer cancel cleanup、light/dark theme toggle、创建 workspace 后 theme 保持、打开本地工作区失败时 starter/loaded shell 可见 alert 且不打开创建弹层、Home/detail 打开 recording drawer、detail 录音 finalize 传当前 `memoryId` 并刷新 active memory detail query、未实现 media/file/AI/auth/global search capability 负向断言。
