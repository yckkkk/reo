...
- Finalized note external edit tests 覆盖 note Segment 和 note SegmentSupplement content read baseline hash、Tiptap JSON sidecar baseline、write request baseline 必填、stale body save 返回 `ERR_SEGMENT_CONTENT_STALE` 与当前磁盘 Markdown/Tiptap JSON/hash、renderer edit save 发送 baseline、visibility refresh 下 dirty editor 显示磁盘变化提示且保留 dirty body、clean editor 更新到磁盘正文、same-snapshot body-only 外部修改也触发 content Query refresh、重新选择 cached note Segment 时重新读取文件真源、stale save 的 `使用磁盘版本` 重置 Markdown/Tiptap JSON/baseline，以及 `保留我的修改` 使用当前磁盘 baseline 重试。
- Content tab action 和 inline text edit tests 覆盖当前用户可见合同：primary content More 只承载内容槽路径、重命名和清空；Memory 顶层 `新片段` 入口只在 workspace titlebar；finalized 转录、Note 正文和补充内容共用 Tiptap-backed 轻量 Markdown editor surface；stale save、baseline 更新、toolbar 能力和 Tiptap JSON 携带按当前编辑合同验证。
- Tiptap Markdown codec 和 sidecar main tests 覆盖 `@tiptap/markdown` parse/serialize、highlight、underline、colored highlight、非法 highlight color 拒绝、text-align、superscript/subscript、core Markdown node roundtrip、sidecar missing/regenerate/JSON-to-Markdown/write conflict、invalid sidecar、非有限 JSON attrs 拒绝、unsupported Tiptap content review surface，以及 audio Segment / audio SegmentSupplement transcript 的 sidecar 生成、JSON-to-Markdown 镜像和 stale baseline 防护。
- Memory file truth tests 覆盖直接 Markdown 创建 note Segment/note SegmentSupplement、duplicate id exclusion、note/audio Segment 跨 Memory 移动、note/audio SegmentSupplement 跨 Segment 移动的 manifest parent reconcile，以及 transcript H2 不截断正文。
