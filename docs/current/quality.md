...
- `test/main/workspaceFiles.test.ts` 覆盖记忆空间初始化、打开、managed `AGENTS.md` block、`skills/reo-edit` / `skills/reo-cover-image` / `skills/reo-cover-aesthetic` / `skills/reo-works` / `skills/reo-generative-runtime` / `skills/reo-works-design` / `skills/reo-doctor` 生成，作品与 runtime skill reference/script 文件生成，缺失配置静默补全、doctor orphan `.reo/objects` mirror 诊断和 doctor script 不覆盖用户 `AGENTS.md` 内容。
