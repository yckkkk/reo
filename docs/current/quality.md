...
- Workspace entry UI 由 renderer tests 覆盖：初始 focus、submit-time validation、folder picker cancel 保留输入、`AGENTS.md` conflict alert、open existing workspace branch、raw path/displayPath 不进入 IPC payload、workspace snapshot query key 不包含 handle、initialize/open 成功后 route state 切换。
