...
- `class-variance-authority`、`clsx`、`tailwind-merge`、`@radix-ui/react-slot`、`@radix-ui/react-label`、`@radix-ui/react-dialog`、`@radix-ui/react-tooltip`、`@radix-ui/react-separator`、`vaul` 和 `lucide-react` 已安装，当前服务 Button、Label、Dialog、Drawer、Textarea、Tooltip、Separator、App shell、recording audio controls 和 icon controls。
