...
- Content tab action 和 transcript edit tests 覆盖 primary `转录` / `正文` tab More 的 slot 菜单语义、Segment card 与 audio SegmentSupplement tab 保留生成/重新生成转录入口、Segment primary content title 投影与更新 IPC、audio Segment 和 audio SegmentSupplement transcript `baselineHash`、stale transcript overwrite 拒绝、转录编辑 dirty close、保存、清空以及重新生成转录保留 content title。
