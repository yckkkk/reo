...
- Memory file truth tests 覆盖 agent 直接创建带冗余 `memory.md` id 的 Memory、直接 Markdown 创建 note Segment/note SegmentSupplement、artifact Segment/artifact SegmentSupplement、duplicate id exclusion、note/audio/artifact Segment 跨 Memory 移动、note/audio/artifact SegmentSupplement 跨 Segment 移动的 manifest parent reconcile，以及 transcript H2 不截断正文。
