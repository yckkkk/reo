...
- Content tab action 和 inline text edit tests 覆盖当前用户可见合同：primary content More 只承载内容槽路径、重命名和清空；Memory 顶层 `新片段` 入口只在 workspace titlebar；finalized 转录、Note 正文和补充内容共用 Tiptap-backed 轻量 Markdown editor surface；finalized inline text autosave、stale save、baseline 更新、toolbar 能力和 Tiptap JSON 携带按当前编辑合同验证。
