...
- Artifact main tests 覆盖 `artifact` Segment / SegmentSupplement recognition、`entry.html` manifest hash/byte 收敛、`state.json` / `runtime.json` / `assets/` 外部修改刷新 preview version、missing entry needs-review、unsupported format review、runtime bundle protocol serving、managed vendor bridge serving、`reo-artifact://` per-object URL containment、GET-only handler、MIME/size gate、response CSP、app shell `frame-src reo-artifact:`、artifact prompt-copy IPC/preload bridge、runtime state baseline conflict/size gate、runtime secret object+slot safeStorage persistence、undeclared slot rejection 和 secure storage unavailable failure。
