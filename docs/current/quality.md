...
- Memory file truth tests 覆盖 agent 直接创建带冗余 `memory.md` id 的 Memory、直接 Markdown 创建 note Segment/note SegmentSupplement、duplicate id exclusion、note/audio Segment 跨 Memory 移动、note/audio SegmentSupplement 跨 Segment 移动的 manifest parent reconcile，以及 transcript H2 不截断正文。
...
- `test/main/workspaceFiles.test.ts` 覆盖记忆空间初始化、打开、managed `AGENTS.md` block、`skills/reo-edit` / `skills/reo-memory-cover` / `skills/reo-cover-aesthetic` / `skills/reo-doctor` 生成、缺失配置静默补全和 doctor script 不覆盖用户 `AGENTS.md` 内容。
