...
- Artifact main tests 覆盖 `artifact` Segment / SegmentSupplement recognition、`entry.html` manifest hash/byte 收敛、`state.json` 外部修改不刷新 host preview version、`runtime.json` / direct `assets/` 外部修改刷新 preview version、新 artifact 候选 missing/oversized entry needs-review、已识别 artifact missing/oversized entry runtime fault projection、unsupported format review、runtime bundle protocol serving、managed vendor bridge serving、vendor bridge parent-only response、pending cap/timeout、`reo-artifact://` per-object URL containment、GET-only handler、MIME/size gate、response CSP、app shell `frame-src reo-artifact:`、artifact prompt-copy IPC/preload bridge、runtime prompt untrusted quoting/size gate、runtime state baseline conflict/size gate、unknown runtime method rejection、inline script syntax validation 和 managed scaffold no-user-text `innerHTML` rendering。
