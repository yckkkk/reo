...
- Artifact main tests 覆盖 `artifact` Segment / SegmentSupplement recognition、`entry.html` manifest hash/byte 收敛、`state.json` / `runtime.json` / `assets/` 外部修改刷新 preview version、missing entry needs-review、unsupported format review、runtime bundle protocol serving、`reo-artifact://` per-object URL containment、GET-only handler、MIME/size gate、response CSP、app shell `frame-src reo-artifact:` 和 artifact prompt-copy IPC/preload bridge。
