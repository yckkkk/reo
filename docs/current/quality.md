...
- Artifact main tests 覆盖 `artifact` Segment / SegmentSupplement recognition、manifest entry hash/byte 收敛、missing entry needs-review、unsupported format review、`reo-artifact://` protocol containment、GET-only handler、MIME/size gate、response CSP、app shell `frame-src reo-artifact:` 和 artifact prompt-copy IPC/preload bridge。
...
- `test/main/workspaceFiles.test.ts` 覆盖记忆空间初始化、打开、managed `AGENTS.md` block、`skills/reo-edit` / `skills/reo-cover-image` / `skills/reo-cover-aesthetic` / `skills/reo-works` / `skills/reo-works-design` / `skills/reo-doctor` 生成，作品 skill reference 文件生成，缺失配置静默补全和 doctor script 不覆盖用户 `AGENTS.md` 内容。
