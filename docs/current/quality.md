...
- TanStack Query、React Hook Form 和 `@hookform/resolvers` 已安装，当前服务 memory space creation form、memory space list、记忆空间 snapshot cache 和 memory detail cache。
...
- Task 2 后 main/preload tests 覆盖：无 one-shot intent 拒绝 microphone permission request、matching sender intent 只能消费一次、过期 intent 拒绝、permission check 永不 grant media、同 sender 第二个 active intent 返回 `ERR_MIC_INTENT_ALREADY_ACTIVE`、clear 只清 matching workspace/drawer owner、clear 在 lock lost 后仍清理 matching pending intent、记忆空间 close/window teardown 清理 pending intents、close 在 lock lost 或 release failure 分支不保留 pending intent、untrusted origin 消费后仍拒绝、video/camera 拒绝、memory detail delayed lock lost、memory detail response strip raw path、memory detail IPC、microphone intent IPC、preload surface 无 generic invoke/send。
- Memory space registry 由 main tests 覆盖：registry metadata 不暴露 root path、listMemorySpaces response 遇到 rootPath unknown field 必须 reject、registry entry 跨新 registry 实例持久读取、并发 upsert 不丢记忆空间、损坏 registry JSON 按空列表处理、不可读 registry 返回 `ERR_WORKSPACE_MEMORY_SPACE_REGISTRY_READ_FAILED`、registry count 和 display text 有界、`openMemorySpace` 使用跨实例 registry 文件打开记忆空间且不需要 selection token、persisted root folder missing 返回 `ERR_WORKSPACE_ROOT_MISSING`、`removeMemorySpace` 只移除 registry entry 且不触碰本地记忆空间文件夹、remove missing memory space 不写 registry。
- Memory space create/open 行为由 main tests 覆盖：create selection folder 是 parent location，initialize 在 parent 下 no-replace 创建 title 同名 child memory space root；同名 child 已存在返回 `ERR_WORKSPACE_ALREADY_EXISTS`；open 空文件夹原地初始化为 Reo 记忆空间；open 非空非 Reo 文件夹返回 metadata invalid 且不写 `.reo/workspace.lock*`；open 空文件夹在 lock 后变成非空时返回 metadata invalid、清理 lock-only `.reo` 并允许重新获取 lock。
- Memory space entry UI 由 renderer tests 覆盖：Home 主内容区不显示独立 `+` 创建按钮、sidebar 添加菜单打开专用“创建本地记忆空间” Dialog、Dialog accessible name、Dialog close control、memory space action pending 保护、初始 focus、名称/描述/位置字段、记忆空间名称 folder-safe validation、description payload、submit-time validation、folder picker cancel 保留输入、同名 memory space folder error、`打开本地记忆空间` open branch、导入记忆空间重启后显示在 sidebar、点击导入记忆空间只发送 `workspaceId`、已删除本地文件夹的导入记忆空间显示 missing-folder alert、sidebar 更多菜单确认移除记忆空间且不删除本地文件夹、移除成功显示 shared toast、移除失败只显示 toast 且不在弹层内重复显示错误、移除 active memory space 时 registry entry removal 不被 close failure 阻断、切换 persisted/local memory space 会释放既有 handle、open promise reject 显示可见 alert、sidebar `首页` 从 loaded memory space 释放当前 handle 并回到 starter shell、loaded memory space 中 `首页` 不保持 current page 高亮、raw path/displayPath 不进入 IPC payload、memory space list/snapshot query key 不包含 handle、initialize/open 成功后 route state 切换。
- App shell 与 App flow UI 由 renderer tests 覆盖：sidebar 显示 `首页`/`资料库` 且不显示 `新记忆`、`资料库` 打开空白 Library page、`首页`/`资料库` 和记忆空间列表项触发真实导航并关闭已打开的 sidebar 菜单、记忆空间更多操作菜单和 `移除记忆空间` menuitem accessible name、记忆空间更多按钮位于同一记忆空间行容器内且仅在对应记忆空间行 hover/focus/open 时显示、48px 无边框 titlebar shell slot、main panel titlebar slot、titlebar drag/no-drag region、titlebar hide/show control token 定位和垂直居中、sidebar 240-520px resize clamp、8px resize hit area、separator keyboard resize、covered panel right-edge anchoring、sidebar add menu stacking above main panel、add menu trigger-left anchoring、记忆空间 header hover/focus/open reveal、interactive component accessible names、drag resize 关闭 motion、pointer cancel cleanup、light/dark theme toggle、创建记忆空间后 theme 保持、打开本地记忆空间失败时 starter/loaded shell 可见 alert 且不打开创建弹层、Home/detail 打开 recording drawer、detail 录音 finalize 传当前 `memoryId` 并刷新 active memory detail query、未实现 media/file/AI/auth/global search capability 负向断言。
...
- Memory space home 和最小 UI primitives 由 renderer tests 覆盖：Button role/name/focus-visible/disabled、Label accessible name、`全部记忆` header、本地 `搜索记忆` 过滤、月份分组、memory card metadata、memory card 打开当前 detail、空记忆空间状态、单一 record action、no future capabilities。
...
- Workspace snapshot contract 由 main tests 覆盖：snapshot 只允许 `workspaceId`、title、description 和 `memories[]`，拒绝顶层 recording 投影，避免同一 recording 事实在 snapshot 与 memory detail 中重复存储。
- 当前 recording loop 由 renderer tests 覆盖：record/pause/resume/stop lifecycle、begin microphone intent 先于 media acquisition、begin intent 失败不启动 media acquisition、begin resolves after unmount 会 clear pending intent 且不创建 draft、draft creation pending 时 unmount 或 记忆空间 handle change 会 clear pending intent、draft 返回后 discard 且不启动 media、media start pending 时 unmount 会 clear pending intent、discard draft 并在 controller resolve 后 stop、media start failure clear pending intent、controller ready 前不显示 stop 或 finalize、pause/resume 只影响 elapsed timer 和 media controller、录音过程中不生成 mock transcript、finalize 后 transcript draft 初始为空、finalize waits for append ack、finalize 传递 active recording clock duration、existing memory target `memoryId`、sub-second duration、finalize 后 session 同步 memory summary projection 并刷新 active memory detail query、append error/reject 立即 failed 且不 finalize、failed 后 retry、failed retry draft/timer reset、failed recorder cleanup、failed draft discard、stale stop ignore、stale chunk ignore、autosave failure draft retention、close/reopen 后忽略 stale autosave 结果、manifest/chunk playback、`转写预览`、local playback surface、unmount Blob URL revoke、close recording panel Blob URL revoke、close 后过期 playback request 不创建 Blob URL、close 后停止继续调度后续 chunk IPC、chunk read 失败后停止继续调度后续 chunk IPC，以及 unmount 后过期 playback request 不创建 Blob URL。
