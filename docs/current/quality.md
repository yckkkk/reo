...
- Renderer source 禁止直接 import Electron 和所有 Node builtin，包括 bare builtin 与 `node:` 前缀；restricted import 规则必须由测试覆盖。
- Preload source 只允许 import `electron` 和相对 preload/contract module，不得引入 Zod-backed contract、Node builtin 或普通 package；preload path 必须指向 `out/preload/index.cjs`，source crawl 与 lint 边界必须由测试覆盖。
