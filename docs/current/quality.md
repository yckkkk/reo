...
- `test/main/workspaceFiles.test.ts` 覆盖记忆空间初始化、打开、managed `AGENTS.md` block、`skills/reo-edit` / `skills/reo-cover-image` / `skills/reo-cover-aesthetic` / `skills/reo-doctor` 生成、缺失配置静默补全和 doctor script 不覆盖用户 `AGENTS.md` 内容。
