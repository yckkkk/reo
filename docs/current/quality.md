...
- Content tab action 和 transcript edit tests 覆盖 primary `转录` / `正文` tab More 的路径、编辑、重命名和清空能力，且不混入生成转录、重新生成转录或删除；content tab 动作组不承载 Memory 顶层 `新片段` 入口；Memory Studio 的片段预览流不承载 `新片段` menu；选中 Memory 后 titlebar 不显示全局 `新建记忆`，改为在 MemoryRail 折叠按钮左侧显示 Memory 顶层 `新片段` menu；Memory Studio 仍挂载红色 Expression FAB，但不把 FAB 放进片段预览流或 content tab 动作组；Note Segment 不渲染可操作 audio 播放 row 或不可播放文案，但渲染等高不可见 player placeholder；Note Segment card 使用人类可读字节标签；Note SegmentSupplement 空态可从面板内进入编辑；Segment card 与 audio SegmentSupplement tab 保留生成/重新生成转录入口；Segment primary content title 投影与更新 IPC；audio Segment 和 audio SegmentSupplement transcript `baselineHash`；stale transcript overwrite 拒绝；转录编辑 dirty close、保存、清空和清空确认文案；Note、Transcript 与补充笔记共用轻量 Markdown textarea surface；Note editor surface 显示 `Markdown 笔记` strip；Transcript editor surface 显示 `Markdown 转录` strip；Note/Transcript 保存按钮位于编辑器 surface 内；编辑 overlay 打开时隐藏 ExpressionDock；Workspace titlebar 使用 `新片段` menu 承载当前 Memory 的录音/笔记创建；重新生成转录保留 content title。
