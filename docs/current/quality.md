...
- `test:renderer`：使用 Vitest 运行 `src/renderer/**/*.test.{ts,tsx}`，测试环境为 jsdom，setup 文件加载 Testing Library DOM matchers、pointer capture 测试替身，并在每个测试后执行 DOM cleanup。
...
- Workspace entry UI 由 renderer tests 覆盖：Home `+` 打开 create/open workspace Dialog、Dialog close control、create/open pending 互斥、初始 focus、submit-time validation、folder picker cancel 保留输入、`AGENTS.md` conflict alert、open existing workspace branch、raw path/displayPath 不进入 IPC payload、workspace snapshot query key 不包含 handle、initialize/open 成功后 route state 切换。
- App shell UI 由 renderer tests 覆盖：starter shell 只显示 Home、loaded shell 显示 Home/New memory、sidebar 240-520px resize clamp、8px resize hit area、separator keyboard resize、covered panel transform、drag resize 关闭 motion、pointer cancel cleanup、未实现 media/file capability 负向断言。
...
- Workspace home 和最小 UI primitives 由 renderer tests 覆盖：Button role/name/focus-visible/disabled、Label accessible name、`All memories` header、本地 `Search memories` 过滤、月份分组、memory card metadata、空 workspace 状态、单一 record action、no future capabilities。
