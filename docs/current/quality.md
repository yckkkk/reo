...
- `class-variance-authority`、`clsx`、`tailwind-merge`、`@radix-ui/react-slot`、`@radix-ui/react-label`、`@radix-ui/react-dialog`、`@radix-ui/react-alert-dialog`、`@radix-ui/react-dropdown-menu`、`@radix-ui/react-select`、`@radix-ui/react-tooltip`、`@radix-ui/react-separator`、Tiptap React/StarterKit/Markdown/Image、`primereact`、`vaul` 和 `lucide-react` 已安装，当前服务 Button、Label、Dialog、AlertDialog、DropdownMenu、Select、Floating Action Button Speed Dial、Drawer、Textarea、Tooltip、Separator、App shell、lightweight Markdown editor、recording audio controls 和 icon controls。
...
- Artifact main tests 覆盖 `artifact` Segment / SegmentSupplement recognition、`entry.html` manifest hash/byte 收敛、`state.json` 外部修改不刷新 host preview version、`runtime.json` / direct `assets/` 外部修改刷新 preview version、新 artifact 候选 missing/oversized entry needs-review、已识别 artifact missing/oversized entry runtime fault projection、unsupported format review、runtime bundle protocol serving、managed vendor bridge serving、vendor bridge parent-only response、pending cap/timeout、`reo-render://` per-object URL containment、GET-only handler、MIME/size gate、response CSP、app shell `frame-src reo-render:`、artifact prompt-copy IPC/preload bridge、runtime prompt untrusted quoting/size gate、runtime state baseline conflict/size gate、unknown runtime method rejection、inline script syntax validation 和 managed scaffold no-user-text `innerHTML` rendering。
- Workspace Widget tests 覆盖 `widgets/*/widget.md` 识别、`entry.html` hash/byte 收敛、runtime fault 投影、invalid Widget needs-review、`widgetTabOrder` 归一化、IPC/preload bridge surface、`reo-render://` Widget URL serving、titlebar rail tab 展开行为和 runtime bridge 的 `ui.selectMemory` / `ui.selectObject`。
- Finalized note external edit tests 覆盖 note Segment 和 note SegmentSupplement content read baseline hash、Tiptap JSON sidecar baseline、write request baseline 必填、stale body save 返回 `ERR_SEGMENT_CONTENT_STALE` 与当前磁盘 Markdown/Tiptap JSON/hash、renderer edit save 发送 baseline、visibility/file-truth refresh 下 dirty editor 显示磁盘变化提示且保留 dirty body、clean editor 更新到磁盘正文、same-snapshot body-only 外部修改也触发 Memory detail 与 content Query refresh、workspace file truth watcher symlink/temp/error 红线、重新选择 cached note Segment 时重新读取文件真源、stale save 的 `使用磁盘版本` 重置 Markdown/Tiptap JSON/baseline，以及 `保留我的修改` 使用当前磁盘 baseline 重试。
- Note speech synthesis tests 覆盖 note Segment 和 note SegmentSupplement speech projection、dedicated MP3 audio read、audio hash 校验、empty body speech cleanup、speech/body save serialization、provider malformed data rejection、Markdown/HTML cleanup、long-text chunking、automatic failed marker、settings batch regeneration cache invalidation、renderer lazy audio query 和 stable playback resource key。
- Content tab action 和 inline text edit tests 覆盖当前用户可见合同：primary content More 只承载内容槽路径、重命名和清空；Workspace titlebar 右侧 `新增` 菜单中的 `新建记忆` 入口不承载 Segment 创建；finalized 转录、Note 正文和补充内容共用 Tiptap-backed 轻量 Markdown editor surface；finalized inline text autosave、stale save、baseline 更新、toolbar 能力和 Tiptap JSON 携带按当前编辑合同验证。
- Tiptap Markdown codec 和 sidecar main tests 覆盖 `@tiptap/markdown` parse/serialize、highlight、underline、colored highlight、非法 highlight color 拒绝、text-align、superscript/subscript、core Markdown node roundtrip、sidecar missing/regenerate/JSON-to-Markdown/write conflict、invalid sidecar、非有限 JSON attrs 拒绝、unsupported Tiptap content review surface、note draft Markdown/Tiptap sidecar 写入失败回滚，以及 audio Segment / audio SegmentSupplement transcript 的 sidecar 生成、JSON-to-Markdown 镜像和 stale baseline 防护。
- Memory file truth tests 覆盖 agent 直接创建带冗余 `memory.md` id 的 Memory、直接 Markdown 创建 note Segment/note SegmentSupplement、artifact Segment/artifact SegmentSupplement、duplicate id exclusion、note/audio Segment 跨 Memory 移动、note/audio SegmentSupplement 跨 Segment 移动、audio/artifact Segment 和 audio/artifact SegmentSupplement 跨 memory space 移动、整 Memory 跨 memory space 移动的 manifest parent reconcile，以及 transcript H2 不截断正文。
...
- `test/main/workspaceFiles.test.ts` 覆盖记忆空间初始化、打开、`.reo/REO.md` 托管入口、一次性 root `AGENTS.md` 指针、`skills/reo-edit` / `skills/reo-cover-image` / `skills/reo-cover-aesthetic` / `skills/reo-works` / `skills/reo-generative-runtime` / `skills/reo-works-design` / `skills/reo-doctor` 生成，作品与 runtime skill reference/script 文件生成，缺失配置静默补全、doctor orphan `.reo/objects` mirror 诊断和 doctor script 不覆盖用户 `AGENTS.md` 内容。
...
- Main tests 必须覆盖 workspace file truth、IPC contract、trusted sender、selection token、single-writer lock、filesystem containment、atomic write、metadata schema、registry、index rebuild/recovery、entity read/write/delete/restore、recording draft/finalize、markdown save、note attachments、custom protocols、artifact recognition/prompt copy、error envelope、lock-lost behavior、diagnostics redaction、voice transcription 和 backfill runtime 的当前合同。
- Renderer tests 必须覆盖当前用户可见行为和 owner 边界，而不是绑定内部 class 串。最小覆盖面包括 App shell、WorkspaceFrame/MemoryRail、Memory Studio selection/content/query ownership、artifact preview/prompt actions、artifact runtime bridge origin/source validation、shared UI primitives、实体菜单、危险操作、optimistic update/rollback、session cleanup、content tab ordering、playback、inline text editing、note attachments、recording overlay、voice settings 和 backfill 用户可见状态。
