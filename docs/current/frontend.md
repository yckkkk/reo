...
- 当前 6 个实体 surface 是 sidebar Memory Space item、loaded workspace titlebar Memory Space title、loaded workspace titlebar current Memory title、MemoryRail Memory item、Memory Studio Segment card 和 Memory Studio SegmentSupplement tab。它们共用 4 个 feature-local typed 菜单壳：`MemorySpaceActionsMenu`（5 项）、`MemoryActionsMenu`（7 项，包含恢复随机默认图片）、`SegmentActionsMenu`（8 项，包含恢复随机默认图片）和 `SegmentSupplementActionsMenu`（7 项），组件位于 `src/renderer/src/workspace/`。Memory Studio primary content tab 使用 `SegmentContentActionsMenu` 表达正文/转录 slot 操作，不承载实体删除或转录生成入口。
