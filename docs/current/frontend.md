...
- SegmentAttachment tab item 保留 attachment type icon、label、active pill、More button 和 reorder anchor。active 只表达当前内容选择；More 是 tab button 的 sibling DropdownMenu trigger，不嵌入 tab button。More 的视觉展开只使用 owner tab item 的 CSS `group-hover` 和 More trigger 自身 `focus-visible`；active selection 只让当前 More 进入键盘路径，不让它视觉常驻展开，DropdownMenu open 也不改变视觉折叠。非 active 且未 hover 的 More 同步收束 pointer events、tabIndex 和 `aria-hidden`；active 但未 hover/focus 的 More 仍保持 `max-width: 0` 和 `opacity: 0`。DropdownMenu 关闭时如果 trigger 已隐藏，焦点回到对应 tab。当前 More 菜单提供 `重命名` 和 `删除`：重命名打开 `SegmentAttachmentRenameDialog`；删除打开 `SegmentAttachmentDeleteDialog`，成功或 `file-written-index-stale` 后都用统一 undo toast 提供恢复入口。reorder 需要持久顺序合同前只作为结构落点，不做本地假排序。内容 tab 的 id、panel id、keyboard selection 和 `aria-controls` / `aria-labelledby` 来自同一份 selected Segment content tab model，不用 DOM query 或 click side effect 驱动状态。
