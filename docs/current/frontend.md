...
- App 当前把普通 Segment 与 SegmentSupplement 的转录重试 callback 都接到 root toast 占位提示「转录引擎尚未上线」；转录重试的真实后台任务和 IPC 写入路径尚未接入。
...
- `SegmentTranscriptView` 是 Memory Studio 的 feature-local transcript surface，使用 outcome 表达 `success`、`empty-never`、`empty-cleared` 和 `failed-retryable`。成功态转录正文保持可选中；两类空态渲染 owner 提供的空态文案；可重试失败态渲染 owner 提供的失败文案和 shadcn/ui compact secondary Button，缺少 retry handler 时按钮禁用。
