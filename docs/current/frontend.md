...
- `rounded-full` 只用于 FAB trigger、FAB action、录音主按钮、Segment strip overlay arrow、圆点、timeline marker 和 drawer/waveform handle。普通文本按钮和 compact 文本按钮使用 `rounded-md`，32px icon button 使用 `rounded-sm`，40px icon button 和菜单 action 使用 `rounded-md`，56px icon button 使用 `rounded-lg`，titlebar Breadcrumb trigger 使用 `rounded-sm`，输入框使用方圆角。Memory card 使用 `ReoCardSurface` 默认形态；Segment preview card 使用 `ReoCardSurface` 的 `segmentPreview` 形态。
...
- Toast：root 只挂载一次，toast surface 使用 `bg-popover text-popover-foreground shadow-float`。所有 toast action 使用同一 action button 结构；默认态透明、无边框、无填充底色，hover/active 只用 `color-mix` 填充高光，不画描边，focus-visible 保留键盘可达 ring。所有 toast close button 统一位于 surface 右上角，close icon 为 16px，默认、hover、active 和 focus-visible 都不显示填充底色。Toast action 使用文字在左、16px icon 在右的 icon+text 结构，不使用裸文字按钮；needs-review 复制成功后，同一 action 临时显示「已复制」和 16px Check icon，不额外弹成功 toast。可撤销删除 toast 统一通过 `showReoToast` 的 undo 输入使用同一 neutral surface、目标标题、`恢复` action、右上角 close button 和底部 2px 进度条；Memory delete 的 undo 调用 main restore，Segment delete 的 undo 只回滚 renderer pending projection。Segment delete toast 手动关闭等同接受删除，Undo action 会先回滚本地 pending 状态再 dismiss toast，避免触发提交。10 秒 toast duration 是 Segment delete grace-period 唯一时钟。
- Separator：保留语义和命中区，不默认画可见线。
- Switch：Radix mechanics，source 位于 `components/ui/switch.tsx`，当前 consumer 是 `VoiceSettingsPanel` 的「启用豆包语音识别」开关；轨道使用 `bg-secondary`，checked 通过 `data-[state=checked]:bg-primary` 使用中性 `--primary`，disabled 使用 `bg-muted`，thumb 使用 `bg-background` 并通过 Radix checked state 位移，不使用描边或阴影。
...
- AppShell sidebar 的首页、资料库和 Memory Space list row 共用 32px 高度、`reo-squircle rounded-md`、`bg-secondary text-foreground` 选中填充；inactive/hover/focus-within 只在同一 row recipe 上改变灰阶，不让 Memory Space list item 维护第二套 active 填充形态。
...
- Titlebar 使用 shadcn/ui Breadcrumb 组合当前记忆空间标题和当前 Memory 标题，两个层级都接入实体 More 菜单，中间 separator 使用 4px 圆点；titlebar 内容使用 `px-28`，右侧操作区始终显示全局 `新记忆` compact action 和 Memory rail 折叠控制。`新记忆` 只打开 Memory 命名弹层，创建成功后才产生 Memory，不承载 Segment 创建入口。
...
- 当前 6 个实体 surface 是 sidebar Memory Space item、loaded workspace titlebar Memory Space title、loaded workspace titlebar current Memory title、MemoryRail Memory item、Memory Studio Segment card 和 Memory Studio SegmentSupplement tab。它们共用 4 个 feature-local typed 菜单壳：`MemorySpaceActionsMenu`（5 项）、`MemoryActionsMenu`（8 项，包含恢复随机默认图片和切换随机默认图片）、`SegmentActionsMenu`（9 项，包含恢复随机默认图片和切换随机默认图片）和 `SegmentSupplementActionsMenu`（7 项），组件位于 `src/renderer/src/workspace/`。Memory Studio primary content tab 使用 `SegmentContentActionsMenu` 表达正文/转录 slot 操作，不承载实体删除或转录生成入口。
...
- Memory Studio 展示空态、按 Segment 投影 `updatedAt` 倒序排列的 Segment 横向预览流、横向浏览按钮、时间轴和当前片段内容区；不重复渲染 Memory 标题、片段数量或总时长 meta。Segment 横向预览流只挂载当前窗口内的真实 Segment item，并用前后 spacer 保持横向滚动范围和选中项定位，避免大量 Segment 时一次性挂载所有 card/menu/timeline 交互树。Memory detail 初次加载时使用同尺寸 poster skeleton strip，而不是文字 loading 块。
...
- 当前片段内容区通过 Segment content Query 读取 selected finalized Segment 的内容：audio Segment 返回本地音频 bytes、已保存 transcript、Tiptap JSON 和两组 baseline hash，note Segment 返回 Markdown 正文、Tiptap JSON 和两组 baseline hash。Audio Segment 的 `转录` tab 在 content Query 成功后常态渲染内联 `LightweightMarkdownEditorSurface`；生成或重新生成转录仍由 Segment card More 菜单承载，后台 mutation 由 App 的手动补转录 running set 表达。
...
- 当前 QueryClient provider 服务 memory space list、记忆空间 snapshot cache、Memory detail cache、selected Segment content cache 和 selected SegmentSupplement content cache。Memory detail cache 用于 Memory Studio 的即时投影渲染；从 sidebar 离开并重新进入同一记忆空间时不显示初始载入态，而是先显示 cached Memory detail，再由新 workspace handle 后台刷新。Memory detail 和 Tiptap-backed selected Segment / SegmentSupplement content 重新挂载、重新选择或收到同 workspace file truth event 时从 main process 文件真源 refetch；same-snapshot body-only 文件变化也 invalidates Memory detail 与 workspace-scoped content Query，让外部 Markdown/sidecar 文件编辑无需点击或刷新即可进入当前 UI。Selected Segment content cache 持有 audio bytes + transcript state 或 note body；selected SegmentSupplement content cache 持有 audio bytes + transcript state 或 note body；Tiptap-backed content cache 同时持有 Markdown、Tiptap JSON 和两组 baseline hash。workspace session close/reopen 时清理。
