...
- 所有内容 tab item 都保留 type icon、label、active pill、More button 和 reorder anchor。active 只表达当前内容选择，使用 active 背景、前景色和 `ring-1 ring-border` 与 inactive tab 区分；inactive hover 只改变前景色，不绘制第二个 pill 背景。More 是 tab button 的 sibling DropdownMenu trigger，不嵌入 tab button。More 的视觉展开使用 owner tab item 的 CSS `group-hover`、More trigger 自身 `focus-visible` 和 DropdownMenu trigger `data-state="open"`；active selection 不让 More 进入隐藏 tab order，不让它视觉常驻展开。DropdownMenu open 期间 More 保持展开，确保 trigger 作为菜单定位 anchor 的几何稳定，避免鼠标移入菜单时菜单左右抖动。未 hover/focus/open 的 More 同步收束 pointer events、tabIndex 和 `aria-hidden`。DropdownMenu 关闭时如果 trigger 已隐藏，焦点回到对应 tab。`转录` / `正文` primary tab 的 More 使用 `SegmentContentActionsMenu`，承载 `用默认应用打开`、`在访达中显示`、`复制相对路径`、`复制绝对路径`、`重命名` 和 `清空转录` 或 `清空正文`；它不承载编辑、生成转录、重新生成转录或删除。清空项使用 destructive 文字语义并继续通过 `WorkspaceDangerConfirmDialog` 二次确认，确认文案说明会把对应内容保存为空且需要手动重新输入或重新生成。重命名只更新 `segment.md` frontmatter `content_title`，不改 Segment 标题或目录名；清空用当前 baseline hash 保存空正文。Finalized 转录、正文、补充笔记和补充录音转录在内容载入后直接显示 Memory Studio 内联 `LightweightMarkdownEditorSurface`；保存转录带当前 transcript text 和 baseline hash，保存成功后复用 transcript save response 更新投影并刷新 exact Segment content Query。SegmentSupplement tab 的 More 使用 `SegmentSupplementActionsMenu` 承载同类实体 More 菜单，audio supplement 额外承载生成或重新生成转录。`生成转录` 和 `重新生成转录` 与 Segment 菜单使用同一 voice settings、recording overlay 和 running disabled 规则；SegmentSupplement 重命名打开 `SegmentSupplementRenameDialog`；SegmentSupplement 删除打开 `SegmentSupplementDeleteDialog`，成功或 `file-written-index-stale` 后都用统一 undo toast 提供恢复入口。内容 tab rail 支持 native drag/drop 重排，`转录` / `正文` tab 和 finalized SegmentSupplement tabs 都可移动；拖拽身份使用 Reo 内部 MIME 和 selected Segment id 校验，More trigger 不参与 drag start，`dragenter` 不改序，`dragover` 以目标 tab 中点计算 before/after 插入位置，同一次拖拽可连续在同一目标前后换位。拖拽期间 action reveal 不接受 target hover 写入：source tab 可保持展开，其他被经过或交换的 tab 不因 drag hover 展开 More。顺序属于 parent Segment 的 durable presentation state：Memory detail projection 读取 Segment manifest 的 `contentTabOrder`，renderer dragover 只维护 pending UI 顺序，drag end 通过 `workspace:updateSegmentContentTabOrder` 写回 `.reo/objects/segments/<segmentId>.json`，成功 response 更新 Workspace snapshot 和 Memory detail cache；该写入不改变 Segment `updatedAt` 或 Memory activity 排序。内容 tab 的 id、panel id、keyboard selection 和 `aria-controls` / `aria-labelledby` 来自同一份 selected Segment content tab model，不用 DOM query 或 click side effect 驱动状态。
...
- Note Segment 不渲染可操作 audio 播放 row 或不可播放文案，但必须保留与主 audio player 等高的不可见布局占位，让 note 和 audio Segment 的 content tab rail、正文/转录正文和编辑 surface 使用同一垂直起点。Note 正文、Note SegmentSupplement 正文、selected audio Segment 转录正文和 Audio SegmentSupplement 转录正文在 content Query 成功后都常态渲染同一个 `LightweightMarkdownEditorSurface`：textarea-first，toolbar 常态可见，用户直接在 textarea 内编辑。Memory、Segment 或 content tab 切换不会自动聚焦 textarea；只有用户点击或键盘进入 textarea 后，编辑容器边框才进入 `ring` 色 focus 状态。正文相对 clean baseline 变 dirty 后才显示 `取消` 和 `保存`，`取消` 丢弃当前本地 dirty 内容并回到 clean 正文；点击保存后立即隐藏 `取消` / `保存`，保存成功后编辑器主动 blur，退出 focus/ring 状态并保持 clean action 隐藏态，保存失败再恢复 dirty action。内联编辑 surface 只允许外层边框颜色变化；toolbar、toolbar button、body 和 textarea 本身保持透明，不继承通用 Textarea 的 `bg-input`、Button disabled 背景或颜色 transition，避免点击或保存时出现短暂填充闪烁。轻量 Markdown 工具栏只向当前 textarea selection 插入 Markdown 字符串，当前覆盖图片占位、项目列表、编号列表、粗体和强调，不提供引用、标题或分割线插入。已 finalized 的普通转录编辑走 `workspace:saveTranscript`；已 finalized 的补充录音转录走 `workspace:saveSegmentSupplementTranscript`；Note Segment 编辑走 `workspace:writeSegmentContent`，Note SegmentSupplement 编辑走 `workspace:writeSegmentSupplementContent`。finalized inline text dirty 时阻止 workspace flow interruption 和窗口关闭，提示先保存当前文本编辑；Note 正文类 stale 保存由当前编辑 surface 的 AlertDialog 处理，转录类 stale 保存显示可见错误并保留当前 textarea 内容。
