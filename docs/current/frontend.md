...
- shadcn/ui 当前 source 范围包含 `components.json`、renderer `@/*` alias、`components/ui/button.tsx`、`components/ui/input.tsx`、`components/ui/label.tsx`、`components/ui/dialog.tsx`、`components/ui/drawer.tsx`、`components/ui/dropdown-menu.tsx`、`components/ui/breadcrumb.tsx`、`components/ui/textarea.tsx`、`components/ui/tooltip.tsx`、`components/ui/separator.tsx`、`components/ui/field.tsx` 和 `lib/utils.ts`。
...
- Active 记忆空间 session 存在且没有录音流程打开时，App 在窗口重新获得 focus 或 document visibility 变为 visible 后调用 `workspace:readWorkspaceSnapshot` 读取当前文件真源。合法的外部 `.reo/workspace.json`、`memory.json`、segment/attachment JSON 或 `transcript.md` 修改会更新 titlebar、MemoryRail 和 snapshot cache，并让 Memory detail、Segment content 和 SegmentAttachment content Query 失效后按需重读；该同步不使用文件 watcher，也不把合法外部编辑显示为错误。录音流程打开时不执行该刷新，避免绕过当前录音的 Workspace/Memory 切换保护。
...
- 当前 `CreateWorkspaceForm` 使用 React Hook Form + Zod resolver、Button/Input/Textarea/Label/Field primitives，并作为可嵌入表单由 `WorkspaceCreateDialog` 承载。当前 `MemoryCreateDialog` 使用 React Hook Form + Zod resolver、Dialog/Input/Button/Field primitives创建 Memory title；`MemoryTitleDialog` 是 Memory title 和记忆空间 title 重命名共用的 Dialog 表单 surface。
...
- 记忆空间列表项容器内右侧有 icon-only 更多操作按钮；记忆空间行容器承载 hover/current surface，primary button 和 more button 都位于同一个记忆空间行容器内。More button 默认隐藏，只在对应记忆空间行 hover、keyboard focus-within 或菜单已打开时显示。当前提供 `重命名记忆空间` 和 `移除记忆空间`。重命名使用共享 `MemoryTitleDialog`，保存成功后以 `workspace:updateMemorySpaceTitle` 返回的 Workspace snapshot 更新当前 titlebar，并同步或 invalidate memory space list；registry 只作为列表投影协调。确认弹层说明只从 Reo 记忆空间列表移除，本地文件夹不会被删除。移除反馈只使用 root `ReoToaster`，确认弹层内不渲染第二套错误提示。移除 inactive 记忆空间只刷新记忆空间列表；移除 active 记忆空间先移除 registry entry，再回到 starter shell，并尽力释放当前记忆空间 handle；handle 释放失败时不阻断列表移除，只显示 toast。
...
- 当前 shadcn/ui source 范围包含 Button、Input、Label、Dialog、Drawer、DropdownMenu、Breadcrumb、Textarea、Tooltip、Separator 和 Field。
- 当前存在 `components.json`、renderer import alias、`components/ui/button.tsx`、`components/ui/input.tsx`、`components/ui/label.tsx`、`components/ui/dialog.tsx`、`components/ui/drawer.tsx`、`components/ui/dropdown-menu.tsx`、`components/ui/breadcrumb.tsx`、`components/ui/floating-action-button-speed-dial.tsx`、`components/ui/textarea.tsx`、`components/ui/tooltip.tsx`、`components/ui/separator.tsx`、`components/ui/field.tsx` 和 `lib/utils.ts`。
