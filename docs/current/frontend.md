...
- NoteEditorOverlay 与 RecordingOverlay 共用沉浸式 `ImmersiveWorkspaceSurface`，后者提供居中与全填两种 stage 布局。NoteEditorOverlay 使用全填布局：编辑 surface 在 titlebar 之下铺满整个窗口、无边框，`保存笔记` 动作位于 titlebar 右侧并在 titlebar 内垂直居中，不再挂载于 editor toolbar；正文使用居中标准可读列宽。返回控件与标题保持原 titlebar 几何。
