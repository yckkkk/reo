...
- Memory Studio 的主片段播放行和 SegmentAttachment 补充录音播放行共享 feature-local `memory-studio-audio-player` pattern：40px tokenized icon control、真实 `Waveform` slider、Geist Mono 时间、相同 grid geometry 和相同 focus/disabled/hover token，不为补充录音另建一套视觉或交互分支。
