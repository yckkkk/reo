...
- Artifact preview 是只读 iframe flow。Memory Studio 只从 Memory detail projection 派生 per-object-origin `reo-artifact://` URL 并渲染 `sandbox="allow-scripts allow-same-origin allow-forms allow-downloads"` iframe；main-owned protocol handler 在 active workspace 和 artifact manifest ownership 下读取 `entry.html`、`runtime.json`、`state.json` 或 `assets/` direct asset，并下发 artifact runtime CSP。作品 iframe 没有 host command channel、postMessage 数据桥、Node/Electron/IPC 或 raw filesystem 权限；普通 Web 网络、browser storage、表单和下载由 iframe sandbox 与 artifact response CSP 控制。
