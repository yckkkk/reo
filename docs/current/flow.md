...
- 当前 IPC request flows 覆盖 `window.reoWorkspace` 的 choose、list memory space entries、initialize、open、open memory space entry、remove memory space entry、close、memory detail、recording draft、audio manifest/chunk read、transcript/reflections save、microphone intent begin/clear。
- 当前 preload/IPC consumer 覆盖记忆空间文件事务基础、create memory space UI flow、MediaRecorder recording overlay、autosave 和 playback。
...
- 当前 Query flow 覆盖 app 启动读取 memory space list、记忆空间 initialization/open 后 invalidate memory space list、memory space ready 后 seed 记忆空间 snapshot cache；snapshot cache 只承载记忆空间身份和 `memories[]` summary 投影；没有 optimistic update。
- 当前 App route flow 使用 renderer in-memory state：无 active memory space 时显示 starter Home shell，创建入口在 sidebar 记忆空间列表；`创建本地记忆空间` 打开受控 `WorkspaceCreateDialog` 弹层；memory space ready 后显示记忆空间 Home；Home memory card 打开 memory detail；memory detail `返回` 和 sidebar 当前记忆空间列表项返回记忆空间 Home；sidebar `首页` 或 `资料库` 在 active memory space 存在时先调用 `closeWorkspace` 释放当前 handle，再进入对应顶层页面；Home `记录记忆` 使用 new-memory recording target；memory detail `继续记录` 使用 existing-memory target，并把当前 `memoryId` 传给 recording finalize request。Finalize 后 App seed 记忆空间 snapshot cache 的 `memories[]` summary，并 invalidate 受影响的 memory detail query；该 route state 不写入 URL、记忆空间文件、Query key 或持久化 store。
...
- Memory space list flow：renderer 通过 TanStack Query 调用 `workspace:listMemorySpaces`；main 校验 sender、channel 和空 payload 后返回不含 root path 的记忆空间元数据。Registry 文件缺失、损坏、schema 不匹配或 symlink leaf 返回空记忆空间列表；不可读 IO 错误返回 `ERR_WORKSPACE_MEMORY_SPACE_REGISTRY_READ_FAILED`，renderer 在当前内容区显示 alert。Registry 写入最多保留最近 100 个记忆空间，title/description display text 有上限；registry 只是记忆空间列表投影，不是记忆空间文件真源。
...
- Memory space session replacement flow：同一个 renderer 窗口只显示一个 active 记忆空间 session。Create/open-local/open-memory-space 成功拿到新 `workspaceHandle` 后，如果已有 active memory space，renderer 必须先调用 `closeWorkspace` 释放既有 handle；既有 handle 释放失败时保留当前记忆空间 UI、显示错误，并尽力关闭新 handle，避免双 lock 泄漏。打开 promise reject 时必须显示可见 alert。
- Memory space creation renderer flow：`创建本地记忆空间` 打开 `WorkspaceCreateDialog`；title、description、selection token 和 displayPath 属于当前 React Hook Form lifecycle；所选目录是 parent location，main 在 parent 下创建 title 同名记忆空间文件夹；OS dialog canceled 不修改 draft 并把 focus 返回 folder picker；同名 child 记忆空间文件夹已存在时显示 create alert，保留 title/description，清除已消费的 selection token 和 displayPath；initialize success seed 记忆空间 snapshot Query cache、关闭创建弹层并设置当前 renderer session state。
- Memory space open-local renderer flow：sidebar `打开本地记忆空间` 单独调用 folder picker；OS dialog canceled 不改变 route state；selected folder 可以是现有 Reo 记忆空间或空文件夹，空文件夹会原地初始化为 Reo 记忆空间；renderer 只发送 selection token 给 `openWorkspace`，不发送 title、description、displayPath 或 raw path；open failure 在当前内容区显示 alert，不打开创建弹层、不清空当前记忆空间 session；open success 先按 记忆空间 session replacement flow 释放既有 handle，再 seed 记忆空间 snapshot Query cache、invalidate memory space list 并设置当前 renderer session state。该动作不进入 `WorkspaceCreateDialog`，也不路由到 `initializeWorkspace`。
...
- 当前 recording overlay flow：home `记录记忆` 或 memory detail `继续记录` 打开 shadcn Drawer/Vaul recording surface；Home/app shell 入口使用 `new-memory` target，finalize request 不传 `memoryId`，由 main 创建新 memory；memory detail 入口使用 `existing-memory` target，finalize request 传当前 `memoryId`，由 main 追加到该 memory。`开始录音` 先 await `beginMicrophoneIntent` 成功，再创建 recording draft，最后启动 browser MediaRecorder adapter；draft 创建失败、media start failure、stale session、组件 unmount 或 记忆空间 handle 切换会 clear matching pending intent；draft 创建成功后如果 session 已 stale，会 discard draft 且不启动 media；只有 MediaRecorder controller ready 后 UI 才进入 recording 并显示 pause/stop；audio data 通过 append queue 串行写入；pause 暂停 MediaRecorder 和 elapsed timer；resume 恢复；stop 先停止 recorder，等待 MediaRecorder `stop` 事件和最后一次 `dataavailable` 转换完成，再等待 append queue 清空，最后 finalize。Finalize 后进入空白 transcript/reflections draft 编辑状态，不生成本地转写文本，并由 App 负责刷新 Workspace snapshot 的 `memories[]` summary 和对应 memory detail cache。非忙碌状态关闭 drawer 会清空 renderer-local drawer draft、error、elapsed、playback URL、recording target 和 machine state，忽略已关闭 session 的 autosave 结果；下一次打开必须从 idle ready state 开始。
- Audio append 必须按 sequence 串行 ack；每个 recording 只允许 1 个 append 在途，超量进入 failed recoverable。当前进程创建的 active draft 可以跳过每个 append 的 durable finalized scan；记忆空间 close、window teardown 或 root runtime state 清理后必须移除该进程态，后续 append 重新检查 finalized truth；stale/manual draft append 必须先检查 durable finalized recording。存在 finalized truth 时返回 finalized error，即使 `.reo/drafts/recordings/<recordingId>/` 中仍有 stale draft。Append 写 chunk 前必须确认 draft `audio.webm` 是 记忆空间内普通文件，拒绝 symlink 或非文件 audio；metadata 写失败时必须 truncate audio 回 append 前 size 并 fsync，返回 `ERR_RECORDING_APPEND_FAILED` 与 `dataRetention: "draft-preserved"`，且不得推进 sequence、audio bytes 或写到 记忆空间外。
- MediaRecorder chunk 转换失败、append 返回错误、append promise reject 或 media start 失败时，recording overlay 立即进入 failed，不调用 finalize，停止当前 recorder controller，discard 当前未完成 draft，并用 recording session token 忽略 stale recorder 后续 chunks；discard draft cleanup 必须重新验证 workspace containment，不能跟随 symlink parent 删除 记忆空间外目录；failed 状态允许用户重新开始一次新的 recording draft，retry 会清空 stale elapsed、transcript draft、reflections draft 和 saved refs。
...
- `.reo/index.json` 损坏、丢失或合法但陈旧时，open/rebuild 会扫描 `memories/*/memory.json`、finalized recording metadata 和 `audio.webm`，只从 schema、status、ownership 和 metadata/audio 一致的 finalized recordings 协调 memory summary 并写回 index；scan 必须绑定 `memories/` root identity，scan 前、scan 后或持久化前 identity 改变时失败并保留原 index；如果 `memories/` 存在但不可读取，open 必须失败并保留原 index，不能把 read error 协调为空记忆空间。Transcript/reflections autosave 只刷新所属 memory 的 index entry，避免每次保存重扫全 workspace；同一 workspace 的全量 index replace 和 index entry refresh 进入同一个 main process write queue。全量 replace 与 open reconciliation replace 都在队列内重新读取当前 read model 后再写入；单条 refresh 在队列内部重新读取当前 memory summary，再按顺序读取当前 index、合并单条 memory summary、atomic 写回。
...
- Memory space write 使用 single-writer lock；第二个 Reo writer 返回 locked error。带 dead owner pid、缺失 owner file、损坏 owner file 或 symlinked owner file 的 stale lock directory 可在下一次 acquire 时被替换；无法确认 owner 仍存活时不做 silent override。
