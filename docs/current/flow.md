...
- Memory space initialize/open flow 必须在获取 single-writer lock 后写入或打开记忆空间。Initialize 在所选父目录下 no-replace 创建 title 同名 child root；open 可打开现有 Reo 记忆空间或把空目录原地初始化。打开现有记忆空间会静默补齐或升级 Reo managed `AGENTS.md` block、`skills/reo-edit/`、`skills/reo-cover-image/`、`skills/reo-cover-aesthetic/`、`skills/reo-works/`、`skills/reo-works-design/` 与 `skills/reo-doctor/`，保留用户自定义 `AGENTS.md` 内容。非空非 Reo、unsafe `.reo`、unsafe draft/root 目录或 invalid metadata 必须返回 typed error，不能留下半初始化状态。
...
- Artifact preview 是隔离 iframe + typed host bridge flow。Memory Studio 只从 Memory detail projection 派生 per-object-origin `reo-artifact://` URL 并渲染 `sandbox="allow-scripts allow-same-origin allow-forms allow-downloads"` iframe；main-owned protocol handler 在 active workspace 和 artifact manifest ownership 下读取 `entry.html`、`runtime.json`、`state.json`、`assets/` direct asset 或 managed vendor bridge，并下发 artifact runtime CSP。作品 iframe 可显式加载 vendor script 获得 `window.reo`；parent router 只接受当前 iframe origin 和当前 iframe `contentWindow` 的 structured request，再调用显式 `window.reoWorkspace` IPC。普通 Web 网络、browser storage、表单和下载由 iframe sandbox 与 artifact response CSP 控制；Node/Electron/raw path、generic filesystem bridge、generic IPC 和任意 product mutation proxy 仍不存在。
...
- Snapshot refresh 执行 passive sidecar reconcile 和 direct Markdown candidate scan 后，必须把无法确定的 sidecar 冲突、invalid/unsupported Tiptap JSON、重复 id、歧义候选、缺失 artifact `entry.html` 入口和 unsupported artifact format 写入 `.reo/review/needs-review.json` / `.reo/review/needs-review.md`；没有 unresolved item 时删除旧 report。Renderer 只收到 snapshot `review` 汇总计数，具体相对路径留在本地 report 和 `reo-doctor` 输出。
