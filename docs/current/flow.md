...
- Artifact runtime state write flow 使用 `state.json` version/baseline。缺失、损坏或超过当前有界读取上限的 `state.json` 按默认 `{ schemaVersion: 1, stores: {} }` fail-open 读取；写入时 baseline 不匹配返回 current state/version，不覆盖外部 agent 或用户刚写入的状态；写入 payload 也必须保持在同一有界读取上限内。Artifact runtime product mutation 只复用现有高频 typed flow：当前作品标题更新和 agent prompt copy；artifact works 不通过 bridge 写 note body。成功后 renderer invalidates Workspace snapshot 与 Memory detail，让文件真源重新投影。
