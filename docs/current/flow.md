...
- 当前 workspace entry flow 覆盖 create workspace title/description/folder selection validation、sidebar `打开本地工作区` 打开现有 Reo 工作区、submit failure 保留 create draft 和成功后 route state 切换。
...
- Workspace creation renderer flow：`新建空白项目` 打开 `WorkspaceCreateDialog`；title、description、selection token 和 displayPath 属于当前 React Hook Form lifecycle；OS dialog canceled 不修改 draft 并把 focus 返回 folder picker；existing `AGENTS.md` conflict 显示 create alert，保留 title/description，清除已消费的 selection token 和 displayPath；initialize success seed workspace snapshot Query cache、关闭创建弹层并设置当前 renderer session state。
- Workspace open-local renderer flow：sidebar `打开本地工作区` 单独调用 folder picker；OS dialog canceled 不改变 route state；selected folder 必须是现有 Reo 工作区，renderer 只发送 selection token 给 `openWorkspace`，不发送 title、description、displayPath 或 raw path；open success seed workspace snapshot Query cache 并设置当前 renderer session state。当前 UI 不使用旧“添加工作区”选择弹层，也不把该动作路由到 `initializeWorkspace`。
