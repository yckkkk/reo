...
- Artifact preview 是只读 iframe flow。Memory Studio 只从 Memory detail projection 派生 `reo-artifact://` URL 并渲染 `sandbox="allow-scripts"` iframe；main-owned protocol handler 在 active workspace 和 artifact manifest ownership 下读取 HTML 入口或 sibling asset，并下发 artifact response CSP。作品 iframe 没有 host command channel、postMessage 数据桥、Node/Electron/IPC 权限或网络出口。
...
- Snapshot refresh 执行 passive sidecar reconcile 和 direct Markdown candidate scan 后，必须把无法确定的 sidecar 冲突、invalid/unsupported Tiptap JSON、重复 id、歧义候选、缺失 artifact HTML 入口和 unsupported artifact format 写入 `.reo/review/needs-review.json` / `.reo/review/needs-review.md`；没有 unresolved item 时删除旧 report。Renderer 只收到 snapshot `review` 汇总计数，具体相对路径留在本地 report 和 `reo-doctor` 输出。
