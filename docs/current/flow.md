...
- Workspace snapshot refresh 只在 ready 后和 document visibility 变为 visible 时协调外部合法文件修改。合法 `.reo/index.json` 是启动 cache；只有 index 丢失、损坏或 unsafe 时才 rebuild。普通 title update 和单条内容保存不为 response 全量扫描 workspace。
