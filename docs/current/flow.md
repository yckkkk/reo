...
- Memory cover 和 Segment cover reset/restore 是真实 request/response mutation。Reset 只在当前对象存在自定义 `cover/` 时把该 cover 目录移动到 `.reo/trash/memory-covers/<restoreToken>` 或 `.reo/trash/segment-covers/<restoreToken>`，刷新对应 Memory summary 与 Segment projection，并显示可撤销 toast；restore 只用同一对象的 restore token 把 cover trash 目录恢复为 active `cover/`。Agent 或用户在 `cover/` 中直接替换合法图片文件时，main-owned file truth watcher 触发 snapshot/detail refresh，并通过 cover projection version 让 renderer 重新加载图片。
