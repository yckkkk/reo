...
- Memory space initialize/open flow 必须在获取 single-writer lock 后写入或打开记忆空间。Initialize 在所选父目录下 no-replace 创建 title 同名 child root；open 可打开现有 Reo 记忆空间或把空目录原地初始化。打开现有记忆空间会静默补齐或升级 Reo managed `AGENTS.md` block、`skills/reo-edit/`、`skills/reo-memory-cover/`、`skills/reo-cover-aesthetic/` 与 `skills/reo-doctor/`，保留用户自定义 `AGENTS.md` 内容。非空非 Reo、unsafe `.reo`、unsafe draft/root 目录或 invalid metadata 必须返回 typed error，不能留下半初始化状态。
...
- Memory cover reset/restore 是真实 request/response mutation。Reset 只在当前 Memory 存在自定义 `cover/` 时把该 cover 目录移动到 `.reo/trash/memory-covers/<restoreToken>`，刷新 Workspace snapshot 的 Memory summary，并显示可撤销 toast；restore 只用该 restore token 把 cover trash 目录恢复为 active `cover/`。Agent 或用户在 `cover/` 中直接替换合法图片文件时，main-owned file truth watcher 触发 snapshot refresh，并通过 cover projection version 让 renderer 重新加载图片。
