...
- Artifact prompt-bridge 是 request/response clipboard flow。FAB `作品`、content tab rail `作品补充` 和已有作品 More 的“让 Agent 更新作品”只向 main 发送当前 workspace、Memory、Segment 或 Supplement identity；main 校验 trusted sender、active handle、workspaceId 和目标文件空间目录，生成包含相对路径、文件合同和托管 skill 入口的提示词并写系统剪贴板。该 flow 不创建 draft、不写空对象、不返回 raw path、不写 Query cache，也不进入录音或笔记 finalize 流程。
- Workspace Widget prompt-bridge 是 request/response clipboard flow。Titlebar 右侧 `新增` 菜单的 `新建 Widget` 和已有 Widget More 的“让 Agent 更新 Widget”只向 main 发送当前 workspace 或 Widget identity；main 校验 trusted sender、active handle、workspaceId 和目标文件空间目录，生成包含 `widgets/` 相对路径、`widget.md` 合同和 runtime bundle 合同的提示词并写系统剪贴板。该 flow 不创建 draft、不写空 Widget、不返回 raw path、不写 Query cache，也不进入录音、笔记 finalize 或 Segment content flow。
