...
- 如果进程在 finalized metadata 写入后、`.reo/index.json` 写入前崩溃，后续 open 会把该 finalized recording 恢复到记忆空间 snapshot。
