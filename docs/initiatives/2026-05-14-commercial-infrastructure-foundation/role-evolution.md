...
- 功能被模型能力变化重新评估，而不是固定在短期模型限制上。
- 复杂系统优先简化，避免为短期模型限制堆复杂流程。
...
每次重要模型或 provider 变化后，保留一份轻量能力检查：

- 当前失败样本是否变好。
- 当前提示词、流程或补丁式步骤是否仍然必要。
...
- 一页 AI-native PM operating model：问题、demo、eval、内测、路线图、模型能力检查。
- 一页 human-AI collaboration map：用户、AI、证据、确认、修正、阻断。
- 一页 FDE-shaped field loop：用户现场、原型、部署边界、反馈、playbook、产品路线图。
- 一页 model release review：模型变化如何影响 Reo 的已实现能力、计划能力和 eval。
