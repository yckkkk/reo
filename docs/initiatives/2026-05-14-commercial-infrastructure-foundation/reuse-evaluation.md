...
- 未来 LLM Gateway 评估 LiteLLM；当前豆包语音实时 ASR 与文件 ASR 需要 Reo 服务端 adapter。
...
| AI Gateway    | LiteLLM                                      | LLM provider routing、virtual keys、budget、rate limits、spend tracking                          | 当前豆包语音实时 WebSocket 和文件识别 HTTP 接口不能直接套用 OpenAI-compatible proxy |
...
ASR gateway: Reo provider adapter for Doubao Speech realtime/file ASR
...
- 豆包语音 ASR 的实时连接、文件补转录、时长计费和失败回滚能否进入同一 usage ledger。
