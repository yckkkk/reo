...
本计划承接 2026-05-16 brainstorm 收口共识。E 已完成；B 已完成、归档并通过真机 E2E gate；C 已按普通个人用户只配置 X-Api-Key 的约束完成、归档并通过真实 Electron E2E/QA；C→D readiness gate 已完成；D active spec 已创建：`docs/specs/2026-05-17-0950-doubao-voice-manual-regenerate-transcript/`。剩余工作是 D 实施 session 按该 spec tasks.md 执行。
...
- 当前 active spec：`docs/specs/2026-05-17-0950-doubao-voice-manual-regenerate-transcript/`
- 当前状态：D active spec 已创建；下一步是 D 实施 session 按 spec tasks.md 推进。
- D 的产品、状态机、合同字段、错误码、TDD/subagent/review/E2E 计划以 active spec 为准；d-brief.md 仍是产品意图原文。
- D 复用 C 的 Turbo 引擎、音频转换、队列、诊断和安全边界；D 不复用 C 当前 missing-only 手动触发语义来覆盖已有 transcript。Active spec 中的 `mode: 'fill-missing' | 'regenerate'`、`ERR_BACKFILL_TRANSCRIPT_CHANGED` 与 main 端 transcript snapshot guard 是当前唯一允许的覆盖式重新生成路径；自动 scanner 永远只入队 fill-missing。
...
- **C→D readiness gate**：已完成。C 归档 spec、current 相关段落、d-brief 与代码事实已重读；结论是 C 的引擎、队列和安全边界足以支撑 D，但 C 当前手动触发合同只允许 missing-only 补转录并用 `requireTranscriptMissing` 防止覆盖。结论已固化在 D active spec `docs/specs/2026-05-17-0950-doubao-voice-manual-regenerate-transcript/`：扩展两个现有 backfill IPC 合同的 `mode: 'fill-missing' | 'regenerate'` 字段、新增 `ERR_BACKFILL_TRANSCRIPT_CHANGED` 错误码、在 main 端 regenerate 路径加入 transcript snapshot guard，自动任务保持 missing-only。
