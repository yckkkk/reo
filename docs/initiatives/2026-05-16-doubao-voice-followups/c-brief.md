...
- C-0：离线 flash endpoint 探针调研（独立技术调研子任务）
...
- C0 探针通过：离线 flash endpoint 可用、key 复用合法、单次时长上限已知
...
| `running`          | 队首；正在调引擎                                                 | 引擎执行                                      | transcript view 显示「正在生成」覆盖按钮     | 成功 → `succeeded`；失败 → `failed-retryable` |
...
- 进入条件：队首任务开始执行
- 页面表现：transcript view 显示「正在生成」覆盖重试按钮
- 用户能做什么：等待 / 切到其他 segment
- 用户不能做什么：取消（不暴露 UI）
- 系统正在做什么：HTTP POST 到 flash endpoint；接收 sync response；调 saveTranscript
...
| 元素                    | 解决问题                                                             | 实现要点                                                                                              |
| ----------------------- | -------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------- |
| `C0FlashClient`         | 调 `POST /api/v3/auc/bigmodel/recognize/flash`，处理 base64 编码音频 | 单文件 `src/main/c0FlashClient.ts`；no concurrency；abort 支持                                        |
...
| `BackfillEventBus`      | 把 enqueue / running / succeeded / failed 事件推给 renderer          | 复用现有 callback-based main→renderer 事件机制（如 recordingTranscriptionEvent 同款，但单独 channel） |
...
- 事件流：新增 `workspace:backfillEvent` 单一事件 channel；按 sender / workspaceHandle / taskId 绑定；event payload 不含 raw path 或 transcript text
...
- `workspace:backfillEvent`（main → renderer callback）
  - payload: `{ workspaceId, segmentId | supplementId, status: 'enqueued' | 'running' | 'succeeded' | 'failed' | 'canceled', errorCode?: string }`
  - 不返回 transcript text、不返回原始错误 message
...
| 项                           | 内容                                                                                          |
| ---------------------------- | --------------------------------------------------------------------------------------------- |
| 使用哪个能力                 | 火山引擎 离线极速版 ASR `POST /api/v3/auc/bigmodel/recognize/flash` + `volc.bigasr.auc_turbo` |
...
| 异步结果如何回写             | flash endpoint 是 sync 同步返回，不需 long-polling                                            |
...
1. **endpoint 可用性**：构造最小 POST request（极短 audio sample，如 1 秒静音），用现有 BYOK key 调用 `https://openspeech.bytedance.com/api/v3/auc/bigmodel/recognize/flash`
2. **resource id 一致性**：header 用 `X-Api-Resource-Id: volc.bigasr.auc_turbo`，确认 200 返回
...
5. **回落策略可行性**：若官方上限覆盖典型 Reo 录音（≤ 60min），暂不需要异步 `submit`/`query`；若需要，记录回落策略
...
C 的最终交付是：让用户在凭证恢复 / 启动 Reo / 切换 workspace 时，所有"上次系统失败、转录仍空"的 segment 与 supplement 自动按串行 FIFO 后台补齐转录，不需要用户主动点任何按钮；整个流程对用户透明，仅在每条 segment / supplement 的转录区显示「正在生成」覆盖原"重试"按钮，成功后 transcript 文本出现、manifest 自动置 `'success'`；失败保留 `'failed'`，下次触发上升沿继续。后台引擎使用火山引擎离线极速版 `POST /api/v3/auc/bigmodel/recognize/flash` + `volc.bigasr.auc_turbo`（C-0 探针必须先确认 endpoint 可用、key 复用、单次时长上限），任务持有当前 active workspace handle，切 workspace / lock lost / app quit 即中断并清空队列；录音中暂停队列但不取消。复用现有 `workspace:saveTranscript` 与 `workspace:saveSegmentSupplementTranscript` 写回，依赖 B 的 `lastTranscriptionAttempt` manifest 字段判断 eligible 集合。C 同批建立两个边界：`workspace:backfillEvent`（main → renderer callback）向 UI 发布中间态；renderer 手动触发合同把 D 与 B 的 inline 重试请求送入同一 `BackfillQueue`，但不实现 D 菜单、确认弹层、任务取消或第二套排序规则。新增的唯一外部依赖是 C0 验证后的离线 flash engine 实现。验收依据 C spec verification.md：scanner 正确收集 failed-eligible 集合、串行队列正确执行、HTTP 成功调 saveTranscript / 失败保留 manifest、workspace 切换正确取消、录音中正确暂停、saveTranscript 成功后 manifest 自动 'success'、手动触发同 target 去重且不抢占、TypeScript strict 与 `npm run verify:quick` 全绿；本 spec 与 `docs/current/electron.md` / `docs/current/flow.md` / `docs/current/frontend.md` 必须在收口时同批更新。
