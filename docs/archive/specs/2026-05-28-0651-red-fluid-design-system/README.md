...
把 Reo 的设计系统从「黑色为主的 Soft Flat」演化为「**品牌红表达入口** + **黑白中性控件语义** + **Fluid Functionalism 多层 surface 骨架** + **保留扁平阅读层**」的统一系统。

- **品牌红**承载 Reo 的"录音/记录"与 Hero 表达入口语义，不占用普通 `primary` 或 `ring`。
- **黑白中性控件语义**承载普通 primary action、Switch checked、status dot 和 focus ring；light 为近黑，dark 为近白。
- **Fluid 多层 surface** 在容器层提供 4 级 elevation（surface-1..4），在表达入口层（FAB、RecordingOverlay、MemoryIcon、Segment 渐变卡）提供 Hero 表面。
...
| token              | light                                                                                             | dark            | 用途                                                                  |
| ------------------ | ------------------------------------------------------------------------------------------------- | --------------- | --------------------------------------------------------------------- |
| `--brand-red`      | `#dc2626`                                                                                         | `#dc2626`       | 品牌主红，brand / recording / Hero raw 资产                           |
...
| token                      | light                                                                        | dark                | 用途                                         |
| -------------------------- | ---------------------------------------------------------------------------- | ------------------- | -------------------------------------------- |
| `--primary`                | `#18181b`                                                                    | `#fafafa`           | 普通主动作、Switch checked、状态点的中性填充 |
| `--primary-foreground`     | `var(--background)`                                                          | `var(--background)` | 主动作文字                                   |
| `--primary-hover`          | `color-mix(in oklab, var(--primary) 86%, var(--background))`                 | 同                  | primary hover                                |
| `--ring`                   | `var(--primary)`                                                             | `var(--primary)`    | 中性 focus ring                              |
| `--destructive`            | `#b91c1c`                                                                    | `#b91c1c`           | 危险动作                                     |
...
| `Button` primary                          | `bg-primary` 黑                   | `bg-primary` light 近黑 / dark 近白                                                                           | 否（token）                          |
...
2. **Hash hue 只着色**：只控制 MemoryIcon、MemoryRailCard 选中态轻量 tint、Segment 选中态轻量 tint。不改文本色、不改 focus ring。chrome 指 AppShell titlebar、Sidebar 背景、ExpressionDock、Settings nav rail——这些层永远品牌表达入口 / 灰阶，不接受 Memory hue。
...
| Phase                                | 范围                                                                                                                                                                                                                                                       | 风险   | TDD?                                                                                       |
| ------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ | ------------------------------------------------------------------------------------------ |
| **1 · Tokens**                       | 改 `theme.css`、`variables.css`、`tokens.json`、`design-system/DESIGN.md`：新 surface-1..4、新 hero shadow、新 radius、neutral primary/ring、destructive 深红；Task 8 追加最小 Button hover consumer、Tailwind source scanning 约束和 neutral primary 纠偏 | 低-中  | Button hover contract + token mirror contract                                              |
...
| **6 · 文档收口**                     | 更新 `docs/current/frontend.md` 加 surface-1..4 / MemoryIcon / Hero 三处映射；更新 `design-system/DESIGN.md`；归档本 spec                                                                                                                                  | 低     | 否                                                                                         |
...
- Phase 1 单独 ship：普通控件保持黑白中性，品牌红保留在表达入口与危险动作之外的 brand raw 资产，功能零退化
...
3. 一个 token 一个用途：同一 token 不得同时承载 raw 值和 semantic 角色；如需桥接，写 `--ring: var(--primary)`，而不是让 raw brand token 直接充当普通控件语义。
4. 不引入无 owner 的 token。无当前消费者的 raw 资产或 semantic 角色，必须在本 active spec 中绑定明确 owner phase；否则不进入设计系统。
...
| 模式                  | 示例                                                                                                                           | 规则                                                    |
| --------------------- | ------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------- |
| `--<role>`            | `--background`、`--card`、`--popover`、`--primary`、`--secondary`、`--destructive`、`--input`、`--muted`、`--accent`、`--ring` | UI 角色名，非颜色名。可引用 raw 资产或承载稳定中性色    |
...
1. 起草 **Phase 2 (MemoryIcon primitive)** plan
2. Phase 2 ship 后再按同样方式进入 Phase 3
