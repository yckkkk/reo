...
- 本文件是 design-hardening spec 内产出的 reconciled implementation plan；active implementation authority 已转交到 `docs/initiatives/2026-05-06-first-product-slice/implementation-plan.md`。
...
本文件在 design-hardening spec 内完成 `$writing-plans` 和 `$plan-eng-review` 门禁。两个门禁均已 PASS；进入 Task 1 前必须先完成 plan handoff gate：

- 将最终可执行任务清单同步到 `docs/initiatives/2026-05-06-first-product-slice/plan.md` 和 `tasks.md`，使 active initiative 承接后续实现；initiative plan 是 implementation 阶段的当前执行权威。
- 更新本 design spec 的 `review.md` 和 `verification.md`，记录 writing-plan review、plan-eng-review、验证命令和 handoff 结论。
