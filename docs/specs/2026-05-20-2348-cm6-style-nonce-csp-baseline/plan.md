...
- 新增 `generateStyleNonce(): string`（`node:crypto` `randomBytes(16).toString('base64url')`；选 base64url 以避免 `+ / =` 进入 meta 属性）。**CSP nonce 是严格字符串匹配**：CSP header 的 `'nonce-${nonce}'`、index.html meta content、CM 注入 `<style>` 的 nonce 必须是**完全相同的字符串**，不做任何编码转换/解码。该字符串被 Chromium CSP 接受**以 §19.3 runtime 证据为准**（不依赖未引用的标准结论）；若 runtime 显示不被接受，改用标准 base64 并对 meta 属性做转义。
