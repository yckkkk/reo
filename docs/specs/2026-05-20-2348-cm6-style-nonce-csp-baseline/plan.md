...
- 新增 `generateStyleNonce(): string`（`node:crypto` `randomBytes(16).toString('base64url')`；base64url 合法且避免 `+ / =` 进 meta 属性）。**CSP nonce 是严格字符串匹配**：CSP header 的 `'nonce-${nonce}'`、index.html meta content、CM 注入 `<style>` 的 nonce 必须是**完全相同的字符串**，不做任何编码转换/解码。验收 §19.3「正确 nonce 样式生效」即对该字符串被 Chromium 接受的运行时证明。
...
- 非 app page（非 `reo-app://`、非 dev origin）维持现有放行。dev 行为与现状一致。
...
9. **需新增/调整类型/函数**：`createProductionDocumentCsp(nonce: string): string`、`isAppDocumentPath(pathname: string): boolean`、`generateStyleNonce(): string`、`injectStyleNonce(html, nonce): string`、`resolveAppPageCspAction({ usesDevServer, isTrustedAppDocument }): 'passthrough' | 'apply-static'`（host-aware：`isTrustedAppDocument = hostname === APP_SHELL_HOST && isAppDocumentPath(pathname)`）。
...
17. **测试**（真实 TDD，先红后绿，main 用 `node:test`@`test/main`）：
    ...
    - **源码级断言**（仿 `test/main/appProtocol.test.ts` 的 TS-AST/文本断言）：`setupContentSecurityPolicy` 确实调用 `resolveAppPageCspAction`，且 `passthrough` 分支回调原 `responseHeaders` 不写 `Content-Security-Policy`。
18. **实现步骤**：
    1) **先 de-risk A2/runtime（throwaway scratch diff）**：临时在协议处理器文档分支硬编码一个 CSP+meta 跑 `npm run build` + `npm start`，DevTools 确认文档响应头 CSP 抵达且未被覆盖、meta 存在。**该临时 diff 仅作探针：记录证据后必须立即还原，不进入最终提交、不作为 GREEN 实现、不绕过 RED 测试。** 探针通过后再进入第 2 步 RED。
