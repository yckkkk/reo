...
- 当 d-brief.md 与本 spec 冲突时，以本 spec 为准；下面三条已知差异由本 spec 锁定。
...
- d-brief §12 "confirming 期间任务被 C 自动入队"描述"D confirm 仍生效"。本 spec 锁定为：confirm mutation 仍发出，但 main 端按 BackfillQueue dedup 返回 `ERR_BACKFILL_ALREADY_RUNNING`，renderer 关闭 Dialog 并 root toast。两条描述行为等价，本 spec 锁定后者措辞，避免 implementer 误读为"renderer 端先 silent skip mutation"。
- d-brief §12 "多 supplement 同 segment"未列入本 plan §14 异常表。理由：每个 supplement 是独立 backfill target，dedup / 状态 / Dialog 都按 target identity 计算，默认 supplement 独立行为已天然涵盖；本 spec 不视其为边界场景，也不在 §14 单列行。
...
3. `SegmentTranscriptView` 在 regenerate running 期间**必须继续渲染 selected segment 当前 transcript text**（非空时保留正文，空时按 fill-missing running 一致渲染空态文案），并叠加进行中文案；不允许"transcript 非空时把已有正文隐藏成空白"。具体实现路径（新增 `running-overwrite` outcome、给现有 `running` outcome 加 `keepBody` 参数、或在 Memory Studio 层把 transcript 文本与 running 文案并列渲染）由 implementer 在 Stage 5 RED 阶段按 Reo 设计系统选择；选择哪条路径不影响 verification §2.4 对 "running 期间不主动隐藏既有 transcript 正文" 的行为断言。
...
下面三条本 spec 已锁定为工程默认；implementer 按本默认实施，不需要在实施 session 重新讨论。只有用户在实施前显式提出新意图时，再回到本 spec 调整 plan 与 verification。

1. `lastValidationCode='network'` 时菜单项**可点击**，让用户手动尝试。理由：`network` 不是凭证错误，与"网络瞬态"语义一致，UI 不预先 disable，由 mutation 实际命中错误时通过 root toast 提示。
2. 当 segment/supplement 同时存在 automatic queued/in-flight 任务时，renderer 端**不 disable 菜单**，依靠 main 返回 `ERR_BACKFILL_ALREADY_RUNNING` + root toast。理由：renderer 没有自动队列状态订阅，且本 spec 不允许新增 main-to-renderer event channel；让 main 端 dedup 成为单一权威。
3. confirming Dialog 期间用户切换 segment 时**立刻关闭 Dialog 并丢弃 confirm intent**，与现有 SegmentSupplementDeleteDialog 切换 segment 的行为对齐。理由：confirm 绑定具体 segment identity，切换 segment 后该 intent 已与当前焦点脱钩，继续保留会让用户在新 segment 上误确认旧 target。
...
| `running`               | 任务出队，main in-flight                                                                                                   | `SegmentTranscriptView` 的 running outcome；regenerate 路径继续显示当前 transcript 文本         | /                                                                | /                                      | main: 读音频 → ffmpeg → Turbo → save                                                 | transcript 暂未变化；regenerate 路径捕获 digest                                                                                                   | save 完成（成功或失败）                                             | recognize/save 失败：见失败状态                                                                                        | regenerate running 时非空 transcript 必须继续渲染原正文（空 transcript 时按 fill-missing running 同样空态展示）；fill-missing running 时 transcript 区域使用 B 已有 running outcome 文案   |
...
| `SegmentTranscriptView` running outcome           | 表达 backfill 进行中           | App manual running Set 包含当前 target | task settle                           | 现有 running outcome          | /           | /      | /                                                                                          | /                                                                                              | 静态 running 文案                                                                                | 不与 retry 按钮共存；retry 按钮仅在 failed-retryable outcome 出现                                                                                    | regenerate 期间按 §2.3 假设 3 锁定行为渲染：transcript 非空时保留正文叠加 running 文案，transcript 为空时按 fill-missing 空态渲染；实现路径自由 | running 文案与 B 一致；切回 success outcome 后 transcript 文本反映新结果                           |
...
- 主进程：
  - `src/main/backfillRuntime.ts`：
    ...
    - **regenerate overwrite-save 锁内顺序不变量（hard invariant）**：「重新读取当前 transcript → 重算 digest → 与捕获的 snapshot digest 比对 → ownership 复核（调用 `assertWorkspaceUsable()` 校验 lock / handle / workspaceId / memoryId / segmentId(/supplementId) 全量 ownership 仍有效）→ 覆盖写 `segment.md` / `supplement.md` 正文 → 写 manifest `lastTranscriptionAttempt='success'`」必须在 **同一段持有 workspace single-writer lock** 的 critical section 内完成，不允许在比对与写入之间释放并重获 lock；否则比对窗口外的外部编辑会让 TRANSCRIPT_CHANGED 保护失效。lock lost 或 ownership 变化导致 critical section 中断时，必须返回 typed error（lock-lost / ownership 错误）并放弃覆盖写。helper 必须在覆盖写文件前**主动调用** `assertWorkspaceUsable()` 至少一次，使 mock 在该调用点返回 lock-lost 时可以阻止后续写入，verification §2.2 的锁内顺序断言依赖该调用点存在。
    - **cancel 与 abort 检查点**：在 (a) digest 捕获前、(b) recognize 完成后但锁未拿到前、(c) 拿锁后但首次重读 transcript 前、(d) 写文件前，至少各检查一次 `signal.aborted`；命中即返回 `canceled` errorCode，不读 / 不写 / 不改 manifest，与 C `backfillRuntime.ts` 已有 abort 分支位置（行 381 / 416 / 422 附近）保持同一节奏。
  ...
  - `src/main/recordingDrafts.ts` 或保存 helper：暴露 `allowOverwrite` 旗标，跳过 `requireTranscriptMissing` 检查；ownership / lock / digest 复核保持；helper 必须把"复核 + 比对 + 写入 + 改 manifest"封装在同一持锁段内，调用方不允许把 digest 比对和 overwrite 写入拆到两次 lock acquire 中。
