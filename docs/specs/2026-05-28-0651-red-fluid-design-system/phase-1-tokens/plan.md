...
**Goal:** Replace Reo's existing「黑色为主 Soft Flat」token system with spec Section 1 + Section 6 的「品牌红 + Fluid 多层 surface」骨架，遵循新命名规范；零 TSX 改动，shadcn 组件通过 token 自动消费新颜色。

**Architecture:** 双层 token 模型：raw 资产（`--brand-*`、`--surface-N`、`--shadow-hero-*`、新增 `--radius-3xl/4xl`）+ semantic 角色（shadcn 既有 `--primary`、`--card`、`--input` 等通过 `var(--surface-N)` 引用 raw 层）。tokens.json 与 variables.css 是设计系统源；theme.css 是 runtime 投影；DESIGN.md 是叙述；index.css 增 `@utility bg-brand-gradient`。

**Tech Stack:** Tailwind CSS v4（`@theme inline` + `@utility`）、CSS custom properties、color-mix(in oklab)、shadcn 既有 token API。

**TDD 红线判断（依 CLAUDE.md）：** Phase 1 = 机械配置 + 重命名 + 纯 token 替换 → TDD 豁免。验证靠 `npm run verify:quick` + `npm run dev` 视觉 smoke。
...
Run: `cat src/renderer/src/index.css | head -20`
确认当前 `@utility` 块的位置（reo-card-squircle 和 reo-segment-card-squircle 已存在）。

- [ ] **Step 2: 在 `reo-segment-card-squircle` utility 之后追加 `bg-brand-gradient`**

把 `index.css` 中这一段：

```css
@utility reo-segment-card-squircle {
  border-radius: clamp(33px, 4.34vw, 36px);
...
- [ ] **Step 3: 在 `implementation-notes.md` 追加 Task 5 证据**

```
## Task 5 · 新增 bg-brand-gradient utility
- 时间：<执行时间> PDT
- 改动：src/renderer/src/index.css（追加一个 @utility）
- 验证：Tailwind v4 编译后业务可使用 `className="bg-brand-gradient"`；gradient 不进入 @theme inline 是 Tailwind v4 当前最佳实践（gradient 不是 color，需自定义 utility）
- 备注：此 utility 在 Phase 4 (FAB) 与 Phase 5 (RecordingOverlay) 才有真实 consumer；Phase 1 提前建立可用性
```

- [ ] **Step 4: Commit**

```bash
git add src/renderer/src/index.css docs/specs/2026-05-28-0651-red-fluid-design-system/phase-1-tokens/implementation-notes.md
git commit -m "feat(theme): expose bg-brand-gradient Tailwind utility"
```
...
1. AppShell sidebar 在 light / dark 两个主题下渲染正常，"新建记忆空间"按钮变红（`bg-primary`）
2. WorkspaceStarterHome、loaded WorkspaceFrame 切换正常
3. 切换 light → dark → 跟随系统三个偏好，主题正确切换
4. 主动作按钮（保存笔记、Memory create dialog 的提交）显示品牌红
5. 焦点环（点击 Input 后 Tab 切走再回来）为品牌红
6. 删除确认 AlertDialog 中的"删除"按钮是深红 `#b91c1c`，与 primary 红有明显亮度差
7. Switch（语音设置中的"启用豆包语音识别"）checked 时为品牌红
...
Expected: 无 correctness bug、无未使用 token、无 raw 层泄漏到业务消费点（本 phase 不应该有业务消费点）。
...
- [ ] **Step 3: 在 `implementation-notes.md` 追加 Task 8 证据**
...
- [ ] **Step 4: 最终 Commit（如有 simplify 改动）**
...
**4. 风险：** `--shadow-X: var(--shadow-X);` 在 `@theme inline` 中的自引用模式与现有 `theme.css` 一致；这是 Tailwind v4 对动态 CSS 变量的标准写法。`bg-brand-gradient` 作为 `@utility` 而非 `@theme inline` 内的 `--gradient-*` 是 deliberate（Tailwind v4 没有 native gradient theme API）。
